...
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v101.py``."""
...
rec = Economy(Config.v101(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)).run()
t = [r["t"] for r in rec]
...
infl_target = float(getattr(Config.v101(), "inflation_target", 0.0))
...
fig.suptitle(f"macro-simulator v10.1 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(central bank 'gentle' + deposit-proportional interest; mean[{BURN}:] policy rate={rr:.3f}, "
             f"inflation={ii:.4f} (target {infl_target:.3f}), u={uu:.3f}; A5 drift {drift:.1e})",
             fontsize=13, y=0.999)
fig.tight_layout(rect=(0, 0, 1, 0.997))
fig.savefig("diagnostic_v101.png", dpi=105)
print("wrote diagnostic_v101.png")
