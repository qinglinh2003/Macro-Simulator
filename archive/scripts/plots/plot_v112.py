...
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v112.py``."""
...
rec = Economy(Config.v112(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)).run()
t = [r["t"] for r in rec]
...
infl_target = float(getattr(Config.v112(), "inflation_target", 0.0))
...
n_banks = len(rec) and int(getattr(Config.v112(), "n_banks", 1))
fails = rec[-1].get("n_bank_failures", 0.0)
lev = np.mean([r.get("bank_leverage", 0.0) for r in rec[BURN:]])
fig.suptitle(f"macro-simulator v11.2 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
...
fig.savefig("diagnostic_v112.png", dpi=105)
print("wrote diagnostic_v112.png")
