"""v11.3 full-economy diagnostic: one long run, as many indicators as fit on a 12x4 grid.
Groups: core macro / real activity / money+drain / credit / equity / distribution / v9 government /
v9.1 supply side / v9.3 labor+welfare / v10 MONETARY / v11 BANKING SECTOR / v11.3 COMPETITION.
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v113.py``."""
...
CFG = Config.v113(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)
rec = Economy(CFG).run()
t = [r["t"] for r in rec]
...
fig, ax = plt.subplots(12, 4, figsize=(22, 48), sharex=True)
...
hhi = np.mean([r.get("bank_loanbook_hhi", 0.0) for r in rec[BURN:]])
sd = np.mean([r.get("bank_rate_spread_sd", 0.0) for r in rec[BURN:]])
fails = rec[-1].get("n_bank_failures", 0.0)
fig.suptitle(f"macro-simulator v11.3 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(full stack + {n_banks} BANKS with LOAN-RATE COMPETITION; mean[{BURN}:] u={uu:.3f}, "
             f"policy rate={rr:.3f}, inflation={ii:.4f}; bank leverage={lev:.2f}, HHI={hhi:.3f} (even {even:.3f}), "
             f"rate-disp SD={sd:.4f}, failures={fails:.0f}; A5 drift {drift:.1e})",
             fontsize=12, y=0.999)
fig.tight_layout(rect=(0, 0, 1, 0.997))
fig.savefig("diagnostic_v113.png", dpi=105)
print("wrote diagnostic_v113.png")
print(f"mean[{BURN}:]  policy rate={rr:.4f}  inflation={ii:.4f}  u={uu:.4f}  "
      f"HHI={hhi:.3f} (even {even:.3f})  rate-disp SD={sd:.4f}  failures={fails:.0f}  A5 drift={drift:.1e}")
