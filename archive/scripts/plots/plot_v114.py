"""v11.4 full-economy diagnostic: one long run, as many indicators as fit on a 13x4 grid.
Groups: ... / v11 BANKING SECTOR / v11.3 COMPETITION / v11.4 RESERVE TIER + INTERBANK + DEPOSIT COMPETITION.
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v114.py``."""
...
CFG = Config.v114(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)
rec = Economy(CFG).run()
t = [r["t"] for r in rec]
...
fig, ax = plt.subplots(13, 4, figsize=(22, 52), sharex=True)
...
fig.suptitle(f"macro-simulator v11.4 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(full stack + {n_banks} BANKS + RESERVE TIER/RTGS + INTERBANK (latent) + DEPOSIT COMPETITION; "
             f"mean[{BURN}:] u={uu:.3f}, policy rate={rr:.3f}, inflation={ii:.4f}; deposit-HHI={dep_hhi:.3f} "
             f"(even {even:.3f}), peak intraday overdraft={peak_od:.1f} (≈0 ⇒ IB latent), failures={fails:.0f}; "
             f"A5 drift {drift:.1e})", fontsize=12, y=0.999)
fig.tight_layout(rect=(0, 0, 1, 0.997))
fig.savefig("diagnostic_v114.png", dpi=105)
print("wrote diagnostic_v114.png")
print(f"mean[{BURN}:]  u={uu:.4f}  deposit-HHI={dep_hhi:.3f} (even {even:.3f})  "
      f"peak intraday overdraft={peak_od:.2f} (IB latent)  failures={fails:.0f}  A5 drift={drift:.1e}")
