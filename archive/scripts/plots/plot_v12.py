...
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v12.py``."""
...
# Row 14 -- v12 SECURITIES (bonds sterilise reserves; the KEYSTONE: does the interbank market activate?)
line(ax[14, 0], ["bonds_outstanding", "gov_debt"], "Bonds outstanding & total government debt",
     ["bonds", "gov debt"], colors=["C0", "C1"])
line(ax[14, 1], ["bank_reserves_total"], "Σ bank reserves  (STERILISATION slows growth to only (1−f)× the deficit)",
     colors=["C3"])
line(ax[14, 2], ["peak_intraday_overdraft", "interbank_volume"],
     "Interbank KEYSTONE: peak overdraft & volume  (>0 = the latent market ACTIVATED)",
     ["peak overdraft", "IB volume"], zero=True, colors=["C3", "C1"])
line(ax[14, 3], ["hh_bond_wealth"], "Household bond wealth  (private NFA reallocated deposits→bonds)", colors=["C2"])
...
fig.suptitle(f"macro-simulator v12 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
...
fig.savefig("diagnostic_v12.png", dpi=105)
print("wrote diagnostic_v12.png")
