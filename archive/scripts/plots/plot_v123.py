...
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v123.py``."""
...
CFG = Config.v123(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)
rec = Economy(CFG).run()
t = [r["t"] for r in rec]
...
# Row 14 -- v12.3 SECURITIES: multi-period COUPON bonds, the THREE VALUES, the coupon bill, and no-refreeze
...
line(ax[14, 1], ["hh_bond_wealth", "bank_bond_face"],
     "Bond HOLDERS: households (θ portfolio) vs banks  (bank=0 default; live drain awaits v12.4 LoLR)",
     ["household", "bank"], colors=["C2", "C1"])
line(ax[14, 2], ["gov_interest_bill"], "Coupon bill paid to holders  (a fiscal expense → deficit)", colors=["C4"])
twin(ax[14, 3], "bonds_outstanding", "unemployment_rate",
     "Bond stock vs u  (NO re-freeze: bonds are a liquid coupon asset)", "bonds", "u")
...
fig.suptitle(f"macro-simulator v12.3 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(v12.3 SECURITIES: multi-period COUPON bonds + household θ-portfolio + duration/SVB; mean[{BURN}:] u={uu:.3f}, "
...
fig.savefig("diagnostic_v123.png", dpi=105)
print("wrote diagnostic_v123.png")
