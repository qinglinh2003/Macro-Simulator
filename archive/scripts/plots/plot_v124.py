"""v12 full-economy diagnostic: one long run, as many indicators as fit on a 15x4 grid.
Groups: ... / v11 BANKING SECTOR / v11.3 COMPETITION / v11.4 RESERVE TIER + INTERBANK + DEPOSIT COMPETITION.
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v124.py``."""
...
CFG = Config.v124(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)
rec = Economy(CFG).run()
t = [r["t"] for r in rec]
...
fig, ax = plt.subplots(15, 4, figsize=(22, 60), sharex=True)
...
# Row 14 -- v12.4 CB QUANTITY TOOLS: multi-period COUPON bonds, the THREE VALUES, the coupon bill, and no-refreeze
line(ax[14, 0], ["bonds_outstanding", "bond_market_total"],
     "Bonds: FACE (=outstanding) vs MARKET  (gap = duration MTM; coupon 1% vs policy rate)",
     ["face", "market"], colors=["C0", "C3"])
line(ax[14, 1], ["reserve_M", "cb_absorbed"],
     "Base money (VARIABLE now) & CB-absorbed reserves  (OMO drains ⇒ reserve_M falls)", ["reserve M", "absorbed"],
     colors=["C0", "C3"])
line(ax[14, 2], ["peak_intraday_overdraft", "interbank_volume"],
     "Interbank KEYSTONE: peak overdraft & volume  (>0 = the §37 latent market finally BINDS via OMO)",
     ["peak overdraft", "IB volume"], zero=True, colors=["C3", "C1"])
twin(ax[14, 3], "lolr_advances", "banks_alive",
     "LoLR advances vs banks alive  (CB liquidity backstop)", "LoLR", "alive")
...
fig.suptitle(f"macro-simulator v12.4 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(v12.4 CB QUANTITY TOOLS: multi-period COUPON bonds + household θ-portfolio + duration/SVB; mean[{BURN}:] u={uu:.3f}, "
...
fig.savefig("diagnostic_v124.png", dpi=105)
print("wrote diagnostic_v124.png")
