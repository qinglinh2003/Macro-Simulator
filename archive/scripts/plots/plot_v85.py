...
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v85.py``."""
...
rec = Economy(Config.v85(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)).run()
t = [r["t"] for r in rec]
...
fig.suptitle(f"macro-simulator v8.5 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(founder-owned genesis; mean u[1000:]={uu:.2f}, A5 drift {drift:.1e})", fontsize=13, y=0.999)
fig.tight_layout(rect=(0, 0, 1, 0.996))
fig.savefig("diagnostic_v85.png", dpi=110)
print("wrote diagnostic_v85.png")
