...
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v9.1.py``."""
...
rec = Economy(Config.v91(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)).run()
t = [r["t"] for r in rec]
...
fig, ax = plt.subplots(8, 4, figsize=(22, 32), sharex=True)
...
# Row 6 -- v9 GOVERNMENT (the new fiscal indicators, normalised)
...
twin(ax[6, 3], "gov_consumption", "unemployment_rate", "Gov spending vs unemployment", "gov C + benefit", "u")
ax[6, 3].plot(t, smooth(col("benefit_paid")), lw=1.3, color="C1", ls="--", label="benefit")
ax[6, 3].legend(fontsize=6, loc="upper left", framealpha=0.6)

# Row 7 -- v9.1 SUPPLY SIDE (the public-capital / productivity channel + the welfare payoff)
line(ax[7, 0], ["public_capital"], "Public capital K_pub  (converges = level boost, not runaway)")
...
fig.suptitle(f"macro-simulator v9.1 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(supply side: gov investment -> public capital; mean u[1000:]={uu:.2f}, A5 drift {drift:.1e})", fontsize=13, y=0.999)
fig.tight_layout(rect=(0, 0, 1, 0.996))
fig.savefig("diagnostic_v91.png", dpi=110)
print("wrote diagnostic_v91.png")
