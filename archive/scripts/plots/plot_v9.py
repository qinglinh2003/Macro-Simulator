...
Pure observation -- never touches the model. Run: ``PYTHONPATH=.:scripts uv run python scripts/plots/plot_v9.py``."""
...
rec = Economy(Config.v9(n_firms_c=NC, n_firms_k=NK, n_households=NH, n_ticks=TICKS, seed=SEED)).run()
t = [r["t"] for r in rec]
...
fig, ax = plt.subplots(7, 4, figsize=(22, 28), sharex=True)
...
ax[6, 3].plot(t, col("benefit_paid"), lw=1.0, color="C1", ls="--", label="benefit")
...
fig.suptitle(f"macro-simulator v9 diagnostic  —  {NC}C/{NK}K/{NH}H, {TICKS} ticks, seed {SEED}  "
             f"(government (3% deficit target); mean u[1000:]={uu:.2f}, A5 drift {drift:.1e})", fontsize=13, y=0.999)
fig.tight_layout(rect=(0, 0, 1, 0.996))
fig.savefig("diagnostic_v9.png", dpi=110)
print("wrote diagnostic_v9.png")
