...
Run: ``PYTHONPATH=.:scripts uv run python scripts/validation/validate_v83.py``.
"""
import numpy as np
...
def run(seed):
    econ = Economy(Config.v83(n_firms_c=NC, n_firms_k=NK, n_households=NH,
                             n_ticks=TICKS, seed=seed))
    recs = econ.run()
    ser = {k: np.array([r[k] for r in recs], float) for k in recs[0] if isinstance(recs[0][k], (int, float))}
    # T8 wealth = FULL net worth (cash + equity − all debt): the v8 balance-sheet measure.
...
print(f"§4 VALIDATION -- Config.v83 (v8.1 + weakened q-investment), {NC}C/{NK}K/{NH}H, "
      f"{TICKS} ticks, seeds {SEEDS}, burn-in {BURN}\n")
...
report("T6 Zipf/Pareto firm size (power-law SHAPE, slope ~ -1)",
       ...
       f"-> {'clean power law (Gibrat growth, §23)' if clean_zipf else 'concentrated but not power-law'}")
...
# T8 wealth distribution: right-skewed, heavy-tailed. FULL net worth has negatives (leveraged
# debtors), so a plain Gini is invalid -> judge on SKEW + heavy tail (excess kurtosis), and check
# whether the RICH tail is a clean power law or a few leverage outliers (upper-tail rank-size slope).
...
tail = pos[:max(8, len(pos) // 10)]                    # top decile of positive holders
pslope = (np.polyfit(np.log(np.arange(1, len(tail) + 1)), np.log(tail), 1)[0]
          if len(tail) >= 8 else float("nan"))
...
print("T8 is now measured on FULL net worth (cash + equity − debt) under v8 margin credit: leveraged\n"
      "capital gains concentrate wealth into a right-skewed heavy tail (the v4 baseline was left-skewed\n"
      "and thin). T6 (firm-size Zipf) is unchanged -- still needs Gibrat multiplicative firm growth.\n"
      "Watch whether margin's added volatility disturbed the cycle regularities (T1/T3/T7).")
