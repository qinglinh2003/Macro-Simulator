...
- 中央六 tab:宏观焦点(公报诚实信道)/ 家庭(家庭—成员主从浏览)/
  ...
  行情和企业详情深链)/ 指标全景(14 个领域页签，每页 6 项核心指标及多种结构图表)/
  世界视图
  (三国卡 + 可切指标排名 + 6 指标三色对比 + 枢纽辐射关系图 + e/NFA/CA/移民/汇款条图)。
- 指标全景覆盖实体经济、国民账户、劳动力、价格货币、财政、银行信贷、债务风险、
  资本市场、住房、能源、外部、分配福利、人口社会与企业生态。住房页直接消费当前
  可玩配置已启用的存量、交易、按揭、租赁与建设记录；人口事件使用
  `births_tick/deaths_tick`，企业进入退出使用 `births/deaths`，不再混淆两套口径。
- 未启用某项结构能力的经济体显示明确的不可用说明，不用零值伪装成已统计结果。
