...
**状态**：设计基线已冻结；`NewGameSpec v1`、后端生产配置和 Godot
...
- 桌面 worker 的 `new_game` 现接收严格、版本化的 `NewGameSpec v1`。Godot
  提交国家、Profile、最终代理规模、结构覆盖、World 参数、场景、时长、玩家国、
  五个席位和初始 Policy；Python 校验成功后才原子替换当前运行。
- 新游戏没有 model 选择器，也不由 Godot 提交 model ID。每次开局由 Python
  后端自动绑定当前生产配置；内部 ID 只留作响应复现元数据。后端更新生产
  配置后，前端无需同步增加或切换模型选项。
...
游戏内顶栏固定提供“主菜单”按钮。点击后立即暂停自动播放并要求确认；确认返回不会重置
当前世界，首页会出现“继续模拟”。取消确认则恢复点击前的播放状态。只有玩家启动另一个
新模拟时，当前世界才会被后端的新世界替换。

### 4.2 新建模拟：六步向导

建议固定左侧步骤导航，中间为当前编辑区，右侧为始终可见的“本局摘要”。底部固定 `返回`、`下一步`；最后一步变为 `启动模拟`。步骤可回退，未完成校验的步骤显示红点或警告数。

1. **场景**：沙盒 / 历史冲击模板 / 自定义场景及说明；
2. **世界设置**：国家数量、跨境机制、随机种子与时间范围；
...
本页没有基础模型、历史版本或校准预设选择器。Godot 不提交这些字段；Python
后端始终注入当前生产配置。历史研究工厂和 YAML 校准不属于游戏开局。
...
| 模拟开始日期 | Gregorian 日期选择器，默认 2000-01-01 | 这是经济体创世日期，不是装饰性显示；生日、婚姻、劳动和人口事件从该日开始推进 |
| 运行时长 | 1 年 / 5 年 / 10 年 / 自定义 / 无限 | `1 tick = 1 自然日`；有限时长最终写为正整数天数。自定义可填写结束年月日，也可直接填写 tick，两者按真实 Gregorian 日历互算（含闰年）；无限写为 `null` |
...
创世匹配的实际结果决定。引擎原始 Profile 的规模倍率不会放大企业或银行数量；客户端
可以通过下述“企业随人口缩放”编辑辅助生成成比例的企业数量。

国家配置页提供两个默认开启、彼此独立的映射开关：

- **Profile 按基线映射**：每个国家分别保存自己的“对称基线”。玩家在对称基线下修改
  数值、能力开关或枚举后，再选择其他 Profile，客户端以该国的新基线为起点应用 Profile
  相对倍率。人口使用 `scale` 比例，生产率使用 `prod` 比例，必需品占比使用 `nec`
  比例；其他字段继承基线。关闭后，切换 Profile 使用引擎原始 Profile 默认值。
- **企业随人口缩放**：保持消费品、资本品、能源和建造企业相对于人口的密度。直接修改
  人口或切换到不同人口规模的 Profile 时，四类企业数量按比例取整；手动修改某类企业
  数量会把当前比例记为该类企业的新密度。银行数量不参与联动。关闭后，人口与企业数
  独立编辑。

两个映射都只负责在编辑阶段生成最终 `CountrySpec.overrides`，不会作为新的模拟参数写入
存档。`应用到所有国家` 会让每个国家用自己的对称基线重新生成选中的 Profile，而不是
复制某一国家的绝对数值。
...
```text
1 tick = 1 Gregorian 自然日
有限时长 = end_date - start_date（半开区间 [start_date, end_date)）
人口、生日、婚姻和劳动事件使用真实日期（包括 2 月 29 日）
Controller 会议间隔和场景 duration 仍以自然日 tick 表达
```

具体实现是
`Config.v13(..., simulation_start_date=start_date, n_ticks=duration_ticks)` 加
`World(periods_per_year=365.0)`。利率和宏观流量的年化基准仍固定为 365
日；闰日是一个正常的额外模拟日，不会被吞掉或压缩。旧请求缺少
`start_date` 时按 `2000-01-01` 读取；旧的 `1y/5y/10y/inf` duration
令牌仍可读取，但规范化响应统一返回整数天数或 `null`。
历史 Config 工厂仅用于研究复现，不进入新游戏请求或界面。
