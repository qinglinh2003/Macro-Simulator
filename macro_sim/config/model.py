...
class Config:
    ...
    # v24 A2-root (zombie AFC death-spiral pricing): floor the output base over which the
    # capital SERVICE cost is allocated into the posted price, as a share of the output the
    # firm's capital was sized for (y_ref = K/v). A shrinking firm otherwise spreads a fixed
    # stock-scaled cost over a vanishing flow: price -> ~270x median, captive deprivation
    # demand still buys, and the chained fixed-basket CPI explodes (Germany x3041 in the 30y
    # portrait). 0.0 = off = bit-identical; FULL_FRONTIER_FLAGS opts into 0.1.
    capital_service_min_utilization: float = 0.0
    # v24 A1 (rent ratchet -> housing collapse): frictional-vacancy deadband (share of the
    # rentable stock whose vacancy does NOT cut the rent) + an absolute per-tick rent floor
    # as a share of the genesis wage. 0.0/0.0 = legacy one-way ratchet, bit-identical;
