...
def _firm_contracts() -> Mapping[str, Mapping[str, Any]]:
    ...
    return {
        ...
        "config.entry_beta": {
            ...
            "activation": "consumption_entry_pressure",
            "rationale": "Entry sensitivity scales the number of consumption-firm entrants generated by a positive median excess return. Zero removes the response and the high identification arm makes it large enough to cross the integer entrant boundary; a shared zero hurdle and wider entry cap expose the coefficient without overwriting the audited treatment.",
        },
        "config.entry_hurdle": {
            "status": "screening_ready",
            "values": (0.0, 0.000268),
        ...
        "config.firm_full_pnl": {
            ...
            "activation": "firm_debt_service_pressure",
        ...
        "config.k_entry_demand": {
            ...
            "activation": "capital_firm_entry_pressure",
            "rationale": "A higher sector-wide labor-demand threshold should make capital-firm spin-offs less frequent. A shared replacement-capital shortage and high spin-off hazard make the eligibility boundary observable.",
        },
        ...
        "config.subscale_viability_workers": {
            ...
            "directions": {exits: "increase"},
            "statistics": {exits: "cumulative"},
            "activation": "subscale_exit_pressure",
            "rationale": "A higher minimum viable labor demand classifies more firms as subscale and should increase exits. A shared short grace period and certain exit hazard isolate the threshold from rare-event noise.",
        },
...
def _demography_contracts() -> Mapping[str, Mapping[str, Any]]:
    ...
    return {
        ...
        "config.social.guardian_search_adult_siblings": {
            "status": "activation_scenario_required",
            "values": (False,),
            "activation": "guardian_sibling_stress",
            "horizon_days": 365,
        ...
        "config.social.guardian_search_same_household_adults": {
            "status": "activation_scenario_required",
            "values": (False,),
            "activation": "guardian_household_stress",
            "horizon_days": 365,
        ...
        "config.demo_signal_halflife_years": {
            "status": "activation_scenario_required",
            "values": (1.0, 10.0),
            "activation": "demographic_signal_transition",
            "horizon_days": 1095,
            "directions": {income_signal: "nonzero"},
            "statistics": {income_signal: "post_burnin_volatility"},
        ...
        "config.demographic_marriage_market_interval_days": {
            ...
            "values": (14, 90),
            "directions": {marriages: "nonzero"},
            "statistics": {marriages: "post_burnin_volatility"},
            "rationale": "This is a numerical market-clearing cadence rather than an economic intensity: the annual acceptance hazard is interval-adjusted, so cumulative marriage incidence should remain approximately stable while a longer interval produces visibly lumpier event timing. It is not a gameplay lever.",
        },
...
def _banking_contracts() -> Mapping[str, Mapping[str, Any]]:
    ...
    return {
        ...
        "config.bank_relationship_lock_in": {
            ...
            "activation": "relationship_refinancing_pressure",
            "rationale": "Keeping an active loan with its creditor prevents costless relationship reassignment and should alter refinancing, origination, and interest flows. A shared capital shortage, rate dispersion, and broad lender search create repeated borrowing decisions where that restriction binds.",
        },
        "config.bank_runs": {
            "status": "activation_scenario_required",
            "values": (False,),
        ...
        "config.interest_by_deposits": {
            ...
            "activation": "bank_payout_distribution_pressure",
            "rationale": "Allocating bank payout equally rather than in proportion to deposits changes who receives financial income. A shared profitable-loan episode and full payout ratio amplify the distributional channel without changing the allocation rule under treatment.",
        },
