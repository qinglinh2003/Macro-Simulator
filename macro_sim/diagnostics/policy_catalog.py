...
SCENARIOS: Mapping[str, ScenarioSpec] = MappingProxyType({
    ...
    for item in (
        ...
        _scenario(
            ...
            ("metric.source.m5.firm_investment_target", "metric.economy.real_output", "metric.economy.unemployment_rate"),
        ),
        _scenario(
            "CR_PANDEMIC",
            "Temporary labor availability, household demand, and cross-border capacity losses create a broad but recoverable downturn.",
            "ready_to_calibrate",
            ("stable burn-in", "labor_availability shock", "household_demand shock", "import_capacity shock", "export_capacity shock"),
            ("metric.economy.employment", "metric.economy.real_output"),
            ("metric.economy.real_output", "metric.economy.unemployment_rate", "metric.source.m9.country.imports_volume"),
        ),
        _scenario(
            "CR_NATURAL_DISASTER",
        ...
        _scenario(
            ...
            ("metric.source.m9.country.exports_volume",),
            ("metric.economy.real_output", "metric.world.current_account", "metric.source.m9.country.exports_volume"),
        ),
        _scenario(
            "CR_PEG_PRESSURE",
        ...
        _scenario(
            ...
            ("metric.source.m5.lolr_advances", "metric.source.m5.new_credit", "metric.economy.real_output"),
            "Uses an endogenous market-loss withdrawal trigger rather than an outcome override.",
        ),
        _scenario(
            "CR_HOUSING_BUST",
            "A housing-specific mortgage freeze stalls transactions and transmits into aggregate demand.",
            "conditional",
            ("active mortgage market", "housing-sector credit_supply shock", "common demand trigger"),
            ("metric.source.m8.housing.mortgage_principal_originated", "metric.source.m8.housing.session_sales"),
            ("metric.source.m8.housing.mortgage_principal_originated", "metric.economy.unemployment_rate", "metric.economy.real_output"),
            "Uses sector-specific mortgage availability and endogenous market clearing; there is no house-price or foreclosure outcome override.",
        ),
        _scenario(
            "CR_SOVEREIGN_STRESS",
            "A sovereign required-return shock reprices government debt and feeds back through bond holders and the real economy.",
            "ready_to_calibrate",
            ("outstanding tradable debt", "sovereign_risk_premium shock"),
            ("metric.shock.severity.sovereign_risk_premium", "metric.source.m6.bond_market_value"),
            ("metric.source.m6.household_bond_market_value", "metric.economy.real_output"),
        ),
    )
})
