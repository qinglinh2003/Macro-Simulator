...
struct M11ControlledState final {
    ...
    M11ControlledState(M11DecisionScheduler scheduler_value,
                       ...
                       M11EventStream events_value, M11ReleaseStream releases_value)
        : scheduler(std::move(scheduler_value)),
          coordinator(std::move(coordinator_value)), events(std::move(events_value)),
...
class M11ControlledSession final {
  public:
    ...
    M11ControlledSession &operator=(const M11ControlledSession &) = delete;
    M11ControlledSession(M11ControlledSession &&) noexcept = default;
    M11ControlledSession &operator=(M11ControlledSession &&) noexcept = default;
    ...
    [[nodiscard]] Tick tick() const noexcept { return bridge_.engine().tick(); }
    [[nodiscard]] M11BoundaryPhase phase() const noexcept { return state_.phase; }
    [[nodiscard]] const EngineSession &engine() const noexcept {
        return bridge_.engine();
    }
    [[nodiscard]] const M11ControllerRunSpec &run_spec() const noexcept {
        return run_spec_;
    }
    [[nodiscard]] const M11ControlledState &state() const noexcept { return state_; }
    [[nodiscard]] const M11PolicyCoordinator &coordinator() const noexcept {
    ...
    [[nodiscard]] const M11SeatRuntime *seat(EconomyId economy,
                                             std::string_view seat) const noexcept;
    ...
    [[nodiscard]] Result<M11PolicyDecision>
    submit_human_proposal(M11PolicyProposal proposal, std::string_view actor);
    [[nodiscard]] Result<M11PolicyDecision>
    timeout_context(std::string_view context_id, std::string_view operation_id,
                    std::string_view actor);
    [[nodiscard]] Result<M11PolicyDecision>
    cancel_pending(std::string_view decision_id, std::string_view operation_id,
                   std::string_view actor);
    ...
    schedule_shock(const M11ControlledShockScheduleRequest &request);
  ...
  private:
    ...
    M11ControlledSession(HybridControlledBridge bridge, M11ControllerRunSpec run_spec,
...
[[nodiscard]] std::string_view m11_boundary_phase_name(M11BoundaryPhase phase) noexcept;
[[nodiscard]] std::string_view m11_occupant_kind_name(M11OccupantKind kind) noexcept;
