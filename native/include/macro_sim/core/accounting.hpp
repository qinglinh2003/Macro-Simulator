...
class PostingBook final {
  public:
    [[nodiscard]] Result<AccountId>
    create_account(AccountKey key, Money opening_balance, bool allow_negative = false);
    [[nodiscard]] Status close_account(AccountId id);
    [[nodiscard]] Result<AccountId> find(AccountKey key) const noexcept;
    [[nodiscard]] Result<SettlementNodeId> settlement_node(AccountId id) const noexcept;
...
class ReserveBook final {
  public:
    [[nodiscard]] Result<SettlementNodeId> create_position(BankId bank,
  ...
  private:
    ...
    [[nodiscard]] MutationResult apply_delta_unchecked(SettlementNodeId node,
                                                       double delta) noexcept;
    [[nodiscard]] MutationResult apply_stock_delta_unchecked(double delta) noexcept;
    void restore_stock_unchecked(const MutationResult &previous) noexcept;
    void restore_unchecked(SettlementNodeId node,
...
class LoanBook final {
  ...
  private:
    ...
    [[nodiscard]] CreateResult create_unchecked(BankId lender, OwnerId borrower,
                                                AccountId borrower_account,
                                                Money principal, LoanTerms terms);
    void rollback_create_unchecked(LoanId id) noexcept;
    [[nodiscard]] MutationResult apply_principal_delta_unchecked(LoanId id,
                                                                 double delta) noexcept;
...
class OwnershipBook final {
  public:
    [[nodiscard]] Result<OwnershipLotId> create_lot(AssetKey asset, OwnerId owner,
                                                    double share);
  ...
  private:
    ...
    [[nodiscard]] MutationResult mutate_unchecked(OwnershipLotId id, OwnerId owner,
                                                  double share) noexcept;
    void restore_unchecked(OwnershipLotId id, const MutationResult &previous) noexcept;
