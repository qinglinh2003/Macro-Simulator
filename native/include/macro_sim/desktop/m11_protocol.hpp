...
inline constexpr std::size_t kM11MaximumProtocolFrameBytes =
    1024U * 1024U;
...
class M11ProtocolWorker final {
  public:
    ...
    M11ProtocolWorker &
    operator=(const M11ProtocolWorker &) = delete;
    M11ProtocolWorker(M11ProtocolWorker &&) noexcept;
    M11ProtocolWorker &
    operator=(M11ProtocolWorker &&) noexcept;
    ~M11ProtocolWorker();

    [[nodiscard]] static Result<M11ProtocolWorker>
    create(M11ProtocolOptions options);

    [[nodiscard]] std::string
    handle_frame(std::string_view frame);
...
[[nodiscard]] bool
valid_m11_capability_token(std::string_view token) noexcept;
