...
struct NativePolicyArtifactBuilder final {
    [[nodiscard]] static NativePolicyArtifact build(
        ...
        NativePolicyArtifact::Activation activation,
        double input_clip, bool has_input_clip) {
...
constexpr std::size_t kMaximumTotalParameterBytes =
    1024U * 1024U * 1024U;
...
inflate_raw(std::span<const std::uint8_t> compressed,
    ...
    if (result != Z_STREAM_END ||
        final_size != static_cast<uLong>(output_size)) {
        throw std::runtime_error("deflate member length differs");
    }
    return output;
}

[[nodiscard]] std::vector<ZipMember>
read_zip(std::span<const std::uint8_t> bytes, std::size_t maximum_total) {
    if (bytes.size() < 22U || bytes.size() > maximum_total) {
        throw std::runtime_error("ZIP container size is invalid");
    }
    const auto search_floor =
        bytes.size() > 65'557U ? bytes.size() - 65'557U : 0U;
    ...
    for (std::size_t index = 0; index < member_count; ++index) {
        ...
        const auto record_size = checked_add(
            46U, checked_add(name_size,
                             checked_add(extra_size, member_comment_size)));
        ...
        total_uncompressed =
            checked_add(total_uncompressed, uncompressed_size);
...
[[nodiscard]] std::vector<std::size_t> json_shape(const Json &value) {
    ...
    for (const auto &dimension : value) {
        ...
        if (checked == 0U ||
            checked > std::numeric_limits<std::size_t>::max()) {
...
[[nodiscard]] NumericArray read_npy(
    ...
    const std::string header(
        ...
        header_length);
    std::smatch match;
    const std::regex descr_pattern(
        R"(['"]descr['"]\s*:\s*['"]([^'"]+)['"])");
    const std::regex order_pattern(
        R"(['"]fortran_order['"]\s*:\s*(True|False))");
    const std::regex shape_pattern(
        R"(['"]shape['"]\s*:\s*\(([^\)]*)\))");
    ...
    for (auto iterator = std::sregex_iterator(
             shape_text.begin(), shape_text.end(), dimension_pattern);
    ...
    for (std::size_t index = 0; index < element_count; ++index) {
        ...
        if (float32) {
            float raw = 0.0F;
            std::memcpy(&raw, bytes.data() + data_offset +
                                  index * sizeof(float),
                        sizeof(float));
            value = static_cast<double>(raw);
        } else {
            std::memcpy(&value, bytes.data() + data_offset +
                                    index * sizeof(double),
                        sizeof(double));
        }
...
member(const std::vector<ZipMember> &members, std::string_view name) {
    const auto found = std::find_if(
        members.begin(), members.end(), [name](const ZipMember &item) {
...
parse_artifact(std::span<const std::uint8_t> artifact) {
    try {
        ...
        const std::set<std::string> outer_names{
            outer[0].name, outer[1].name,
        };
        if (outer_names != std::set<std::string>{
        ...
        if (manifest.dump() != manifest_text ||
            !exact_keys(
                ...
                {"action_codec", "action_contract_hash", "artifact_format",
                 "artifact_schema_version", "context_codec",
                 "context_contract_hash", "metadata", "model",
                 "model_contract_hash", "weights"}) ||
        ...
        if (!action.is_object() || !dimensions.is_array() ||
            dimensions.empty()) {
        ...
        if (!exact_keys(
            ...
            model.at("context_contract_hash") !=
                manifest.at("context_contract_hash") ||
            model.at("action_contract_hash") !=
                manifest.at("action_contract_hash") ||
        ...
        const auto layer_count =
            model.at("layer_count").get<std::size_t>();
        ...
        for (const auto &[name, spec] : parameter_specs.items()) {
            if (!exact_keys(spec, {"dtype", "shape"}) ||
                spec.at("dtype") != dtype) {
            ...
            const auto bytes = checked_product(
                shape, float32 ? sizeof(float) : sizeof(double));
            total_parameter_bytes =
                checked_add(total_parameter_bytes, bytes);
        ...
        if (!expected.contains("input_mean") ||
            !expected.contains("input_scale")) {
            throw std::runtime_error("artifact input normalization is absent");
        }
        for (std::size_t index = 0; index < layer_count; ++index) {
            if (!expected.contains(
                    "layer_" + std::to_string(index) + "_weight") ||
                !expected.contains(
                    "layer_" + std::to_string(index) + "_bias")) {
        ...
        for (const auto &item : npz) {
            ...
            if (specification == expected.end() ||
                arrays.contains(name)) {
                throw std::runtime_error("NPZ parameter differs");
            }
            arrays.emplace(
                name,
                read_npy(item.bytes, specification->second, float32));
        }

        const auto deterministic =
            model.at("deterministic").get<bool>();
        if (!deterministic) {
            throw UnsupportedArtifact(
                "stochastic msrl v1 inference is not portable");
        }
        const auto temperature =
            model.at("temperature").get<double>();
        ...
        if (input_mean.size() != info.observation_dimension ||
            ...
            std::any_of(
                input_scale.begin(), input_scale.end(),
        ...
        for (std::size_t index = 0; index < layer_count; ++index) {
            ...
            if (weight.shape.size() != 2U || bias.shape.size() != 1U ||
                weight.shape[1U] != previous ||
                weight.shape[0U] != bias.shape[0U]) {
        ...
        return NativePolicyArtifactBuilder::build(
            std::move(info), std::move(feature_names),
            std::move(input_mean), std::move(input_scale),
            std::move(layers), native_activation, input_clip,
            has_input_clip);
        ...
        return Status(ErrorCode::allocation_failure,
                      "M11 artifact allocation failed");
    } catch (const std::exception &) {
        return Status(ErrorCode::corrupt_input,
                      "M11 artifact validation failed");
    }
}

template <typename Scalar>
[[nodiscard]] Result<std::vector<Scalar>>
normalize(std::span<const double> observation,
    ...
    if (observation.size() != expected_dimension) {
        return Status(ErrorCode::invalid_argument,
                      "M11 observation dimension differs");
    }
    ...
    for (std::size_t index = 0; index < observation.size(); ++index) {
        ...
        auto value = static_cast<Scalar>(
            (static_cast<Scalar>(observation[index]) -
...
forward(const NativePolicyArtifact &artifact,
        ...
        NativePolicyArtifact::Activation activation,
        bool has_clip, double input_clip) {
    auto normalized = normalize<Scalar>(
        observation, artifact.info().observation_dimension, mean, scale,
        has_clip, input_clip);
    if (!normalized.ok()) {
    ...
    for (std::size_t layer_index = 0; layer_index < layers.size();
        ...
        for (std::size_t row = 0; row < layer.outputs; ++row) {
            ...
            for (std::size_t column = 0; column < layer.inputs; ++column) {
                accumulator += static_cast<Scalar>(
                                   layer.weights[offset + column]) *
                               values[column];
            }
    ...
    std::transform(
        values.begin(), values.end(), output.begin(),
    ...
    try {
        ...
        if (!stream) {
            return Status(ErrorCode::not_found,
                          "M11 artifact file was not found");
        }
        ...
        if (ending < 0 ||
            ...
            return Status(ErrorCode::out_of_range,
                          "M11 artifact file is too large");
        }
        std::vector<std::uint8_t> bytes(
            static_cast<std::size_t>(ending));
...
NativePolicyArtifact::forward_float32(
    std::span<const double> observation) const {
    return forward<float>(
        *this, observation, input_mean_, input_scale_, layers_, activation_,
        has_input_clip_, input_clip_);
}

Result<std::vector<double>>
NativePolicyArtifact::forward_float64(
    std::span<const double> observation) const {
    return forward<double>(
        *this, observation, input_mean_, input_scale_, layers_, activation_,
        has_input_clip_, input_clip_);
}
...
NativePolicyArtifact::logits(std::span<const double> observation) const {
    return info_.float32 ? forward_float32(observation)
                         : forward_float64(observation);
}

Result<std::vector<double>>
NativePolicyArtifact::normalized_input(
    std::span<const double> observation) const {
    if (info_.float32) {
        auto calculated = normalize<float>(
            observation, info_.observation_dimension, input_mean_,
        ...
        std::transform(
            calculated.get_if()->begin(), calculated.get_if()->end(),
            result.begin(),
...
NativePolicyArtifact::probabilities(
    ...
    for (std::size_t dimension = 0; dimension < info_.action_dimension;
         ++dimension) {
        ...
        for (std::uint32_t code = 0U; code < 3U; ++code) {
            ...
            permitted = true;
            maximum = std::max(
                maximum, values[index] / info_.temperature);
        }
        if (!permitted) {
        ...
        for (std::uint32_t code = 0U; code < 3U; ++code) {
            ...
            result[index] = std::exp(
                values[index] / info_.temperature - maximum);
            total += result[index];
        }
...
NativePolicyArtifact::predict_codes(
    std::span<const double> observation,
    std::span<const std::uint8_t> action_mask) const {
