...
[[nodiscard]] Status corrupt_checkpoint() noexcept {
    return Status(ErrorCode::corrupt_input, "M11 controlled checkpoint is invalid");
}

void require_object(const Json &value, std::initializer_list<std::string_view> names) {
...
[[nodiscard]] Enum checked_enum(const Json &value, std::uint8_t maximum) {
...
[[nodiscard]] std::optional<Tick> tick_from_json(const Json &value) {
    return value.is_null() ? std::optional<Tick>{}
                           : std::optional<Tick>(Tick(value.get<std::uint64_t>()));
}

[[nodiscard]] Json digest_json(const core::StateDigest &digest) { return digest.hex(); }
...
[[nodiscard]] core::StateDigest digest_from_json(const Json &value) {
    ...
    for (std::size_t index = 0U; index < digest.bytes.size(); ++index) {
        digest.bytes[index] = static_cast<std::uint8_t>(
            (hex_digit(text[index * 2U]) << 4U) | hex_digit(text[index * 2U + 1U]));
    }
    return digest;
}
...
[[nodiscard]] std::vector<std::uint8_t> bytes_from_hex(const Json &value) {
    const auto text = value.get<std::string>();
    if (text.size() % 2U != 0U || text.size() / 2U > kM11MaximumArtifactBytes) {
        throw std::runtime_error("byte archive length differs");
    }
    std::vector<std::uint8_t> result(text.size() / 2U);
    for (std::size_t index = 0U; index < result.size(); ++index) {
        result[index] = static_cast<std::uint8_t>((hex_digit(text[index * 2U]) << 4U) |
...
[[nodiscard]] PolicyValue policy_value_from_json(const Json &value) {
    ...
    if (kind == "economy_set") {
        ...
        for (const auto raw : value.at("value").get<std::vector<std::uint64_t>>()) {
...
[[nodiscard]] NativePolicyAction action_from_json(const Json &value) {
...
[[nodiscard]] Json policy_version_json(const M11PolicyVersion &value) {
    return Json{
        {"economy", value.economy.value()},
...
[[nodiscard]] M11PolicyVersion policy_version_from_json(const Json &value) {
    require_object(value, {"economy", "last_effective", "lever", "version"});
    return {
        EconomyId(value.at("economy").get<std::uint64_t>()),
        value.at("lever").get<std::string>(),
...
[[nodiscard]] Json observation_json(const M11ReleasedObservation &value) {
    return Json{
        {"observed_at", value.observed_at.value()},
        {"released_at", value.released_at.value()},
        {"revision", value.revision},
        {"series_id", value.series_id},
        {"value", value.value.has_value() ? Json(*value.value) : Json(nullptr)},
    };
}

[[nodiscard]] M11ReleasedObservation observation_from_json(const Json &value) {
    require_object(value,
                   {"observed_at", "released_at", "revision", "series_id", "value"});
...
[[nodiscard]] Json permitted_action_json(const M11PermittedAction &value) {
    return Json{
...
[[nodiscard]] M11PermittedAction permitted_action_from_json(const Json &value) {
    require_object(value, {"allowed", "current_value", "earliest_effective", "lever",
...
[[nodiscard]] Json context_json(const M11DecisionContext &value) {
    ...
    return Json{
        ...
        {"answered_by_proposal", value.answered_by_proposal.has_value()
...
[[nodiscard]] M11DecisionContext context_from_json(const Json &value) {
    require_object(value,
                   {"administrative_capacity", "administrative_remaining",
                    "administrative_reserved", "administrative_window",
                    "answered_by_proposal", "boundary", "context_id", "decision_group",
                    "decision_window_id", "economy", "elapsed_ticks", "emergency",
                    "emergency_trigger", "expires_at", "observation",
                    "permitted_actions", "policy_versions", "seat"});
    M11DecisionContext result;
    result.context_id = value.at("context_id").get<std::string>();
    result.decision_window_id = value.at("decision_window_id").get<std::string>();
    result.economy = EconomyId(value.at("economy").get<std::uint64_t>());
    result.seat = value.at("seat").get<std::string>();
    result.decision_group = value.at("decision_group").get<std::string>();
    result.boundary = Tick(value.at("boundary").get<std::uint64_t>());
    result.expires_at = Tick(value.at("expires_at").get<std::uint64_t>());
    result.administrative_window =
        Tick(value.at("administrative_window").get<std::uint64_t>());
    ...
    for (const auto &entry : value.at("permitted_actions")) {
        result.permitted_actions.push_back(permitted_action_from_json(entry));
    }
    for (const auto &entry : value.at("policy_versions")) {
        result.policy_versions.push_back(policy_version_from_json(entry));
    }
    result.administrative_remaining =
        value.at("administrative_remaining").get<double>();
    result.administrative_reserved = value.at("administrative_reserved").get<double>();
    result.administrative_capacity = value.at("administrative_capacity").get<double>();
    result.emergency = value.at("emergency").get<bool>();
    result.emergency_trigger = value.at("emergency_trigger").get<std::string>();
    result.elapsed_ticks = value.at("elapsed_ticks").get<std::uint64_t>();
...
[[nodiscard]] Json proposal_json(const M11PolicyProposal &value) {
    ...
    return Json{
        ...
        {"reason", value.reason},
        {"supersedes_proposal_id", value.supersedes_proposal_id.has_value()
                                       ? Json(*value.supersedes_proposal_id)
                                       : Json(nullptr)},
    };
}

[[nodiscard]] M11PolicyProposal proposal_from_json(const Json &value) {
    require_object(value, {"actions", "based_on_policy_versions", "context_id",
                           "idempotency_key", "proposal_id", "reason",
                           "supersedes_proposal_id"});
    M11PolicyProposal result;
    result.proposal_id = value.at("proposal_id").get<std::string>();
    ...
    for (const auto &entry : value.at("based_on_policy_versions")) {
...
[[nodiscard]] Json decision_json(const M11PolicyDecision &value) {
    return Json{
        ...
        {"proposal_id", value.proposal_id},
        {"reason_code", value.reason_code},
        {"reserved_administrative_cost", value.reserved_administrative_cost},
        {"status", static_cast<std::uint8_t>(value.status)},
    };
}

[[nodiscard]] M11PolicyDecision decision_from_json(const Json &value) {
    require_object(value, {"accepted_at", "accepted_sequence", "adjustment_cost",
    ...
    return {
        ...
        checked_enum<M11DecisionStatus>(
            value.at("status"),
            static_cast<std::uint8_t>(M11DecisionStatus::failed_at_execution)),
        value.at("reason_code").get<std::string>(),
...
[[nodiscard]] M11CalendarSpec calendar_from_json(const Json &value) {
    require_object(value, {"administrative_capacity", "decision_group", "offset_ticks",
...
[[nodiscard]] Json trigger_json(const M11TriggerSpec &value) {
    return Json{
        ...
        {"minimum_persistence_ticks", value.minimum_persistence_ticks},
        {"series_id", value.series_id},
        {"trigger_id", value.trigger_id},
    };
}

[[nodiscard]] M11TriggerSpec trigger_from_json(const Json &value) {
    require_object(value,
                   {"authorized_seats", "context_expiry_ticks", "cooldown_ticks",
                    "decision_group", "direction", "enter_threshold", "exit_threshold",
    ...
    return {
        ...
        checked_enum<M11TriggerDirection>(
            value.at("direction"),
            static_cast<std::uint8_t>(M11TriggerDirection::below)),
        ...
        value.at("authorized_seats").get<std::vector<std::string>>(),
        value.at("decision_group").get<std::string>(),
    };
}
...
[[nodiscard]] M11CostWeights weights_from_json(const Json &value) {
...
[[nodiscard]] Json cost_spec_json(const M11AdjustmentCostSpec &value) {
    return Json{
        ...
        {"proposal_administrative_overhead", value.proposal_administrative_overhead},
        {"refund_on_cancel", value.refund_on_cancel},
        {"refund_on_failed_execution", value.refund_on_failed_execution},
        {"refund_on_supersede", value.refund_on_supersede},
        {"regime_switch", weights_json(value.regime_switch)},
    };
}

[[nodiscard]] M11AdjustmentCostSpec cost_spec_from_json(const Json &value) {
    require_object(value, {"emergency_premium", "major", "operational", "ordinary",
    ...
    result.regime_switch = weights_from_json(value.at("regime_switch"));
    result.operational = weights_from_json(value.at("operational"));
    result.proposal_administrative_overhead =
        value.at("proposal_administrative_overhead").get<double>();
    result.emergency_premium = value.at("emergency_premium").get<double>();
    result.refund_on_cancel = value.at("refund_on_cancel").get<double>();
    result.refund_on_supersede = value.at("refund_on_supersede").get<double>();
...
[[nodiscard]] Json occupant_json(const M11OccupantSpec &value) {
    ...
    return Json{
        ...
        {"random_action_probability", value.random_action_probability},
        {"schedule", std::move(schedule)},
        {"seed", value.seed},
    };
}

[[nodiscard]] M11OccupantSpec occupant_from_json(const Json &value) {
    require_object(value, {"action_dimensions", "artifact_path", "kind", "occupant_id",
                           "random_action_probability", "schedule", "seed"});
    M11OccupantSpec result;
    result.kind = checked_enum<M11OccupantKind>(
        value.at("kind"),
        static_cast<std::uint8_t>(M11OccupantKind::reinforcement_learning));
    result.occupant_id = value.at("occupant_id").get<std::string>();
    ...
    for (const auto &entry : value.at("schedule")) {
        require_object(entry, {"actions", "boundary", "decision_group"});
        M11ScheduledPolicyActions scheduled;
        scheduled.boundary = Tick(entry.at("boundary").get<std::uint64_t>());
        scheduled.decision_group = entry.at("decision_group").get<std::string>();
    ...
    result.artifact_path = value.at("artifact_path").get<std::string>();
    result.action_dimensions =
        value.at("action_dimensions").get<std::vector<std::string>>();
    return result;
}

[[nodiscard]] Json shock_authority_json(const M11ShockAuthority &value) {
    ...
    return Json{
        ...
        {"maximum_absolute_magnitude", value.maximum_absolute_magnitude},
        {"maximum_duration_ticks", value.maximum_duration_ticks},
        {"maximum_schedule_ahead_ticks", value.maximum_schedule_ahead_ticks},
        {"minimum_announcement_lead_ticks", value.minimum_announcement_lead_ticks},
        {"principal", value.principal},
    };
}

[[nodiscard]] M11ShockAuthority shock_authority_from_json(const Json &value) {
    require_object(value,
                   {"allow_all_economies", "allow_global", "allowed_economies",
                    "allowed_kinds", "granted_seats", "maximum_absolute_magnitude",
                    "maximum_duration_ticks", "maximum_schedule_ahead_ticks",
                    "minimum_announcement_lead_ticks", "principal"});
    M11ShockAuthority result;
    result.principal = value.at("principal").get<std::string>();
    result.granted_seats = value.at("granted_seats").get<std::vector<std::string>>();
    ...
    result.allow_all_economies = value.at("allow_all_economies").get<bool>();
    result.allow_global = value.at("allow_global").get<bool>();
    result.minimum_announcement_lead_ticks =
        value.at("minimum_announcement_lead_ticks").get<std::uint64_t>();
    result.maximum_schedule_ahead_ticks =
        value.at("maximum_schedule_ahead_ticks").get<std::uint64_t>();
...
[[nodiscard]] Json seat_json(const M11SeatRuntime &value) {
    const auto artifact_bytes = value.artifact.has_value()
...
[[nodiscard]] M11SeatRuntime seat_from_json(const Json &value) {
    require_object(value, {"artifact_bytes", "artifact_sha256", "decision_counter",
                           "economy", "occupant", "seat"});
    M11SeatRuntime result;
    result.assignment.economy = EconomyId(value.at("economy").get<std::uint64_t>());
    result.assignment.seat = value.at("seat").get<std::string>();
    result.assignment.occupant = occupant_from_json(value.at("occupant"));
    result.decision_counter = value.at("decision_counter").get<std::uint64_t>();
    result.artifact_sha256 = value.at("artifact_sha256").get<std::string>();
    auto bytes = bytes_from_hex(value.at("artifact_bytes"));
    if (result.assignment.occupant.kind == M11OccupantKind::reinforcement_learning) {
        if (bytes.empty()) {
            throw std::runtime_error("RL artifact is absent");
        }
        auto artifact = NativePolicyArtifact::load_bytes(bytes);
        if (!artifact.ok() ||
            artifact.get_if()->info().artifact_sha256 != result.artifact_sha256 ||
    ...
    } else if (!bytes.empty() || !result.artifact_sha256.empty()) {
        throw std::runtime_error("non-RL seat contains an artifact");
    }
    return result;
}

[[nodiscard]] Json archived_seat_json(const M11ArchivedSeatOccupant &value) {
    return {
...
[[nodiscard]] M11ArchivedSeatOccupant archived_seat_from_json(const Json &value) {
...
[[nodiscard]] Json seat_operation_json(const M11SeatOperationRecord &value) {
    return {
        {"archived_occupant_id", value.archived_occupant_id.has_value()
...
[[nodiscard]] M11SeatOperationRecord seat_operation_from_json(const Json &value) {
    require_object(value,
                   {"archived_occupant_id", "kind", "operation_id", "request_hash"});
    std::optional<std::string> archived;
    if (!value.at("archived_occupant_id").is_null()) {
        archived = value.at("archived_occupant_id").get<std::string>();
    }
    return {
        value.at("operation_id").get<std::string>(),
        checked_enum<M11SeatOperationKind>(
            value.at("kind"),
            static_cast<std::uint8_t>(M11SeatOperationKind::restoration)),
...
[[nodiscard]] M11ControllerEvent event_from_json(const Json &value) {
    require_object(value,
                   {"actor", "boundary", "canonical_payload", "event_type", "hash",
                    "operation_id", "prior_hash", "sequence", "visibility"});
    return {
...
[[nodiscard]] M11ReleaseRecord release_from_json(const Json &value) {
    require_object(value, {"economy", "observed_at", "released_at", "revision",
...
[[nodiscard]] Json controller_archive_json(const M11ControlledState &state,
    ...
    for (const auto &entry : state.coordinator.idempotency_records()) {
    ...
    for (const auto &entry : state.archived_seat_occupants) {
        archived_seat_occupants.push_back(archived_seat_json(entry));
    }
    Json seat_operations = Json::array();
    for (const auto &entry : state.seat_operations) {
        seat_operations.push_back(seat_operation_json(entry));
    }
    ...
    return Json{
        {"archived_seat_occupants", std::move(archived_seat_occupants)},
        ...
        {"fill_unassigned_with_null", run_spec.fill_unassigned_with_null},
        {"idempotency", std::move(idempotency)},
        {"maximum_events", run_spec.maximum_events},
        {"maximum_releases", run_spec.maximum_releases},
        {"next_decision_sequence", state.coordinator.next_decision_sequence()},
...
[[nodiscard]] bool
checkpointable_advance_options(const M11ControllerRunSpec &spec) noexcept {
    return spec.advance_options.domestic.empty() &&
           spec.advance_options.fault_point == simulation::M9FaultPoint::none &&
           !spec.advance_options.require_world_rollback &&
           spec.advance_options.worker_count == spec.worker_count;
}

[[nodiscard]] M11ControlledState state_from_archive(const Json &root,
                                                    const simulation::M9World &world,
                                                    M11ControllerRunSpec &run_spec) {
    require_object(root, {"archived_seat_occupants",
                          "boundary_sequence",
                          "budgets",
                          "calendars",
                          "contexts",
                          ...
                          "triggers",
                          "worker_count"});
    if (root.at("schema_version").get<std::uint32_t>() != kCheckpointSchemaVersion) {
    ...
    run_spec.cost_spec = cost_spec_from_json(root.at("cost_spec"));
    run_spec.fill_unassigned_with_null =
        root.at("fill_unassigned_with_null").get<bool>();
    run_spec.worker_count = root.at("worker_count").get<std::uint32_t>();
    run_spec.advance_options.worker_count = run_spec.worker_count;
    run_spec.maximum_events = root.at("maximum_events").get<std::size_t>();
    run_spec.maximum_releases = root.at("maximum_releases").get<std::size_t>();
    for (const auto &entry : root.at("shock_authorities")) {
        run_spec.shock_authorities.push_back(shock_authority_from_json(entry));
    }
    if (!std::is_sorted(
    ...
    auto coordinator = M11PolicyCoordinator::create(world, run_spec.cost_spec);
    if (!scheduler.ok() || !coordinator.ok() || run_spec.worker_count == 0U ||
        run_spec.maximum_events == 0U || run_spec.maximum_releases == 0U ||
    ...
    for (const auto &entry : root.at("trigger_states")) {
        require_object(entry, {"active", "cooldown_until", "economy",
    ...
    if (!scheduler.get_if()
             ->restore_trigger_states(std::move(trigger_states), world.economy_count())
             .ok()) {
    ...
    for (const auto &entry : root.at("policy_versions")) {
        policy_versions.push_back(policy_version_from_json(entry));
    }
    ...
    for (const auto &entry : root.at("pending")) {
        require_object(entry, {"context", "decision", "proposal"});
        pending.push_back({
    ...
    for (const auto &entry : root.at("budgets")) {
        require_object(entry, {"capacity", "decision_group", "economy", "remaining",
    ...
    for (const auto &entry : root.at("idempotency")) {
        require_object(entry, {"decision_id", "key", "request_hash"});
    ...
    if (!coordinator.get_if()
             ->restore(world, std::move(policy_versions), std::move(contexts),
                       std::move(pending), std::move(decisions), std::move(budgets),
                       std::move(idempotency),
                       root.at("next_decision_sequence").get<std::uint64_t>())
             .ok()) {
    ...
    if (!events
             .restore(std::move(event_values),
                      root.at("events_next_sequence").get<std::uint64_t>(),
    ...
    if (!releases
             .restore(std::move(release_values),
                      root.at("releases_next_sequence").get<std::uint64_t>())
             .ok()) {
    ...
    state.phase = checked_enum<M11BoundaryPhase>(
        root.at("phase"), static_cast<std::uint8_t>(M11BoundaryPhase::ready_to_commit));
    state.opened_context_ids =
        root.at("opened_context_ids").get<std::vector<std::string>>();
    state.boundary_sequence = root.at("boundary_sequence").get<std::uint64_t>();
    ...
    if (std::adjacent_find(state.seats.begin(), state.seats.end(),
                           [](const M11SeatRuntime &left, const M11SeatRuntime &right) {
                               return left.assignment.economy ==
                                          right.assignment.economy &&
                                      left.assignment.seat == right.assignment.seat;
                           }) != state.seats.end()) {
        throw std::runtime_error("checkpoint seats are duplicated");
    }
    for (const auto &seat : state.seats) {
        if (seat.assignment.economy.value() >= world.economy_count() ||
    ...
    for (const auto &entry : root.at("archived_seat_occupants")) {
        state.archived_seat_occupants.push_back(archived_seat_from_json(entry));
    }
    if (!std::is_sorted(state.archived_seat_occupants.begin(),
        ...
        std::adjacent_find(state.archived_seat_occupants.begin(),
                           state.archived_seat_occupants.end(),
                           [](const M11ArchivedSeatOccupant &left,
                              const M11ArchivedSeatOccupant &right) {
                               return left.archive_id == right.archive_id;
                           }) != state.archived_seat_occupants.end()) {
        throw std::runtime_error("checkpoint seat archive differs");
    }
    for (const auto &entry : state.archived_seat_occupants) {
        if (entry.archive_id.empty() || entry.archive_id.size() > 256U ||
            entry.archive_id.find('\0') != std::string::npos ||
            entry.runtime.assignment.economy.value() >= world.economy_count() ||
            !m11_valid_seat(entry.runtime.assignment.seat)) {
            throw std::runtime_error("checkpoint archived seat is invalid");
        }
    }
    for (const auto &entry : root.at("seat_operations")) {
        state.seat_operations.push_back(seat_operation_from_json(entry));
    }
    if (state.seat_operations.size() > run_spec.maximum_events) {
        throw std::runtime_error("checkpoint seat operation history is oversized");
    }
    std::set<std::string> operation_ids;
    for (const auto &entry : state.seat_operations) {
        if (entry.operation_id.empty() ||
            entry.operation_id.size() > kM10MaximumOperationIdBytes ||
            entry.operation_id.find('\0') != std::string::npos ||
            ...
            (entry.archived_occupant_id.has_value() &&
             (entry.archived_occupant_id->empty() ||
              ...
              entry.archived_occupant_id->find('\0') != std::string::npos))) {
            throw std::runtime_error("checkpoint seat operation is invalid");
        }
    }
...
save_m11_checkpoint(const M11ControlledSession &session) {
    ...
    try {
        const auto archive_text =
            controller_archive_json(session.state_, session.run_spec_).dump();
        const auto archive = std::span<const std::uint8_t>(
            reinterpret_cast<const std::uint8_t *>(archive_text.data()),
            archive_text.size());
        return save_hybrid_checkpoint(session.bridge_, {}, archive);
    } catch (const std::bad_alloc &) {
        return Status(ErrorCode::allocation_failure,
                      "M11 checkpoint allocation failed");
    } catch (const std::exception &) {
        return Status(ErrorCode::internal_error, "M11 checkpoint encoding failed");
    }
}
...
load_m11_checkpoint(std::span<const std::uint8_t> checkpoint) {
    ...
    try {
        const std::string archive_text(
            reinterpret_cast<const char *>(loaded.get_if()->controller_archive.data()),
        ...
        auto state = state_from_archive(root, loaded.get_if()->bridge.engine().world(),
                                        run_spec);
        auto expected = seal_m11_controller_state(
            loaded.get_if()->bridge.engine().tick(),
            loaded.get_if()->bridge.engine().policy_generation(), state);
        if (!expected.ok() ||
            loaded.get_if()->bridge.controller_envelope() != *expected.get_if()) {
            return corrupt_checkpoint();
        }
        return M11ControlledSession(std::move(loaded.get_if()->bridge),
