...
[[nodiscard]] std::string context_identifier(EconomyId economy, std::string_view seat,
                                             std::string_view decision_group,
                                             Tick boundary, bool emergency,
                                             std::string_view trigger) {
    std::ostringstream stream;
    stream << "ctx:" << economy.value() << ':' << seat << ':' << decision_group << ':'
           << boundary.value() << ':';
    if (emergency) {
...
[[nodiscard]] Tick administrative_window_marker(const M11CalendarSpec &calendar,
                                                Tick boundary) noexcept {
    if (boundary.value() < calendar.offset_ticks) {
        return Tick(calendar.offset_ticks);
    }
    const auto cycle =
        (boundary.value() - calendar.offset_ticks) / calendar.period_ticks;
    return Tick(calendar.offset_ticks + cycle * calendar.period_ticks);
}

[[nodiscard]] const M11CalendarSpec *
calendar_for(const M11DecisionScheduler &scheduler,
             std::string_view decision_group) noexcept {
    static const M11CalendarSpec emergency_calendar{"emergency", 91U, 0U, 1U, 20.0};
...
void append_policy_value(std::vector<std::uint8_t> &bytes, const PolicyValue &value) {
    ...
    if (const auto *boolean = std::get_if<bool>(&value)) {
        ...
        append_string(bytes, *text);
    } else if (const auto *economies = std::get_if<PolicyEconomySet>(&value)) {
        append_u64(bytes, static_cast<std::uint64_t>(economies->size()));
...
[[nodiscard]] core::StateDigest proposal_hash(const M11PolicyProposal &proposal) {
    ...
    append_u64(bytes,
               static_cast<std::uint64_t>(proposal.based_on_policy_versions.size()));
...
[[nodiscard]] const M11ProposalVersion *
proposal_version(std::span<const M11ProposalVersion> versions,
                 std::string_view lever) noexcept {
    const auto found =
        std::lower_bound(versions.begin(), versions.end(), lever,
                         [](const M11ProposalVersion &entry, std::string_view name) {
                             return entry.lever < name;
                         });
    return found == versions.end() || found->lever != lever ? nullptr : &*found;
}

[[nodiscard]] const M11PermittedAction *
permitted_action(std::span<const M11PermittedAction> actions,
                 std::string_view lever) noexcept {
    const auto found =
    ...
    return found == actions.end() || found->lever != lever ? nullptr : &*found;
}

[[nodiscard]] const M11PendingDecision *
active_pending_for_lever(std::span<const M11PendingDecision> pending, EconomyId economy,
                         std::string_view lever,
                         std::string_view excluded_decision = {}) noexcept {
    const auto found = std::find_if(
        pending.begin(), pending.end(), [&](const M11PendingDecision &item) {
            ...
            return std::any_of(item.proposal.actions.begin(),
...
[[nodiscard]] const M11PendingDecision *
pending_for_proposal(std::span<const M11PendingDecision> pending,
                     std::string_view proposal_id) noexcept {
    const auto found = std::find_if(
        pending.begin(), pending.end(), [proposal_id](const M11PendingDecision &item) {
            return item.proposal.proposal_id == proposal_id &&
                   item.decision.status == M11DecisionStatus::accepted_pending;
        });
    return found == pending.end() ? nullptr : &*found;
}
...
[[nodiscard]] bool policy_truthy(const PolicyValue &value) noexcept {
    ...
    if (const auto *economies = std::get_if<PolicyEconomySet>(&value)) {
        return !economies->empty();
    }
    return false;
}

[[nodiscard]] Result<PolicyValue> world_policy_value(const simulation::M9World &world,
                                                     EconomyId economy,
                                                     std::string_view lever) {
    ...
    if (index >= world.external_policies().size()) {
        return Status(ErrorCode::out_of_range, "M11 policy economy is out of range");
    }
    return m11_policy_value(*domestic.get_if(), world.external_policies()[index],
                            lever);
}

[[nodiscard]] Result<PolicyValue>
batch_policy_value(const simulation::WorldPolicyBatch &batch, EconomyId economy,
                   std::string_view lever) {
    const auto index = static_cast<std::size_t>(economy.value());
    if (index >= batch.domestic.size() || index >= batch.external.size()) {
        return Status(ErrorCode::out_of_range,
                      "M11 projected policy economy is out of range");
    }
    return m11_policy_value(batch.domestic[index], batch.external[index], lever);
}

[[nodiscard]] Status
validate_enabled_prerequisites(const simulation::WorldPolicyBatch &batch,
    ...
    for (const auto &action : actions) {
        ...
        for (const auto prerequisite : split_contract_list(lever->enabled_if)) {
            auto value = batch_policy_value(batch, action.economy, prerequisite);
            if (!value.ok()) {
                return value.status();
            }
...
[[nodiscard]] Result<simulation::M9World>
project_timeline_before(const simulation::M9World &world,
                        std::span<const M11PendingDecision> pending,
                        Tick effective_tick, std::string_view excluded_decision) {
    ...
    std::sort(timeline.begin(), timeline.end(),
              [](const M11PendingDecision *left, const M11PendingDecision *right) {
                  return std::tuple{left->decision.effective_at->value(),
                                    left->decision.accepted_sequence,
                                    left->decision.decision_id} <
                         std::tuple{right->decision.effective_at->value(),
...
[[nodiscard]] std::vector<NativePolicyAction>
flatten_actions(std::span<const M11PendingDecision *const> candidates) {
    std::vector<NativePolicyAction> actions;
    ...
    std::sort(actions.begin(), actions.end(),
              [](const NativePolicyAction &left, const NativePolicyAction &right) {
...
validate_candidate_set(const simulation::M9World &world,
    ...
    auto status = validate_enabled_prerequisites(*batch.get_if(), actions);
    if (!status.ok()) {
        return status;
    }
...
[[nodiscard]] std::vector<NativePolicyAction>
changed_actions(const simulation::M9World &world,
    ...
    for (std::size_t economy = 0U; economy < world.economy_count(); ++economy) {
        ...
        for (const auto &lever : levers) {
            auto before = world_policy_value(world, economy_id, lever.name);
            auto after = batch_policy_value(batch, economy_id, lever.name);
            if (before.ok() && after.ok() &&
                !m11_policy_values_equal(*before.get_if(), *after.get_if())) {
                result.push_back(
                    {economy_id, std::string(lever.name), *after.get_if()});
            }
        }
    }
    return result;
...
[[nodiscard]] std::string event_payload(std::string_view name,
...
[[nodiscard]] std::string decision_event_payload(const M11PolicyDecision &decision) {
    return Json{
        ...
        {"adjustment_cost_bits",
         std::bit_cast<std::uint64_t>(decision.adjustment_cost)},
        ...
        {"reserved_administrative_cost_bits",
         std::bit_cast<std::uint64_t>(decision.reserved_administrative_cost)},
        {"schema_version", 1U},
...
[[nodiscard]] M11PolicyDecision decision_from_event_payload(std::string_view payload) {
    const auto value = Json::parse(payload);
    static constexpr std::array<std::string_view, 10U> keys{
        "accepted_at",          "accepted_sequence",
        "adjustment_cost_bits", "decision_id",
        "effective_at",         "proposal_id",
        "reason_code",          "reserved_administrative_cost_bits",
        "schema_version",       "status",
    };
    if (!value.is_object() || value.size() != keys.size() ||
        std::any_of(keys.begin(), keys.end(),
                    [&](std::string_view key) { return !value.contains(key); }) ||
        value.at("schema_version").get<std::uint32_t>() != 1U) {
        throw std::runtime_error("decision replay payload schema differs");
    }
    const auto status = value.at("status").get<std::uint8_t>();
    if (status > static_cast<std::uint8_t>(M11DecisionStatus::failed_at_execution)) {
        throw std::runtime_error("decision replay status differs");
    }
    const auto optional_tick_from = [](const Json &entry) -> std::optional<Tick> {
        return entry.is_null() ? std::optional<Tick>{}
                               : std::optional<Tick>(Tick(entry.get<std::uint64_t>()));
    };
    M11PolicyDecision result{
        ...
        std::bit_cast<double>(
            value.at("reserved_administrative_cost_bits").get<std::uint64_t>()),
        std::bit_cast<double>(value.at("adjustment_cost_bits").get<std::uint64_t>()),
    };
    if (!valid_identifier(result.decision_id) ||
        ...
        !std::isfinite(result.reserved_administrative_cost) ||
        !std::isfinite(result.adjustment_cost) ||
        result.reserved_administrative_cost < 0.0 || result.adjustment_cost < 0.0) {
        throw std::runtime_error("decision replay payload is invalid");
    }
    return result;
}

[[nodiscard]] bool same_touched_levers(const M11PolicyProposal &left,
...
[[nodiscard]] bool budget_less(const M11AdministrativeBudget &left,
    ...
    return std::tuple{left.economy.value(), left.seat, left.decision_group} <
           std::tuple{right.economy.value(), right.seat, right.decision_group};
}

} // namespace

std::string_view m11_decision_status_name(M11DecisionStatus status) noexcept {
    switch (status) {
...
replay_m11_decisions(std::span<const M11ControllerEvent> events) {
    ...
    try {
        std::vector<M11ControllerEvent> copy(events.begin(), events.end());
        M11EventStream validated(kM11MaximumControllerEvents);
        const core::StateDigest head =
            copy.empty() ? core::StateDigest{} : copy.back().hash;
        const auto validation = validated.restore(std::move(copy), events.size(), head);
        if (!validation.ok()) {
            return validation;
        }
        ...
        for (const auto &event : events) {
            const bool initial = event.event_type == "decision_rejected" ||
                                 event.event_type == "decision_accepted_noop" ||
                                 event.event_type == "decision_accepted";
            const bool transition =
                event.event_type == "decision_superseded" ||
                event.event_type == "decision_cancelled" ||
                event.event_type == "decision_failed_at_execution" ||
                event.event_type == "decision_effective";
            if (!initial && !transition) {
                continue;
            }
            auto decision = decision_from_event_payload(event.canonical_payload);
            const auto expected_status =
                event.event_type == "decision_rejected" ? M11DecisionStatus::rejected
                : event.event_type == "decision_accepted_noop"
            ...
            const auto found =
                std::find_if(decisions.begin(), decisions.end(),
                             [&](const M11PolicyDecision &entry) {
                                 return entry.decision_id == decision.decision_id;
                             });
            if (initial) {
        ...
        std::sort(decisions.begin(), decisions.end(),
                  [](const M11PolicyDecision &left, const M11PolicyDecision &right) {
                      return left.accepted_sequence < right.accepted_sequence;
                  });
...
bool m11_world_capability(const simulation::M9World &world, EconomyId economy,
    ...
    const auto has = [&](simulation::M4Capability item) {
        return (real->capability_mask & simulation::capability_bit(item)) != 0U;
    };
    ...
    if (capability == "housing_construction_enabled") {
        return domestic->housing_rules.enabled && domestic->housing_rules.construction;
    }
    if (capability == "housing_enabled") {
        return domestic->housing_rules.enabled;
    }
    if (capability == "housing_market_enabled") {
        return domestic->housing_rules.enabled && domestic->housing_rules.resale_market;
    }
    ...
    if (capability == "mortgage_enabled") {
        return domestic->housing_rules.enabled && domestic->housing_rules.mortgages;
    }
    return false;
}
...
const M11DecisionContext *
M11PolicyCoordinator::find_context(std::string_view context_id) const noexcept {
    const auto found =
        std::lower_bound(contexts_.begin(), contexts_.end(), context_id,
                         [](const M11DecisionContext &context, std::string_view id) {
                             return context.context_id < id;
                         });
    return found == contexts_.end() || found->context_id != context_id ? nullptr
                                                                       : &*found;
}

const M11PolicyDecision *
M11PolicyCoordinator::find_decision(std::string_view decision_id) const noexcept {
    const auto found =
        std::lower_bound(decisions_.begin(), decisions_.end(), decision_id,
                         [](const M11PolicyDecision &decision, std::string_view id) {
                             return decision.decision_id < id;
                         });
    return found == decisions_.end() || found->decision_id != decision_id ? nullptr
                                                                          : &*found;
}

const M11PendingDecision *
M11PolicyCoordinator::find_pending(std::string_view decision_id) const noexcept {
    const auto found =
        std::lower_bound(pending_.begin(), pending_.end(), decision_id,
                         [](const M11PendingDecision &pending, std::string_view id) {
                             return pending.decision.decision_id < id;
                         });
    return found == pending_.end() || found->decision.decision_id != decision_id
               ? nullptr
               : &*found;
}

Result<std::uint64_t>
M11PolicyCoordinator::policy_version(EconomyId economy,
                                     std::string_view lever) const noexcept {
    const auto *found = find_policy_version(economy, lever);
    return found == nullptr
               ? Result<std::uint64_t>(
                     Status(ErrorCode::not_found, "M11 policy version was not found"))
               : Result<std::uint64_t>(found->version);
}

const M11PolicyVersion *
M11PolicyCoordinator::find_policy_version(EconomyId economy,
                                          std::string_view lever) const noexcept {
    const auto key = std::pair{economy.value(), lever};
    const auto found = std::lower_bound(
        policy_versions_.begin(), policy_versions_.end(), key,
        [](const M11PolicyVersion &entry, const auto &candidate) {
            return std::pair{entry.economy.value(), std::string_view(entry.lever)} <
                   candidate;
        });
    return found == policy_versions_.end() || found->economy != economy ||
                   found->lever != lever
               ? nullptr
               : &*found;
}
...
M11PolicyVersion *
M11PolicyCoordinator::mutable_policy_version(EconomyId economy,
                                             std::string_view lever) noexcept {
    const auto key = std::pair{economy.value(), lever};
    const auto found = std::lower_bound(
        policy_versions_.begin(), policy_versions_.end(), key,
...
M11DecisionContext *
M11PolicyCoordinator::mutable_context(std::string_view context_id) noexcept {
    return const_cast<M11DecisionContext *>(find_context(context_id));
}

M11PendingDecision *
M11PolicyCoordinator::mutable_pending(std::string_view decision_id) noexcept {
    return const_cast<M11PendingDecision *>(find_pending(decision_id));
}

M11PolicyDecision *
M11PolicyCoordinator::mutable_decision(std::string_view decision_id) noexcept {
    return const_cast<M11PolicyDecision *>(find_decision(decision_id));
}

M11AdministrativeBudget *
M11PolicyCoordinator::mutable_budget(EconomyId economy, std::string_view seat,
                                     std::string_view decision_group) noexcept {
    const auto key = std::tuple{economy.value(), seat, decision_group};
    const auto found = std::lower_bound(
        budgets_.begin(), budgets_.end(), key,
        [](const M11AdministrativeBudget &budget, const auto &candidate) {
            return std::tuple{budget.economy.value(), std::string_view(budget.seat),
                              std::string_view(budget.decision_group)} < candidate;
        });
    return found == budgets_.end() || found->economy != economy ||
                   found->seat != seat || found->decision_group != decision_group
               ? nullptr
               : &*found;
}

Result<M11AdministrativeBudget *>
M11PolicyCoordinator::ensure_budget(const M11DecisionScheduler &scheduler,
                                    EconomyId economy, std::string_view seat,
                                    std::string_view decision_group, Tick boundary) {
    const auto *calendar = calendar_for(scheduler, decision_group);
    if (calendar == nullptr) {
        return Status(ErrorCode::not_found, "M11 decision calendar was not found");
    }
    const auto marker = administrative_window_marker(*calendar, boundary);
    auto *budget = mutable_budget(economy, seat, decision_group);
    if (budget == nullptr) {
        budgets_.push_back({
            economy,
            std::string(seat),
...
Result<M11DecisionContext> M11PolicyCoordinator::open_context(
    const simulation::M9World &world, const M11DecisionScheduler &scheduler,
    Tick boundary, EconomyId economy, std::string seat, std::string decision_group,
    Tick expires_at, bool emergency, std::string emergency_trigger,
    std::uint64_t elapsed_ticks, std::span<const M11ReleasedObservation> observation) {
    if (boundary != world.tick() ||
        static_cast<std::size_t>(economy.value()) >= world.economy_count() ||
        !m11_valid_seat(seat) || expires_at < boundary ||
        (!emergency && !m11_valid_decision_group(decision_group)) ||
        (emergency &&
         (!valid_identifier(emergency_trigger) || decision_group != "emergency")) ||
        (!emergency && !emergency_trigger.empty())) {
        return Status(ErrorCode::invalid_argument,
                      "M11 decision context request is invalid");
    }
    const auto context_id = context_identifier(economy, seat, decision_group, boundary,
                                               emergency, emergency_trigger);
    if (const auto *existing = find_context(context_id); existing != nullptr) {
        return *existing;
    }
    ...
    for (const auto &lever : m11_policy_levers()) {
        if (lever.owner_role != context.seat ||
            (!emergency && lever.decision_group != context.decision_group) ||
            (emergency && !lever.emergency)) {
            continue;
        }
        auto current = world_policy_value(world, economy, lever.name);
        if (!current.ok()) {
            return current.status();
        }
        auto *version = mutable_policy_version(economy, lever.name);
        if (version == nullptr) {
            return Status(ErrorCode::internal_error,
                          "M11 policy version table is incomplete");
        }
        const auto lag = emergency && lever.emergency_implementation_lag.has_value()
                             ? *lever.emergency_implementation_lag
                             : lever.implementation_lag;
        auto earliest = Tick(boundary.value() + lag);
        std::string reason;
        for (const auto capability : split_contract_list(lever.required_capabilities)) {
            if (!m11_world_capability(world, economy, capability)) {
                reason = "missing_capability:" + std::string(capability);
                break;
            }
        }
        if (reason.empty() &&
            active_pending_for_lever(pending_, economy, lever.name) != nullptr) {
            reason = "pending_conflict";
        }
        if (version->last_effective.has_value()) {
            const auto hold_until =
                Tick(version->last_effective->value() + lever.minimum_hold_ticks);
        ...
        for (const auto prerequisite : split_contract_list(lever.enabled_if)) {
            auto value = world_policy_value(world, economy, prerequisite);
            if (!value.ok()) {
                return value.status();
            }
            if (reason.empty() && !policy_truthy(*value.get_if())) {
                reason = "disabled_prerequisite:" + std::string(prerequisite);
            }
            auto prerequisite_version = policy_version(economy, prerequisite);
            if (!prerequisite_version.ok()) {
                return prerequisite_version.status();
            }
            context.policy_versions.push_back({economy, std::string(prerequisite),
                                               *prerequisite_version.get_if(),
                                               std::nullopt});
        }
        const auto minimum_admin =
            cost_spec_.proposal_administrative_overhead + lever.administrative_weight;
        if (reason.empty() && budget->remaining + kBudgetTolerance < minimum_admin) {
    ...
    std::sort(context.permitted_actions.begin(), context.permitted_actions.end(),
              [](const M11PermittedAction &left, const M11PermittedAction &right) {
                  return left.lever < right.lever;
              });
    std::sort(context.policy_versions.begin(), context.policy_versions.end(),
              policy_version_less);
    context.policy_versions.erase(
        std::unique(context.policy_versions.begin(), context.policy_versions.end(),
                    [](const M11PolicyVersion &left, const M11PolicyVersion &right) {
...
M11PolicyDecision M11PolicyCoordinator::make_decision(
    const M11PolicyProposal &proposal, M11DecisionStatus status, std::string reason,
    std::optional<Tick> accepted_at, std::optional<Tick> effective_at,
    double administrative_cost, double adjustment_cost) {
    std::ostringstream identifier;
    identifier << "decision:" << std::setw(12) << std::setfill('0')
               << next_decision_sequence_;
    M11PolicyDecision result{
        identifier.str(),        proposal.proposal_id, status,
        std::move(reason),       accepted_at,          effective_at,
        next_decision_sequence_, administrative_cost,  adjustment_cost,
    };
    ++next_decision_sequence_;
    return result;
}
...
M11PolicyCoordinator::reject(const M11PolicyProposal &proposal, std::string reason,
    ...
    auto decision = make_decision(proposal, M11DecisionStatus::rejected,
                                  std::move(reason), std::nullopt, std::nullopt);
    decisions_.push_back(decision);
    std::sort(decisions_.begin(), decisions_.end(), decision_less);
    idempotency_.push_back(
        {proposal.idempotency_key, request_hash, decision.decision_id});
    std::sort(idempotency_.begin(), idempotency_.end(),
              [](const M11IdempotencyRecord &left, const M11IdempotencyRecord &right) {
...
Result<M11PolicyDecision>
M11PolicyCoordinator::submit(const simulation::M9World &world,
    ...
    if (boundary != world.tick() || !valid_actor(actor) ||
        ...
        return Status(ErrorCode::invalid_argument, "M11 policy proposal is invalid");
    }
    std::sort(proposal.actions.begin(), proposal.actions.end(),
              [](const NativePolicyAction &left, const NativePolicyAction &right) {
                  return std::pair{left.economy.value(), left.lever} <
                         std::pair{right.economy.value(), right.lever};
              });
    if (std::adjacent_find(
            ...
            [](const NativePolicyAction &left, const NativePolicyAction &right) {
                return left.economy == right.economy && left.lever == right.lever;
    ...
    std::sort(proposal.based_on_policy_versions.begin(),
              ...
              [](const M11ProposalVersion &left, const M11ProposalVersion &right) {
                  return left.lever < right.lever;
              });
    if (std::adjacent_find(
    ...
    const auto receipt = std::lower_bound(
        idempotency_.begin(), idempotency_.end(), proposal.idempotency_key,
        [](const M11IdempotencyRecord &entry, std::string_view key) {
            return entry.key < key;
        });
    if (receipt != idempotency_.end() && receipt->key == proposal.idempotency_key) {
        if (receipt->request_hash != request_hash) {
            return Status(ErrorCode::already_exists,
                          "M11 idempotency key payload differs");
        }
        const auto *decision = find_decision(receipt->decision_id);
        return decision == nullptr ? Result<M11PolicyDecision>(
    ...
    auto event = staged_events.append(
        boundary, "proposal_submitted", proposal.idempotency_key, std::string(actor),
        event_payload("proposal_id", proposal.proposal_id),
        M11EventVisibility::privileged_audit);
    if (!event.ok()) {
        return event.status();
    }
    const auto reject_with = [&](std::string reason) -> Result<M11PolicyDecision> {
        auto decision = staged.reject(proposal, std::move(reason), request_hash);
        if (!decision.ok()) {
            return decision.status();
        }
        auto rejected = staged_events.append(boundary, "decision_rejected",
    ...
    if (std::any_of(staged.decisions_.begin(), staged.decisions_.end(),
    ...
    if (context->answered_by_proposal.has_value()) {
        if (!proposal.supersedes_proposal_id.has_value() ||
            *proposal.supersedes_proposal_id != *context->answered_by_proposal) {
            return reject_with("context_already_answered");
        }
    }
    if (proposal.supersedes_proposal_id.has_value()) {
        const auto *found =
            pending_for_proposal(staged.pending_, *proposal.supersedes_proposal_id);
        if (found == nullptr) {
            return reject_with("proposal_not_supersedable");
        }
        superseded = staged.mutable_pending(found->decision.decision_id);
        if (superseded == nullptr || superseded->context.economy != context->economy ||
            superseded->context.seat != context->seat) {
            return reject_with("supersede_scope_mismatch");
        }
        if (!same_touched_levers(proposal, superseded->proposal)) {
    ...
    for (const auto &action : proposal.actions) {
        ...
        const auto *permitted =
            permitted_action(context->permitted_actions, action.lever);
        if (lever == nullptr || permitted == nullptr ||
            lever->owner_role != context->seat ||
            (!context->emergency && lever->decision_group != context->decision_group) ||
            (context->emergency && !lever->emergency)) {
            return reject_with("unauthorized_lever");
        }
        auto value_status = validate_m11_policy_value(
            *lever, action.value, action.economy, world.economy_count());
        if (!value_status.ok()) {
            return reject_with("registry_validation");
        }
        for (const auto capability :
             split_contract_list(lever->required_capabilities)) {
            if (!m11_world_capability(world, context->economy, capability)) {
                return reject_with("missing_capability");
            }
        }
        const auto expected =
            proposal_version(proposal.based_on_policy_versions, action.lever);
        const auto current = staged.policy_version(context->economy, action.lever);
        ...
        for (const auto prerequisite : split_contract_list(lever->enabled_if)) {
            const auto *expected_prerequisite =
                proposal_version(proposal.based_on_policy_versions, prerequisite);
            const auto current_prerequisite =
                staged.policy_version(context->economy, prerequisite);
            if (expected_prerequisite == nullptr || !current_prerequisite.ok() ||
                expected_prerequisite->version != *current_prerequisite.get_if()) {
                return reject_with("stale_prerequisite_version");
            }
        }
        const auto excluded = superseded == nullptr
                                  ? std::string_view{}
                                  : std::string_view(superseded->decision.decision_id);
        if (active_pending_for_lever(staged.pending_, context->economy, action.lever,
                                     excluded) != nullptr) {
    ...
    for (const auto &action : proposal.actions) {
        const auto *lever = find_m11_policy_lever(action.lever);
        const auto lag =
            context->emergency && lever->emergency_implementation_lag.has_value()
                ? *lever->emergency_implementation_lag
                : lever->implementation_lag;
        maximum_lag = std::max(maximum_lag, lag);
    }
    const auto effective = Tick(boundary.value() + maximum_lag);
    auto projected = project_timeline_before(
        world, staged.pending_, effective,
        superseded == nullptr ? std::string_view{}
                              : std::string_view(superseded->decision.decision_id));
    if (!projected.ok()) {
        return reject_with("projected_timeline_conflict");
    }
    auto projected_batch =
        project_m11_policy_actions(*projected.get_if(), proposal.actions);
    if (!projected_batch.ok()) {
        return reject_with("registry_validation");
    }
    auto prerequisites =
        validate_enabled_prerequisites(*projected_batch.get_if(), proposal.actions);
    if (!prerequisites.ok()) {
    ...
    for (const auto &action : proposal.actions) {
        const auto *lever = find_m11_policy_lever(action.lever);
        auto before =
            world_policy_value(*projected.get_if(), action.economy, action.lever);
        auto after =
            batch_policy_value(*projected_batch.get_if(), action.economy, action.lever);
        if (!before.ok() || !after.ok()) {
            return reject_with("registry_validation");
        }
        auto distance =
            m11_policy_numeric_distance(*lever, *before.get_if(), *after.get_if());
        if (!distance.ok()) {
            return reject_with("registry_validation");
        }
        if (lever->maximum_step.has_value() && lever->control_scale.has_value() &&
            *distance.get_if() * *lever->control_scale >
                *lever->maximum_step + 1.0e-15) {
            return reject_with("maximum_step");
        }
        auto *version = staged.mutable_policy_version(action.economy, action.lever);
        if (version == nullptr) {
            return Status(ErrorCode::internal_error,
                          "M11 policy version table is incomplete");
        }
        if (!m11_policy_values_equal(*before.get_if(), *after.get_if()) &&
            version->last_effective.has_value() &&
            effective.value() <
                version->last_effective->value() + lever->minimum_hold_ticks) {
            return reject_with("minimum_hold");
        }
        changes.push_back({lever, *before.get_if(), *after.get_if()});
    }
    const auto changed =
        std::any_of(changes.begin(), changes.end(), [](const M11PolicyChange &change) {
            return !m11_policy_values_equal(change.old_value, change.new_value);
        });
    if (staged.decisions_.size() >= kM11MaximumDecisionHistory ||
        staged.idempotency_.size() >= kM11MaximumIdempotencyRecords) {
        return Status(ErrorCode::out_of_range,
                      "M11 decision receipt capacity was reached");
    }
    if (!changed) {
        if (superseded != nullptr) {
            superseded->decision.status = M11DecisionStatus::superseded;
            superseded->decision.reason_code = "superseded";
            auto *history = staged.mutable_decision(superseded->decision.decision_id);
            if (history != nullptr) {
                *history = superseded->decision;
            }
            staged.refund(*superseded, staged.cost_spec_.refund_on_supersede);
        }
        auto decision = staged.make_decision(proposal, M11DecisionStatus::accepted_noop,
                                             "no_change", boundary, std::nullopt);
        staged.decisions_.push_back(decision);
        std::sort(staged.decisions_.begin(), staged.decisions_.end(), decision_less);
        staged.idempotency_.push_back(
            {proposal.idempotency_key, request_hash, decision.decision_id});
        std::sort(
            staged.idempotency_.begin(), staged.idempotency_.end(),
            [](const M11IdempotencyRecord &left, const M11IdempotencyRecord &right) {
                return left.key < right.key;
            });
        ...
        auto accepted = staged_events.append(
            boundary, "decision_accepted_noop", proposal.idempotency_key, "coordinator",
            decision_event_payload(decision), M11EventVisibility::institution);
        if (!accepted.ok()) {
    ...
    auto administrative_cost = m11_administrative_cost(staged.cost_spec_, changes);
    auto adjustment_cost =
        m11_adjustment_cost(staged.cost_spec_, changes, context->emergency);
    if (!administrative_cost.ok() || !adjustment_cost.ok()) {
        return reject_with("cost_validation");
    }
    const auto budget_group = std::string_view(context->decision_group);
    auto budget_result = staged.ensure_budget(scheduler, context->economy,
                                              context->seat, budget_group, boundary);
    if (!budget_result.ok()) {
        return budget_result.status();
    }
    auto *budget = *budget_result.get_if();
    ...
    if (superseded != nullptr &&
        superseded->context.administrative_window == budget->window_marker) {
        supersede_refund = superseded->decision.reserved_administrative_cost *
                           staged.cost_spec_.refund_on_supersede;
    }
    if (*administrative_cost.get_if() >
        budget->remaining + supersede_refund + kBudgetTolerance) {
    ...
    if (superseded != nullptr) {
        ...
        auto superseded_event = staged_events.append(
            boundary, "decision_superseded", proposal.idempotency_key, "coordinator",
            decision_event_payload(superseded->decision),
            M11EventVisibility::institution);
        if (!superseded_event.ok()) {
            return superseded_event.status();
        }
        budget = staged.mutable_budget(context->economy, context->seat, budget_group);
    }
    budget->remaining -= *administrative_cost.get_if();
    budget->reserved += *administrative_cost.get_if();
    if (staged.pending_.size() >= kM11MaximumPendingDecisions) {
        return Status(ErrorCode::out_of_range,
                      "M11 pending decision capacity was reached");
    }
    auto decision = staged.make_decision(
        proposal, M11DecisionStatus::accepted_pending, "accepted", boundary, effective,
        *administrative_cost.get_if(), *adjustment_cost.get_if());
    staged.pending_.push_back({decision, proposal, *context});
    std::sort(staged.pending_.begin(), staged.pending_.end(), pending_less);
    staged.decisions_.push_back(decision);
    std::sort(staged.decisions_.begin(), staged.decisions_.end(), decision_less);
    staged.idempotency_.push_back(
        {proposal.idempotency_key, request_hash, decision.decision_id});
    std::sort(staged.idempotency_.begin(), staged.idempotency_.end(),
              [](const M11IdempotencyRecord &left, const M11IdempotencyRecord &right) {
                  return left.key < right.key;
              });
    context = staged.mutable_context(proposal.context_id);
    ...
    context->administrative_reserved = budget->reserved;
    auto accepted = staged_events.append(
        boundary, "decision_accepted", proposal.idempotency_key, "coordinator",
        decision_event_payload(decision), M11EventVisibility::institution);
    if (!accepted.ok()) {
        return accepted.status();
    }
...
void M11PolicyCoordinator::refund(M11PendingDecision &pending,
                                  double fraction) noexcept {
    const auto group = std::string_view(pending.context.decision_group);
    auto *budget = mutable_budget(pending.context.economy, pending.context.seat, group);
    if (budget == nullptr ||
        budget->window_marker != pending.context.administrative_window) {
        return;
    }
    const auto reserved = pending.decision.reserved_administrative_cost;
    budget->remaining += reserved * fraction;
    budget->remaining = std::min(budget->remaining, budget->capacity);
    budget->reserved = std::max(0.0, budget->reserved - reserved);
}

Result<M11PolicyDecision> M11PolicyCoordinator::cancel(Tick boundary,
                                                       std::string_view decision_id,
                                                       std::string_view operation_id,
                                                       std::string_view actor,
                                                       M11EventStream &events) {
    if (!valid_identifier(decision_id) || !valid_identifier(operation_id) ||
        !valid_actor(actor)) {
        return Status(ErrorCode::invalid_argument,
    ...
    if (active == nullptr ||
        active->decision.status != M11DecisionStatus::accepted_pending) {
        return Status(ErrorCode::invalid_transaction_state,
                      "M11 decision is not active pending");
    }
    M11PolicyCoordinator staged = *this;
    auto staged_events = events;
    auto input = staged_events.append(boundary, "pending_cancel_requested",
    ...
    if (history == nullptr) {
        return Status(ErrorCode::internal_error, "M11 decision history is incomplete");
    }
    *history = pending->decision;
    staged.refund(*pending, staged.cost_spec_.refund_on_cancel);
    auto derived = staged_events.append(
        boundary, "decision_cancelled", std::string(operation_id), "coordinator",
        decision_event_payload(pending->decision), M11EventVisibility::institution);
    if (!derived.ok()) {
...
Result<M11DuePlan> M11PolicyCoordinator::prepare_due(const simulation::M9World &world,
                                                     Tick boundary) const {
    if (boundary != world.tick()) {
        return Status(ErrorCode::stale_handle, "M11 due boundary is stale");
    }
    ...
    for (const auto &item : pending_) {
        ...
        for (const auto &action : item.proposal.actions) {
            const auto *lever = find_m11_policy_lever(action.lever);
            const auto *expected =
                proposal_version(item.proposal.based_on_policy_versions, action.lever);
            auto current = policy_version(action.economy, action.lever);
            const auto *version = find_policy_version(action.economy, action.lever);
            if (lever == nullptr || expected == nullptr || !current.ok() ||
                expected->version != *current.get_if() ||
            ...
            for (const auto prerequisite : split_contract_list(lever->enabled_if)) {
                const auto *expected_prerequisite = proposal_version(
                    item.proposal.based_on_policy_versions, prerequisite);
        ...
        if (valid) {
            ...
            immediately_failed.push_back(item.decision.decision_id);
        }
    }
    ...
    if (!due.empty()) {
        ...
        if (full.ok()) {
            ...
            for (std::size_t size = count - 1U; size > 0U && safe.empty(); --size) {
                std::vector<std::vector<const M11PendingDecision *>> valid_subsets;
                if (count < 63U) {
                    ...
                    for (std::uint64_t mask = 1U; mask < limit; ++mask) {
                        ...
                        ++attempts;
                        if (attempts > kMaximumConservativeAttempts) {
                            break;
                        }
                        std::vector<const M11PendingDecision *> subset;
                        for (std::size_t index = 0U; index < count; ++index) {
                            if ((mask & (std::uint64_t{1U} << index)) != 0U) {
                ...
                if (attempts > kMaximumConservativeAttempts || valid_subsets.empty()) {
                    continue;
                }
                safe = valid_subsets.front();
                for (std::size_t index = 1U; index < valid_subsets.size(); ++index) {
                    safe.erase(std::remove_if(
                                   safe.begin(), safe.end(),
                                   [&](const M11PendingDecision *candidate) {
                                       return std::find(valid_subsets[index].begin(),
                ...
                if (!safe.empty() && !validate_candidate_set(world, safe).ok()) {
                    safe.clear();
                }
            }
        }
    ...
    for (const auto *item : safe) {
        plan.effective_decision_ids.push_back(item->decision.decision_id);
    }
    for (const auto *item : due) {
        if (std::find(safe.begin(), safe.end(), item) == safe.end()) {
            plan.failed_decision_ids.push_back(item->decision.decision_id);
        }
    }
    std::sort(plan.failed_decision_ids.begin(), plan.failed_decision_ids.end());
    plan.effective_changes = changed_actions(world, plan.policy_batch);
    return plan;
}

Status M11PolicyCoordinator::commit_due(const M11DuePlan &plan,
                                        M11EventStream &events) {
    M11PolicyCoordinator staged = *this;
    auto staged_events = events;
    for (const auto &decision_id : plan.failed_decision_ids) {
        ...
        if (pending == nullptr || history == nullptr ||
            pending->decision.status != M11DecisionStatus::accepted_pending) {
            return Status(ErrorCode::stale_handle, "M11 due failure decision is stale");
        }
        pending->decision.status = M11DecisionStatus::failed_at_execution;
        pending->decision.reason_code = plan.exclusion_reason.empty()
                                            ? "stale_at_execution"
                                            : plan.exclusion_reason;
        *history = pending->decision;
        staged.refund(*pending, staged.cost_spec_.refund_on_failed_execution);
        auto event = staged_events.append(plan.boundary, "decision_failed_at_execution",
    ...
    for (const auto &decision_id : plan.effective_decision_ids) {
        auto *pending = staged.mutable_pending(decision_id);
        auto *history = staged.mutable_decision(decision_id);
        if (pending == nullptr || history == nullptr ||
            pending->decision.status != M11DecisionStatus::accepted_pending) {
        ...
        *history = pending->decision;
        const auto group = std::string_view(pending->context.decision_group);
        auto *budget = staged.mutable_budget(pending->context.economy,
                                             pending->context.seat, group);
        if (budget != nullptr &&
            budget->window_marker == pending->context.administrative_window) {
            budget->reserved = std::max(
                0.0, budget->reserved - pending->decision.reserved_administrative_cost);
        }
        auto event = staged_events.append(
            plan.boundary, "decision_effective", decision_id, "policy_executor",
            decision_event_payload(pending->decision), M11EventVisibility::institution);
        if (!event.ok()) {
            return event.status();
        }
    ...
    for (const auto &change : plan.effective_changes) {
        auto *version = staged.mutable_policy_version(change.economy, change.lever);
        if (version == nullptr) {
            return Status(ErrorCode::internal_error,
    ...
    if (!plan.effective_decision_ids.empty()) {
        auto event = staged_events.append(
            plan.boundary, "policy_transaction_effective", "boundary-policy-commit",
            "policy_executor",
            event_payload("decision_count",
                          std::to_string(plan.effective_decision_ids.size())),
...
Status M11PolicyCoordinator::fail_due(const M11DuePlan &plan, std::string_view reason,
                                      M11EventStream &events) {
    ...
    auto failed = plan;
    failed.failed_decision_ids.insert(failed.failed_decision_ids.end(),
    ...
    std::sort(failed.failed_decision_ids.begin(), failed.failed_decision_ids.end());
    failed.failed_decision_ids.erase(std::unique(failed.failed_decision_ids.begin(),
                                                 failed.failed_decision_ids.end()),
                                     failed.failed_decision_ids.end());
    return commit_due(failed, events);
}

Status M11PolicyCoordinator::restore(const simulation::M9World &world,
    ...
    const auto expected_versions = world.economy_count() * m11_policy_levers().size();
    ...
    std::sort(policy_versions.begin(), policy_versions.end(), policy_version_less);
    for (std::size_t economy = 0U; economy < world.economy_count(); ++economy) {
        for (std::size_t lever = 0U; lever < m11_policy_levers().size(); ++lever) {
            const auto index = economy * m11_policy_levers().size() + lever;
            if (policy_versions[index].economy.value() != economy ||
                policy_versions[index].lever != m11_policy_levers()[lever].name) {
                return Status(ErrorCode::corrupt_input,
    ...
    if (std::adjacent_find(
            ...
            [](const M11DecisionContext &left, const M11DecisionContext &right) {
                return left.context_id == right.context_id;
            }) != contexts.end() ||
        std::adjacent_find(
        ...
        std::adjacent_find(budgets.begin(), budgets.end(),
        ...
        return Status(ErrorCode::corrupt_input, "M11 coordinator keys are not unique");
    }
    for (const auto &budget : budgets) {
        if (!m11_valid_seat(budget.seat) || !std::isfinite(budget.remaining) ||
            !std::isfinite(budget.reserved) || !std::isfinite(budget.capacity) ||
            budget.remaining < 0.0 || budget.reserved < 0.0 || budget.capacity < 0.0 ||
            budget.remaining + budget.reserved > budget.capacity + kBudgetTolerance) {
            return Status(ErrorCode::corrupt_input,
    ...
    for (const auto &decision : decisions) {
        if (decision.accepted_sequence >= next_decision_sequence) {
            return Status(ErrorCode::corrupt_input, "M11 decision sequence is corrupt");
        }
    }
