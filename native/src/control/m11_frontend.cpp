...
void append_optional_tick(std::vector<std::uint8_t> &bytes, std::optional<Tick> value) {
...
void append_optional_string(std::vector<std::uint8_t> &bytes,
...
void append_policy_value(std::vector<std::uint8_t> &bytes, const PolicyValue &value) {
    ...
    } else if (const auto *choice = std::get_if<std::string>(&value);
               choice != nullptr) {
        append_string(bytes, *choice);
    } else if (const auto *targets = std::get_if<PolicyEconomySet>(&value);
               targets != nullptr) {
        append_u64(bytes, static_cast<std::uint64_t>(targets->size()));
...
void append_released_observation(std::vector<std::uint8_t> &bytes,
...
void append_policy_version(std::vector<std::uint8_t> &bytes,
...
void append_native_policy_action(std::vector<std::uint8_t> &bytes,
...
void append_decision_context(std::vector<std::uint8_t> &bytes,
    ...
    append_u64(bytes, static_cast<std::uint64_t>(context.observation.size()));
    for (const auto &observation : context.observation) {
        append_released_observation(bytes, observation);
    }
    append_u64(bytes, static_cast<std::uint64_t>(context.permitted_actions.size()));
    ...
    append_u64(bytes, static_cast<std::uint64_t>(context.policy_versions.size()));
...
void append_policy_proposal(std::vector<std::uint8_t> &bytes,
    ...
    append_optional_string(bytes, proposal.supersedes_proposal_id);
}

void append_policy_decision(std::vector<std::uint8_t> &bytes,
                            const M11PolicyDecision &decision) {
    append_string(bytes, decision.decision_id);
    append_string(bytes, decision.proposal_id);
    bytes.push_back(static_cast<std::uint8_t>(decision.status));
...
void append_controller_event(std::vector<std::uint8_t> &bytes,
    ...
    bytes.push_back(static_cast<std::uint8_t>(event.visibility));
    bytes.insert(bytes.end(), event.prior_hash.bytes.begin(),
                 event.prior_hash.bytes.end());
    bytes.insert(bytes.end(), event.hash.bytes.begin(), event.hash.bytes.end());
}

void append_shock_bulletin(std::vector<std::uint8_t> &bytes,
    ...
    if (bulletin.sector.has_value()) {
        bytes.push_back(static_cast<std::uint8_t>(*bulletin.sector));
    }
}

[[nodiscard]] bool valid_scope(const M11ControlledSession &session,
                               const M11AccessScope &scope) noexcept {
    return !scope.principal.empty() && scope.principal.size() <= 128U &&
           scope.economy.value() < session.engine().world().economy_count() &&
           (scope.role == "player" || m11_valid_seat(scope.role));
}

[[nodiscard]] bool
frontend_metric(const reporting::MetricDescriptor &descriptor) noexcept {
...
m11_frontend_snapshot_id(const M11FrontendSnapshot &snapshot) noexcept {
    try {
        ...
        bytes.reserve(256U + snapshot.metrics.size() * 32U +
        ...
        bytes.push_back(static_cast<std::uint8_t>(snapshot.phase));
        ...
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.metrics.size()));
        ...
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.economies.size()));
        for (const auto &economy : snapshot.economies) {
            append_u64(bytes, economy.economy.value());
            append_u64(bytes, static_cast<std::uint64_t>(economy.metrics.size()));
        ...
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.policies.size()));
        ...
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.releases.size()));
        for (const auto &release : snapshot.releases) {
            append_released_observation(bytes, release);
        }
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.contexts.size()));
        for (const auto &context : snapshot.contexts) {
            append_decision_context(bytes, context);
        }
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.pending.size()));
        ...
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.public_events.size()));
        for (const auto &event : snapshot.public_events) {
            append_controller_event(bytes, event);
        }
        append_u64(bytes, static_cast<std::uint64_t>(snapshot.shock_bulletins.size()));
...
M11FrontendProjection::snapshot(const M11ControlledSession &session,
    ...
    result.awaiting_human = session.phase() == M11BoundaryPhase::awaiting_human;
    ...
    for (std::size_t economy_index = 0U; economy_index < frame.economy_count;
         ++economy_index) {
        ...
        std::ranges::sort(economy.metrics, {}, &M11FrontendMetric::stable_id);
    ...
    auto domestic = session.engine().world().domestic_policy(scope.economy);
    if (!domestic.ok()) {
    ...
    const auto &external =
        session.engine()
            .world()
            .external_policies()[static_cast<std::size_t>(scope.economy.value())];
    for (const auto &lever : m11_policy_levers()) {
        if (scope.role != "player" && lever.owner_role != scope.role) {
            continue;
        }
        auto value = m11_policy_value(*domestic.get_if(), external, lever.name);
        const auto *version =
            session.coordinator().find_policy_version(scope.economy, lever.name);
    ...
    std::ranges::sort(result.policies, {}, &M11FrontendPolicy::lever);
    const auto append_observations = [&](std::string_view role) -> Status {
        auto observations = release_service_.observation(
            session.releases(), scope.economy, session.tick(), role);
        if (!observations.ok()) {
            return observations.status();
        }
        result.releases.insert(result.releases.end(),
                               std::make_move_iterator(observations.get_if()->begin()),
                               std::make_move_iterator(observations.get_if()->end()));
    ...
    std::ranges::sort(result.releases, [](const M11ReleasedObservation &left,
                                          const M11ReleasedObservation &right) {
        return std::tie(left.series_id, left.observed_at, left.released_at,
                        left.revision) < std::tie(right.series_id, right.observed_at,
                                                  right.released_at, right.revision);
    });
    result.releases.erase(std::unique(result.releases.begin(), result.releases.end()),
                          result.releases.end());
    for (const auto &context : session.coordinator().contexts()) {
        if (context.economy == scope.economy &&
            (scope.role == "player" || context.seat == scope.role) &&
            context.boundary <= session.tick() &&
            context.expires_at >= session.tick()) {
            result.contexts.push_back(context);
        }
    }
    std::ranges::sort(result.contexts, {}, &M11DecisionContext::context_id);
    for (const auto &pending : session.coordinator().pending()) {
        if (pending.context.economy == scope.economy &&
            (scope.role == "player" || pending.context.seat == scope.role) &&
            pending.decision.status == M11DecisionStatus::accepted_pending) {
            result.pending.push_back(pending);
        }
    }
    std::ranges::sort(result.pending, [](const M11PendingDecision &left,
                                         const M11PendingDecision &right) {
        return left.decision.decision_id < right.decision.decision_id;
    });
    const auto first_event = result.event_cursor > kM11FrontendEventWindow
                                 ? result.event_cursor - kM11FrontendEventWindow
                                 : 0U;
    auto events = session.events().page(first_event, kM11FrontendEventWindow,
    ...
    auto bulletins =
        session.engine().probe_shock_bulletins(scope.economy, session.tick());
    if (!bulletins.ok()) {
...
M11FrontendProjection::delta(const M11FrontendSnapshot &base,
                             const M11FrontendSnapshot &result) const {
    if (base.schema_version != kM11FrontendSnapshotSchemaVersion ||
        result.schema_version != kM11FrontendSnapshotSchemaVersion ||
        base.snapshot_id != m11_frontend_snapshot_id(base) ||
        result.snapshot_id != m11_frontend_snapshot_id(result) ||
        base.scope != result.scope || base.cache_epoch != result.cache_epoch ||
        result.snapshot_sequence != base.snapshot_sequence + 1U ||
        result.boundary < base.boundary || result.event_cursor < base.event_cursor ||
    ...
    for (const auto &metric : result.metrics) {
        const auto previous = std::lower_bound(
            base.metrics.begin(), base.metrics.end(), metric.stable_id,
            [](const M11FrontendMetric &entry, std::string_view stable_id) {
                return entry.stable_id < stable_id;
            });
        if (previous == base.metrics.end() || previous->stable_id != metric.stable_id ||
            *previous != metric) {
            delta.changed_metrics.push_back(metric);
        }
    }
    for (const auto &economy : result.economies) {
        const auto previous = std::ranges::find(base.economies, economy.economy,
                                                &M11FrontendEconomy::economy);
        if (previous == base.economies.end() || *previous != economy) {
            delta.changed_economies.push_back(economy);
        }
    }
    for (const auto &policy : result.policies) {
        const auto previous = std::lower_bound(
            base.policies.begin(), base.policies.end(), policy.lever,
            [](const M11FrontendPolicy &entry, std::string_view lever) {
                return entry.lever < lever;
            });
        if (previous == base.policies.end() || previous->lever != policy.lever ||
            *previous != policy) {
