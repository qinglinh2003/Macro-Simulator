...
[[nodiscard]] bool choice_contains(std::string_view choices,
    ...
    while (start <= choices.size()) {
        ...
        const auto item = choices.substr(start, ending == std::string_view::npos
                                                    ? choices.size() - start
                                                    : ending - start);
        if (item == value) {
            return true;
...
[[nodiscard]] std::optional<double> number_value(const PolicyValue &value) noexcept {
...
#undef MACRO_SIM_M11_SPECIAL
    ...
    if (name == "bank_resolution_fund") {
        return PolicyValue{domestic.financial.bank_resolution_fund};
    }
    ...
    if (name == "fx_regime") {
        return PolicyValue{std::string(
            external.fx_regime == simulation::FxRegime::peg ? "peg" : "float")};
    }
    return Status(ErrorCode::not_found, "M11 policy lever is not mapped");
}

[[nodiscard]] Status write_policy_value(simulation::DomesticPolicyState &domestic,
...
#define MACRO_SIM_M11_EXTERNAL_ECONOMY_ID(label, field)                                \
    if (name == label) {                                                               \
        if (std::holds_alternative<std::monostate>(value)) {                           \
            ...
            external.field =                                                           \
                EconomyId(static_cast<std::uint64_t>(std::get<std::int64_t>(value)));  \
        }                                                                              \
        return Status::success();                                                      \
    }
#define MACRO_SIM_M11_EXTERNAL_ECONOMY_SET(label, field)                               \
...
#undef MACRO_SIM_M11_SPECIAL
    ...
    if (name == "energy_rationing") {
        ...
        if (rationing == "market") {
            domestic.energy.rationing = simulation::EnergyRationing::market;
        } else if (rationing == "proportional") {
            domestic.energy.rationing = simulation::EnergyRationing::proportional;
        } else if (rationing == "household_first") {
            domestic.energy.rationing = simulation::EnergyRationing::household_first;
        } else {
            domestic.energy.rationing = simulation::EnergyRationing::industry_first;
        }
        return Status::success();
    }
    if (name == "fx_regime") {
        const auto &regime = std::get<std::string>(value);
        external.fx_regime = regime == "peg" ? simulation::FxRegime::peg
...
const PolicyLeverDescriptor *find_m11_policy_lever(std::string_view name) noexcept {
    const auto iterator =
        std::lower_bound(kM11PolicyLevers.begin(), kM11PolicyLevers.end(), name,
                         [](const PolicyLeverDescriptor &lever,
                            std::string_view target) { return lever.name < target; });
...
Status validate_m11_policy_value(const PolicyLeverDescriptor &lever,
                                 const PolicyValue &value, EconomyId owner,
    ...
    case PolicyValueKind::nullable_number:
        if (!std::holds_alternative<std::monostate>(value) && !numeric.has_value()) {
            return Status(ErrorCode::invalid_argument,
    ...
    case PolicyValueKind::choice: {
        ...
        if (choice == nullptr || !choice_contains(lever.choices, *choice)) {
            return Status(ErrorCode::invalid_argument,
    ...
    case PolicyValueKind::economy_set: {
        ...
        if (targets == nullptr || !std::is_sorted(targets->begin(), targets->end()) ||
            std::adjacent_find(targets->begin(), targets->end()) != targets->end()) {
            return Status(ErrorCode::invalid_argument,
                          "M11 policy economy set is not canonical");
        }
        for (const auto target : *targets) {
            if (target.value() >= economy_count || target == owner) {
                return Status(ErrorCode::out_of_range,
    ...
    if (numeric.has_value()) {
        ...
        if ((lever.minimum.has_value() && *numeric < *lever.minimum) ||
            (lever.maximum.has_value() && *numeric > *lever.maximum)) {
            return Status(ErrorCode::out_of_range,
...
Result<PolicyValue> m11_policy_value(const simulation::DomesticPolicyState &domestic,
                                     const simulation::ExternalPolicyState &external,
                                     std::string_view name) {
    if (find_m11_policy_lever(name) == nullptr) {
        return Status(ErrorCode::not_found, "M11 policy lever is unknown");
    }
...
project_m11_policy_actions(const simulation::M9World &world,
    ...
    for (const auto &action : ordered) {
        const auto economy = static_cast<std::size_t>(action.economy.value());
        ...
        if (lever == nullptr) {
            return Status(ErrorCode::not_found, "M11 policy action lever is unknown");
        }
        auto status = validate_m11_policy_value(*lever, action.value, action.economy,
                                                world.economy_count());
        if (!status.ok()) {
            return status;
        }
        status = write_policy_value(batch.domestic[economy], batch.external[economy],
    ...
    for (auto &domestic : batch.domestic) {
        domestic.housing.wealth_tax_rate = domestic.fiscal_monetary.wealth_tax_rate;
    ...
    for (std::size_t index = 0; index < batch.external.size(); ++index) {
        ...
        const auto anchor = static_cast<std::size_t>(external.peg_anchor->value());
        if (batch.external[anchor].fx_regime != simulation::FxRegime::floating) {
            return Status(ErrorCode::contract_violation,
...
Result<double> m11_policy_numeric_distance(const PolicyLeverDescriptor &lever,
    ...
    if (!lever.control_scale.has_value() || !before.has_value() || !after.has_value()) {
        return 1.0;
    }
    const auto distance = std::abs(*after - *before) / *lever.control_scale;
    if (!std::isfinite(distance)) {
        return Status(ErrorCode::out_of_range, "M11 policy distance is not finite");
    }
    return distance;
}
