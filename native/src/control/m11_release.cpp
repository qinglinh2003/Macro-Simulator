...
split_roles(std::string_view value) {
...
[[nodiscard]] std::string metric_stable_id(
...
[[nodiscard]] bool already_published(
    const M11ReleaseStream &stream, EconomyId economy,
...
[[nodiscard]] std::optional<double> latest_release(
    const M11ReleaseStream &stream, EconomyId economy,
    std::string_view series_id, Tick as_of) noexcept {
    std::optional<double> result;
    std::uint64_t latest_sequence = 0U;
    bool found = false;
    for (const auto &record : stream.releases()) {
        if (record.economy != economy ||
            record.series_id != series_id ||
...
[[nodiscard]] double maximum_optional(
    std::initializer_list<std::optional<double>> values) noexcept {
    double result = 0.0;
...
std::span<const M11ObservationFieldDescriptor>
        ...
        std::lower_bound(fields.begin(), fields.end(), series_id,
                         [](const M11ObservationFieldDescriptor &field,
                            std::string_view name) {
                             return field.series_id < name;
                         });
    return found == fields.end() || found->series_id != series_id
               ? nullptr
               : &*found;
}

bool m11_release_permitted(
    const M11ObservationFieldDescriptor &field,
...
Result<std::optional<double>> M11ReleaseService::aggregate(
    const reporting::MetricHistoryPage &history,
    ...
    if (endpoint.value() + 1U < field.window_ticks &&
        field.require_full_window) {
    ...
    const auto frame_offset =
        field.source == M11ObservationSource::shock ? 0U : 1U;
    ...
    for (const auto &frame : history.frames) {
        if (frame.tick.value() < first_tick ||
            frame.tick.value() > last_tick) {
            continue;
        }
    ...
    if (field.require_full_window &&
        values.size() != field.window_ticks) {
        return std::optional<double>{};
    }
    if (values.empty()) {
        return std::optional<double>{};
    ...
    if (!std::isfinite(total)) {
        return Status(ErrorCode::out_of_range,
                      "M11 release aggregation overflowed");
    }
    return std::optional<double>{
        field.aggregation == M11ReleaseAggregation::mean
            ? total / static_cast<double>(values.size())
            : total};
}

Result<std::size_t> M11ReleaseService::publish_due(
    const reporting::MetricPipeline &metrics,
    ...
    if (boundary != world.tick() ||
        boundary != metrics.current().tick) {
        return Status(ErrorCode::stale_handle,
                      "M11 release boundary is stale");
    }
    const auto &history_store = metrics.history();
    auto history = history_store.page(
        history_store.oldest_sequence(), history_store.size());
    if (!history.ok()) {
    ...
    for (const auto &field : m11_observation_fields()) {
        ...
        if (field.source == M11ObservationSource::shock) {
            ...
            if (release_index < field.phase_offset_ticks ||
                (release_index - field.phase_offset_ticks) %
        ...
        for (std::size_t economy = 0U;
            ...
            if (already_published(
                    staged, economy_id, field.series_id, endpoint,
                    0U)) {
                continue;
            }
            auto value =
                aggregate(*history.get_if(), field, economy_id,
                          endpoint);
            if (!value.ok()) {
                return value.status();
            }
            auto appended = staged.append(
                economy_id, std::string(field.series_id), endpoint,
                boundary, 0U, *value.get_if(),
...
M11ReleaseService::observation(const M11ReleaseStream &stream,
                               EconomyId economy, Tick as_of,
                               std::string_view role) const {
    if (!economy.valid() || role.empty()) {
        return Status(ErrorCode::invalid_argument,
                      "M11 observation scope is invalid");
    }
    ...
    for (const auto &field : m11_observation_fields()) {
        ...
        for (const auto &record : stream.releases()) {
            if (record.economy != economy ||
                record.series_id != field.series_id ||
                record.released_at > as_of ||
                (latest != nullptr &&
...
std::vector<M11MetricSample> M11ReleaseService::trigger_samples(
    const M11ReleaseStream &stream, EconomyId economy,
    Tick as_of) const {
    const auto reserve = latest_release(
        stream, economy, "reserve_floor_breach_share", as_of);
    const auto failures = latest_release(
        stream, economy, "near_failure_bank_count", as_of);
    const auto energy =
        latest_release(stream, economy, "energy_unfilled", as_of);
    const auto productivity = latest_release(
        stream, economy, "shock_productivity", as_of);
    const auto labor = latest_release(
        stream, economy, "shock_labor_availability", as_of);
    const auto capital = latest_release(
        stream, economy, "shock_capital_destruction", as_of);
    const auto energy_shock = latest_release(
        stream, economy, "shock_energy_capacity", as_of);
    const auto credit = latest_release(
        stream, economy, "shock_credit_supply", as_of);
    const auto import_shock = latest_release(
        stream, economy, "shock_import_capacity", as_of);
    const auto export_shock = latest_release(
        stream, economy, "shock_export_capacity", as_of);
    const auto demand = latest_release(
        stream, economy, "shock_household_demand", as_of);
    return {
        {"reserve_floor_breach_share", reserve},
        {"near_failure_bank_count", failures},
        {"energy_unfilled", energy},
        {"shock_supply_severity",
         maximum_optional({productivity, labor, capital})},
        {"shock_energy_severity",
         maximum_optional({energy_shock})},
        {"shock_financial_severity", maximum_optional({credit})},
        {"shock_trade_severity",
         maximum_optional({import_shock, export_shock})},
