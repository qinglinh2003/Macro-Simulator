...
owner_for_group(std::string_view group) noexcept {
...
split_choices(std::string_view choices) {
...
permitted(const M11DecisionContext &context,
          std::string_view lever) noexcept {
    const auto found = std::lower_bound(
        context.permitted_actions.begin(),
        context.permitted_actions.end(), lever,
        [](const M11PermittedAction &entry, std::string_view name) {
            return entry.lever < name;
        });
    return found == context.permitted_actions.end() ||
                   found->lever != lever
               ? nullptr
               : &*found;
}

[[nodiscard]] const M11ReleasedObservation *
released(const M11DecisionContext &context,
         std::string_view series) noexcept {
    const auto found = std::lower_bound(
        context.observation.begin(), context.observation.end(), series,
        [](const M11ReleasedObservation &entry, std::string_view name) {
            return entry.series_id < name;
        });
    return found == context.observation.end() ||
...
released_value(const M11DecisionContext &context,
               std::string_view series) noexcept {
...
pending_for(const M11PolicyCoordinator &coordinator, EconomyId economy,
    ...
    for (const auto &entry : coordinator.pending()) {
        if (entry.decision.status !=
            M11DecisionStatus::accepted_pending) {
            continue;
        }
        const auto action = std::find_if(
            entry.proposal.actions.begin(), entry.proposal.actions.end(),
            [&](const NativePolicyAction &value) {
                return value.economy == economy &&
                       value.lever == lever;
            });
...
pending_action(const M11PendingDecision &pending_value,
               EconomyId economy, std::string_view lever) noexcept {
    const auto found = std::find_if(
        pending_value.proposal.actions.begin(),
        pending_value.proposal.actions.end(),
        [&](const NativePolicyAction &action) {
            return action.economy == economy &&
                   action.lever == lever;
        });
    return found == pending_value.proposal.actions.end() ? nullptr
                                                         : &*found;
}

[[nodiscard]] double
encode_policy_value(const PolicyLeverDescriptor &lever,
    ...
    if (const auto *choice = std::get_if<std::string>(&value);
        ...
        const auto found =
            std::find(choices.begin(), choices.end(), *choice);
        return found == choices.end()
                   ? -1.0
                   : static_cast<double>(
                         std::distance(choices.begin(), found));
    }
    if (const auto *targets = std::get_if<PolicyEconomySet>(&value);
        ...
        for (const auto target : *targets) {
            if (target.value() < economy_count &&
                target.value() < 52U) {
                encoded += std::ldexp(
                    1.0, static_cast<int>(target.value()));
            }
        }
        return encoded;
    }
...
directional_value(const PolicyLeverDescriptor &lever,
                  const PolicyValue &current, std::uint32_t code,
                  EconomyId owner, std::size_t economy_count) {
    if (code == 1U) {
        return current;
    }
    if (code > 2U) {
    ...
    if (lever.kind == PolicyValueKind::economy_set) {
        ...
        if (const auto *existing =
                std::get_if<PolicyEconomySet>(&current);
            existing != nullptr) {
            targets = *existing;
        }
        if (!upward) {
    ...
    const auto step =
        lever.control_scale.value_or(lever.maximum_step.value_or(0.0));
    if (!(step > 0.0)) {
        return current;
    }
    const auto nullable =
        lever.kind == PolicyValueKind::nullable_number;
    ...
    if (const auto *number = std::get_if<double>(&current);
        ...
        base = *number;
                   ...
                   std::get_if<std::int64_t>(&current);
               integer != nullptr) {
    ...
    if (nullable && !upward && lever.minimum.has_value() &&
        value < *lever.minimum) {
        return PolicyValue(std::monostate{});
    }
    ...
    if (lever.kind == PolicyValueKind::integer) {
        return PolicyValue(static_cast<std::int64_t>(
            std::llround(value)));
    }
...
encode_artifact_context(const NativePolicyArtifact &artifact,
    ...
    for (const auto &feature : artifact.feature_names()) {
        ...
        if (feature == "elapsed_ticks") {
            values.push_back(
                static_cast<double>(context.elapsed_ticks));
            continue;
        }
        if (feature.starts_with("release:")) {
            const auto suffix = feature.rfind(':');
            const auto series =
                feature.substr(8U, suffix - 8U);
            const auto *entry = released(context, series);
            if (feature.ends_with(":missing")) {
                values.push_back(entry == nullptr ||
                ...
                auto value =
                    entry == nullptr
                        ...
                        : entry->value;
                const auto *field =
                    find_m11_observation_field(series);
        ...
        if (feature ==
            ...
            values.push_back(
                context.emergency_trigger.empty() ? 0.0 : 1.0);
            continue;
        }
        ...
        const auto *version =
            coordinator.find_policy_version(
                context.economy, lever_name);
        const auto *pending =
            pending_for(coordinator, context.economy, lever_name);
        const auto *target =
            pending == nullptr
                ? nullptr
                : pending_action(*pending, context.economy,
                                 lever_name);
        if (field_name == "value") {
            values.push_back(
            ...
            values.push_back(
                lever == nullptr || target == nullptr
                    ? 0.0
                    : encode_policy_value(
                          *lever, target->value, economy_count));
        } else if (field_name ==
                   "pending_target_is_none") {
            values.push_back(
                target != nullptr &&
                        std::holds_alternative<std::monostate>(
                            target->value)
            ...
            values.push_back(
                version == nullptr ||
                   ...
                   "next_eligibility_delta") {
            values.push_back(
                item == nullptr ||
                        item->earliest_effective <= context.boundary
                    ? 0.0
                    : static_cast<double>(
                          item->earliest_effective.value() -
                          context.boundary.value()));
        } else if (field_name ==
                   "pending_effective_delta") {
            values.push_back(
                pending == nullptr ||
                        !pending->decision.effective_at.has_value() ||
                        *pending->decision.effective_at <=
                            context.boundary
                    ? 0.0
                    : static_cast<double>(
                          pending->decision.effective_at->value() -
                          context.boundary.value()));
        } else {
            values.push_back(0.0);
        }
...
[[nodiscard]] CanonicalControllerEnvelope make_envelope(
    Tick boundary, std::uint64_t policy_generation,
    ...
    envelope.decision_versions.reserve(
        state.coordinator.policy_versions().size());
    envelope.effective_versions.reserve(
        state.coordinator.policy_versions().size());
    for (const auto &version :
         state.coordinator.policy_versions()) {
...
[[nodiscard]] bool has_unanswered_human(
    const M11ControlledState &state) noexcept {
    for (const auto &context_id : state.opened_context_ids) {
        const auto *context =
            state.coordinator.find_context(context_id);
        if (context == nullptr ||
        ...
        const auto found = std::find_if(
            ...
            [&](const M11SeatRuntime &seat) {
                return seat.assignment.economy ==
                           context->economy &&
                       seat.assignment.seat == context->seat;
            });
        if (found != state.seats.end() &&
            found->assignment.occupant.kind ==
                M11OccupantKind::human_queue) {
            return true;
        }
    }
    return false;
...
[[nodiscard]] std::uint64_t previous_context_elapsed(
    const M11PolicyCoordinator &coordinator, EconomyId economy,
    ...
    for (const auto &context : coordinator.contexts()) {
        if (context.economy == economy &&
            context.seat == seat &&
            context.decision_group == group &&
            context.boundary < boundary &&
            (!previous.has_value() ||
             context.boundary > *previous)) {
...
[[nodiscard]] bool valid_occupant_spec(
    const M11OccupantSpec &spec) noexcept {
    return !spec.occupant_id.empty() &&
           spec.occupant_id.size() <= 128U &&
...
M11ControlledSession::create(EngineSession engine,
                             M11ControllerRunSpec run_spec) {
    if (run_spec.worker_count == 0U ||
    ...
    auto coordinator =
        M11PolicyCoordinator::create(engine.world(),
                                     run_spec.cost_spec);
    if (!scheduler.ok()) {
    ...
    std::vector<M11SeatAssignment> assignments =
        run_spec.assignments;
    for (const auto &assignment : assignments) {
        if (assignment.economy.value() >=
                engine.world().economy_count() ||
            !m11_valid_seat(assignment.seat) ||
            !valid_occupant_spec(assignment.occupant)) {
            return invalid_session(
                "M11 seat assignment is invalid");
        }
    }
    std::sort(
        assignments.begin(), assignments.end(),
        [](const M11SeatAssignment &left,
           const M11SeatAssignment &right) {
    ...
    for (auto &assignment : assignments) {
        ...
        if (assignment.occupant.kind ==
            ...
            auto artifact = NativePolicyArtifact::load_file(
            ...
            for (const auto &lever_name :
                 runtime.assignment.occupant.action_dimensions) {
                const auto *lever =
                    find_m11_policy_lever(lever_name);
                if (lever == nullptr ||
                    lever->owner_role != assignment.seat) {
                    return Status(
                        ErrorCode::contract_violation,
    ...
    auto initial =
        make_envelope(engine.tick(), engine.policy_generation(), state);
    auto sealed = seal_controller_envelope(initial);
    if (!sealed.ok()) {
        return sealed;
    }
    auto bridge = HybridControlledBridge::create(
        std::move(engine), std::move(initial));
    if (!bridge.ok()) {
        return bridge.status();
    }
    return M11ControlledSession(
        std::move(*bridge.get_if()), std::move(run_spec),
        std::move(state));
}
...
M11ControlledSession::seat(EconomyId economy,
    ...
    const auto found = std::find_if(
        state_.seats.begin(), state_.seats.end(),
...
Status M11ControlledSession::synchronize_state(
    std::string operation_id, M11ControlledState next) {
    auto envelope = make_envelope(
        bridge_.engine().tick(),
    ...
    return bridge_.acknowledge_receipt(
        receipt.get_if()->operation_id);
}

Result<M11PolicyProposal>
M11ControlledSession::automatic_proposal(
    M11SeatRuntime &seat_runtime,
    const M11DecisionContext &context) const {
    const auto counter = seat_runtime.decision_counter++;
    const auto suffix =
        std::to_string(context.boundary.value()) + ":" +
        std::to_string(counter);
    M11PolicyProposal proposal;
    proposal.proposal_id =
        "proposal:" + seat_runtime.assignment.occupant.occupant_id +
    ...
    proposal.based_on_policy_versions =
        proposal_versions(context);
    proposal.reason = std::string(m11_occupant_kind_name(
        seat_runtime.assignment.occupant.kind));
    ...
    if (occupant.kind == M11OccupantKind::heuristic) {
        const auto *target =
            permitted(context, "gov_deficit_target");
        const auto unemployment =
            released_value(context, "unemployment_rate");
        const auto inflation =
            released_value(context, "inflation");
        if (target != nullptr && target->allowed &&
            unemployment.has_value() && inflation.has_value()) {
            std::uint32_t direction = 1U;
            if (*unemployment > 0.10 && *inflation < 0.01) {
                direction = 2U;
            } else if (*unemployment < 0.06 ||
                       *inflation > 0.02) {
                direction = 0U;
            }
            const auto *lever =
                find_m11_policy_lever("gov_deficit_target");
            auto value = directional_value(
                *lever, target->current_value, direction,
                context.economy,
                bridge_.engine().world().economy_count());
            if (!value.ok()) {
                return value.status();
            }
            if (!m11_policy_values_equal(
                    *value.get_if(), target->current_value)) {
    ...
    if (occupant.kind == M11OccupantKind::random_fuzz) {
        auto entropy = mix64(
            occupant.seed ^ context.boundary.value() ^
            mix64(counter) ^
            stable_string_hash(context.context_id));
        if (unit_random(entropy) >
        ...
        const auto *entry =
            choices[entropy % choices.size()];
        const auto direction =
            (mix64(entropy) & 1U) == 0U ? 0U : 2U;
        const auto *lever =
            find_m11_policy_lever(entry->lever);
        auto value = directional_value(
            *lever, entry->current_value, direction,
            context.economy,
            bridge_.engine().world().economy_count());
        if (!value.ok()) {
            return value.status();
        ...
        if (!m11_policy_values_equal(
                *value.get_if(), entry->current_value)) {
            proposal.actions.push_back(
                {context.economy, entry->lever,
                 std::move(*value.get_if())});
        }
        return proposal;
    }
    if (occupant.kind ==
        ...
        const auto features = encode_artifact_context(
            artifact, state_.coordinator, context,
            bridge_.engine().world().economy_count());
        std::vector<std::uint8_t> mask(
            artifact.info().action_dimension * 3U, 0U);
        for (std::size_t index = 0U;
             index < occupant.action_dimensions.size(); ++index) {
            mask[index * 3U + 1U] = 1U;
            const auto *entry =
                permitted(context,
                          occupant.action_dimensions[index]);
...
M11ControlledSession::run_boundary_prelude() {
    ...
    auto published = release_service_.publish_due(
        bridge_.engine().metrics(), bridge_.engine().world(),
        boundary, next.events.next_sequence(), next.releases);
    if (!published.ok()) {
    ...
    auto boundary_event = next.events.append(
        ...
        "scheduler",
        "released=" + std::to_string(*published.get_if()),
    ...
    for (std::size_t economy = 0U;
         economy < bridge_.engine().world().economy_count();
         ++economy) {
        const auto samples = release_service_.trigger_samples(
            next.releases, EconomyId(economy), boundary);
        auto evaluated = next.scheduler.evaluate_triggers(
            boundary, EconomyId(economy), samples);
        if (!evaluated.ok()) {
            return evaluated.status();
        }
        notices.insert(
            notices.end(), evaluated.get_if()->begin(),
    ...
    for (std::size_t economy = 0U;
        ...
        for (const auto &calendar : next.scheduler.calendars()) {
            if (!next.scheduler.due(
                    calendar.decision_group, boundary,
                    EconomyId(economy))) {
                continue;
            }
            const auto owner =
                owner_for_group(calendar.decision_group);
            if (owner.empty() ||
                seat(EconomyId(economy), owner) == nullptr) {
                continue;
            }
    ...
    std::sort(
        context_specs.begin(), context_specs.end(),
        [](const ContextSpec &left, const ContextSpec &right) {
            return std::tuple{
                       left.economy.value(), left.seat, left.group,
                       left.emergency, left.trigger} <
                   std::tuple{
                       right.economy.value(), right.seat, right.group,
    ...
    for (const auto &spec : context_specs) {
        auto observation = release_service_.observation(
            next.releases, spec.economy, boundary, spec.seat);
        if (!observation.ok()) {
            return observation.status();
        }
        const auto elapsed = previous_context_elapsed(
            next.coordinator, spec.economy, spec.seat,
            spec.group, boundary);
        auto opened = next.coordinator.open_context(
            bridge_.engine().world(), next.scheduler, boundary,
            spec.economy, spec.seat, spec.group, spec.expiry,
            spec.emergency, spec.trigger, elapsed,
            *observation.get_if());
        if (!opened.ok()) {
            return opened.status();
        }
        next.opened_context_ids.push_back(
            opened.get_if()->context_id);
        result.opened_context_ids.push_back(
            opened.get_if()->context_id);
        auto event = next.events.append(
            boundary, "decision_context_opened",
            "context-open:" + opened.get_if()->context_id,
            "scheduler",
            "group=" + spec.group +
                "|emergency=" +
                (spec.emergency ? "1" : "0"),
        ...
        if (spec.emergency) {
            auto emergency = next.events.append(
                boundary, "emergency_trigger",
                "trigger:" + opened.get_if()->context_id,
                "scheduler",
                "trigger=" + spec.trigger +
                    "|value=" +
                    std::to_string(spec.trigger_value),
    ...
    std::sort(next.opened_context_ids.begin(),
              next.opened_context_ids.end());
    std::sort(result.opened_context_ids.begin(),
              result.opened_context_ids.end());

    for (const auto &context_id : next.opened_context_ids) {
        const auto *context =
            next.coordinator.find_context(context_id);
        if (context == nullptr) {
            return Status(ErrorCode::internal_error,
                          "M11 opened context is missing");
        }
        ...
        if (runtime == next.seats.end()) {
            return Status(ErrorCode::internal_error,
                          "M11 context seat is unassigned");
        }
        if (runtime->assignment.occupant.kind ==
            M11OccupantKind::human_queue) {
            continue;
        }
        auto proposal =
            automatic_proposal(*runtime, *context);
        if (!proposal.ok()) {
            return proposal.status();
        }
        auto decision = next.coordinator.submit(
            bridge_.engine().world(), next.scheduler, boundary,
            std::move(*proposal.get_if()),
            runtime->assignment.occupant.occupant_id,
            next.events);
        if (!decision.ok()) {
    ...
    next.phase = has_unanswered_human(next)
                     ? M11BoundaryPhase::awaiting_human
                     : M11BoundaryPhase::ready_to_commit;
    result.phase = next.phase;
    result.awaiting_human =
        next.phase == M11BoundaryPhase::awaiting_human;
    auto synchronized = synchronize_state(
        "m11-prelude:" + std::to_string(boundary.value()) +
            ":" + std::to_string(next.boundary_sequence),
        std::move(next));
    if (!synchronized.ok()) {
        return synchronized;
...
Result<M11DecisionResult>
M11ControlledSession::commit_ready_boundary() {
    ...
    const auto boundary = bridge_.engine().tick();
    auto next = state_;
    auto plan = next.coordinator.prepare_due(
        bridge_.engine().world(), boundary);
    if (!plan.ok()) {
        return plan.status();
    }
    auto committed_plan =
        next.coordinator.commit_due(*plan.get_if(), next.events);
    ...
    auto event = next.events.append(
        ...
        "effective=" +
            std::to_string(
                plan.get_if()->effective_decision_ids.size()) +
            "|failed=" +
            std::to_string(
                plan.get_if()->failed_decision_ids.size()),
        M11EventVisibility::institution);
    if (!event.ok()) {
        return event.status();
    }
    ...
    batch.operation_id =
        ...
        std::to_string(next.boundary_sequence);
    batch.expected_controller_hash =
        bridge_.controller_envelope().hash;
    ...
    auto envelope = make_envelope(
        lease.get_if()->preview().next_tick,
        lease.get_if()->preview().policy_generation, next);
    auto sealed = seal_controller_envelope(envelope);
    if (!sealed.ok()) {
        (void)bridge_.abort_boundary(
            std::move(*lease.get_if()));
        return sealed;
    }
    auto advanced = bridge_.commit_boundary(
        std::move(*lease.get_if()), std::move(envelope));
    if (!advanced.ok()) {
...
Result<M11DecisionResult>
M11ControlledSession::advance_until_decision(
    const M11AdvanceLimit &limit) {
    if (limit.maximum_ticks == 0U) {
        return invalid_session(
            "M11 advance limit must be positive");
    }
    ...
    if (state_.phase == M11BoundaryPhase::awaiting_human) {
        ...
        aggregate.opened_context_ids =
            state_.opened_context_ids;
        return aggregate;
    }
    ...
    while (aggregate.elapsed_ticks < limit.maximum_ticks) {
        if (state_.phase == M11BoundaryPhase::boundary_start) {
            ...
            boundary_had_context =
                !prelude.get_if()->opened_context_ids.empty();
            aggregate.opened_context_ids =
                prelude.get_if()->opened_context_ids;
            aggregate.decisions.insert(
                aggregate.decisions.end(),
        ...
        aggregate.phase = state_.phase;
        aggregate.boundary = bridge_.engine().tick();
        if (limit.stop_after_context_boundary &&
            boundary_had_context) {
            return aggregate;
        }
...
M11ControlledSession::submit_policy_proposal(
    ...
    auto next = state_;
    auto decision = next.coordinator.submit(
        bridge_.engine().world(), next.scheduler,
        bridge_.engine().tick(), std::move(proposal), actor,
        next.events);
    if (!decision.ok()) {
        return decision.status();
    }
    next.phase = has_unanswered_human(next)
                     ? M11BoundaryPhase::awaiting_human
                     : M11BoundaryPhase::ready_to_commit;
    auto status = synchronize_state(
        "m11-proposal:" + decision.get_if()->decision_id,
        std::move(next));
    if (!status.ok()) {
        return status;
    }
...
Result<M11PolicyDecision>
M11ControlledSession::submit_human_proposal(
    M11PolicyProposal proposal, std::string_view actor) {
    const auto *context =
        state_.coordinator.find_context(proposal.context_id);
    if (state_.phase != M11BoundaryPhase::awaiting_human ||
        context == nullptr) {
        return Status(
            ErrorCode::invalid_transaction_state,
    ...
    if (runtime == nullptr ||
        runtime->assignment.occupant.kind !=
            M11OccupantKind::human_queue) {
        return Status(ErrorCode::invalid_transaction_state,
...
Result<M11PolicyDecision>
M11ControlledSession::timeout_context(
    std::string_view context_id, std::string_view operation_id,
    std::string_view actor) {
    const auto *context =
        state_.coordinator.find_context(context_id);
    if (context == nullptr) {
        return Status(ErrorCode::not_found,
                      "M11 context was not found");
    }
    M11PolicyProposal proposal;
    proposal.proposal_id =
        "timeout:" + std::string(operation_id);
...
Result<M11PolicyDecision>
M11ControlledSession::cancel_pending(
    std::string_view decision_id, std::string_view operation_id,
    std::string_view actor) {
    auto next = state_;
    auto decision = next.coordinator.cancel(
        bridge_.engine().tick(), decision_id, operation_id, actor,
        next.events);
    if (!decision.ok()) {
        return decision.status();
    }
    auto status = synchronize_state(
        "m11-cancel:" + std::string(operation_id),
        std::move(next));
    if (!status.ok()) {
        return status;
    }
...
Status M11ControlledSession::assign_seat(
    ...
    replacement.assignment = {
        request.economy, request.seat, request.occupant};
    if (request.occupant.kind ==
        M11OccupantKind::reinforcement_learning) {
        auto artifact = NativePolicyArtifact::load_file(
            request.occupant.artifact_path);
        if (!artifact.ok()) {
            return artifact.status();
        }
...
Result<M11ControlledSession> M11ControlledSession::clone() const {
    ...
    return M11ControlledSession(
        std::move(*bridge.get_if()), run_spec_, state_);
}
