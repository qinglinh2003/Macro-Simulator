...
inline constexpr std::size_t kMaximumBootstrapBytes =
    64U * 1024U;
...
class Socket final {
  public:
    explicit Socket(SocketHandle handle = kInvalidSocket) noexcept
        : handle_(handle) {}
...
read_small_file(const std::filesystem::path &path) {
    ...
    if (::stat(path.c_str(), &native_status) != 0 ||
        (native_status.st_mode &
         static_cast<mode_t>(0077)) != 0) {
        return std::nullopt;
    }
#endif
    const auto size =
...
load_bootstrap(const std::filesystem::path &path) {
    ...
    try {
        ...
        if (!document.is_object() ||
            ...
            !document.contains("save_root") ||
            !document.contains("ready_file") ||
            !document.at("token").is_string() ||
        ...
        Bootstrap result;
        result.token =
            document.at("token").get<std::string>();
        result.save_root =
            document.at("save_root").get<std::string>();
        result.ready_file =
            document.at("ready_file").get<std::string>();
        if (document.contains("rl_artifact")) {
            if (!document.at("rl_artifact").is_string()) {
                return std::nullopt;
            }
            result.rl_artifact =
                document.at("rl_artifact")
                    .get<std::string>();
        }
        if (!macro_sim::desktop::valid_m11_capability_token(
                result.token) ||
            !result.save_root.is_absolute() ||
            !result.ready_file.is_absolute() ||
            (!result.rl_artifact.empty() &&
             !result.rl_artifact.is_absolute())) {
            return std::nullopt;
        }
        return result;
    } catch (...) {
...
[[nodiscard]] bool write_ready_file(
...
[[nodiscard]] bool send_all(
    SocketHandle socket, std::string_view bytes) {
...
#endif
        const auto sent = ::send(
            socket, bytes.data() + offset, count,
            send_flags);
        if (sent < 0) {
            if (errno == EINTR) {
                continue;
...
[[nodiscard]] bool serve_connection(
    SocketHandle socket, M11ProtocolWorker &worker) {
    std::string buffer;
...
#else
        const auto received = ::recv(
            socket, chunk.data(), chunk.size(), 0);
        if (received < 0) {
            if (errno == EINTR) {
                continue;
            }
...
[[nodiscard]] std::optional<std::pair<Socket, std::uint16_t>>
listen_loopback() {
#if defined(_WIN32)
    WSADATA data{};
    ...
    static_cast<void>(setsockopt(
        listener.get(), SOL_SOCKET, SO_REUSEADDR,
    ...
    return std::pair(
        std::move(listener), ntohs(address.sin_port));
}
...
[[nodiscard]] int run(
    ...
    if (!write_ready_file(
            bootstrap->ready_file, listener->second)) {
...
#endif
    while (!worker.get_if()->shutdown_requested() &&
           stop_requested == 0) {
        sockaddr_in peer{};
        ...
        static_cast<void>(
            serve_connection(
                connection.get(), *worker.get_if()));
    }
    std::error_code cleanup_error;
    std::filesystem::remove(
        bootstrap->ready_file, cleanup_error);
#if defined(_WIN32)
    WSACleanup();
#endif
    return 0;
...
int main(int argc, char **argv) {
    if (argc != 3 ||
        std::string_view(argv[1]) != "--bootstrap") {
        std::cerr
            << "Usage: macro_sim_server --bootstrap <absolute-path>\n";
        return 1;
    }
