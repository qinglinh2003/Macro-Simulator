...
[[nodiscard]] BankId bank_for_node(const M5TickScratch &scratch,
                                   SettlementNodeId node) noexcept {
    const auto index = node_index(node);
    return index < scratch.bank_by_node_.size() ? scratch.bank_by_node_[index]
                                                : BankId{};
}
...
[[nodiscard]] const core::BankComponent *bank_component(const core::RootState &state,
                                                        BankId bank) noexcept {
...
[[nodiscard]] Status transfer(const core::RootState &state, M4TickScratch &scratch,
    ...
    if (source_index >= scratch.balances_.size() ||
        ...
        return Status(ErrorCode::internal_error, "M5 posting projection is stale");
    }
...
void refresh_aggregates(const core::RootState &state, const M4TickScratch &real,
                        M5TickScratch &scratch) {
    std::fill(scratch.debt_by_account_.begin(), scratch.debt_by_account_.end(), 0.0);
    std::fill(scratch.exposure_by_bank_.begin(), scratch.exposure_by_bank_.end(), 0.0);
    ...
    double household_interest_arrears = 0.0;
    for (const auto &loan : scratch.loans_) {
        if (loan.borrower.kind() == core::OwnerKind::household) {
            household_interest_arrears += std::max(0.0, loan.interest_arrears);
        }
    }
    scratch.working_metrics_.household_interest_arrears_closing =
        household_interest_arrears;
    scratch.working_metrics_.household_interest_arrears_stock_flow_residual =
        scratch.working_metrics_.household_interest_arrears_opening +
        scratch.working_metrics_.household_interest_accrued -
        scratch.working_metrics_.household_interest_arrears_cash_paid -
        scratch.working_metrics_.household_interest_arrears_extinguished -
        household_interest_arrears;
    std::fill(scratch.deposits_by_bank_.begin(), scratch.deposits_by_bank_.end(), 0.0);
    state.households.for_each_alive(
        [&real, &scratch](HouseholdId, const core::HouseholdComponent &item) {
            ...
            const auto bank = bank_for_node(scratch, real.account_nodes_[account]);
...
[[nodiscard]] BankId relationship_bank(const core::RootState &state,
    ...
    const bool cache_available =
        account_slot < scratch.relationship_loan_by_account_.size();
    if (cache_available) {
        const auto indexed = scratch.relationship_loan_by_account_[account_slot];
        ...
        scratch.relationship_loan_by_account_[account_slot] = LoanId{};
    }
    for (const auto &loan : scratch.loans_) {
        if (loan.active && loan.borrower_account == account &&
            loan.principal.value() > algorithms::kEconomicEpsilon) {
            if (cache_available) {
                scratch.relationship_loan_by_account_[account_slot] = loan.id;
            }
            return loan.lender;
        }
    }
    if (account_slot >= real.account_nodes_.size()) {
        return BankId{};
    }
    const auto bank = bank_for_node(scratch, real.account_nodes_[account_slot]);
...
void reserve_household_debt_service(const core::RootState &state, M4TickScratch &real,
                                    ...
                                    M5TickScratch &scratch) noexcept {
    if (!runtime.rules.direct_monetary_transmission &&
        !runtime.rules.household_interest_arrears) {
        return;
    }
    ...
    for (const auto &loan : scratch.loans_) {
        if (!loan.active || (loan.principal.value() <= algorithms::kEconomicEpsilon &&
                             loan.interest_arrears <= algorithms::kEconomicEpsilon)) {
            continue;
        }
        ...
        const double carried =
            runtime.rules.household_interest_arrears ? loan.interest_arrears : 0.0;
        scratch.scheduled_service_by_account_[slot] +=
            ...
            carried;
        scratch.working_metrics_.household_contractual_debt_service_due +=
            (runtime.rules.household_amortization +
             loan_service_rate(state, runtime, loan)) *
                loan.principal.value() +
            carried;
        scratch.working_metrics_.household_interest_arrears_in_goods_reservation +=
            carried;
    }
...
[[nodiscard]] Status migrate_account(const core::RootState &state, M4TickScratch &real,
                                     ...
                                     BankId destination,
                                     bool move_net_deposit) noexcept {
    const auto *bank = state.banks.get(destination);
    const auto index = account_index(account);
    if (bank == nullptr || index >= real.account_nodes_.size()) {
        return Status(ErrorCode::not_found, "M5 account migration target is absent");
    }
    ...
    if (move_net_deposit) {
        const double debt = index < scratch.debt_by_account_.size()
                                ...
                                : 0.0;
        const double reserves = std::max(0.0, real.balances_[index] - debt);
        const auto status =
            move_reserves(real, old_node, bank->settlement_node, reserves);
        if (!status.ok()) {
            return status;
        }
    }
...
[[nodiscard]] BankId choose_bank(const core::RootState &state, const M5Runtime &runtime,
                                 const M4TickScratch &real, M5TickScratch &scratch,
    ...
    if (!runtime.rules.rate_competition || state.banks.alive_count() <= 1) {
        return incumbent;
    }
    ...
    if (incumbent_component != nullptr &&
        bank_capacity(state, runtime, scratch, incumbent) + kTolerance >= requested) {
        best_rate =
            std::max(0.0, runtime.policy_rate + incumbent_component->loan_spread);
    }
    ...
    for (std::size_t offset = 0; offset < limit; ++offset) {
        const auto raw =
            scratch
                .candidate_banks_[(start + offset) % scratch.candidate_banks_.size()];
        ...
        const double rate = std::max(0.0, runtime.policy_rate + component->loan_spread);
        if (rate < best_rate - algorithms::kEconomicEpsilon ||
            (std::abs(rate - best_rate) <= algorithms::kEconomicEpsilon &&
             candidate < best)) {
            best = candidate;
            best_rate = rate;
        }
...
[[nodiscard]] Status set_policy_rate(M5Runtime &runtime) noexcept {
    ...
    case MonetaryRegime::manual:
        runtime.policy_rate =
            std::clamp(*policy.manual_policy_rate, 0.0, policy.maximum_policy_rate);
        break;
    case MonetaryRegime::taylor: {
        const double target =
            policy.neutral_rate +
            policy.taylor_inflation *
                (runtime.inflation_sensor - policy.inflation_target) -
            policy.taylor_unemployment *
                (runtime.previous_unemployment - policy.natural_unemployment);
        runtime.policy_rate = std::clamp(policy.rate_inertia * runtime.policy_rate +
                                             (1.0 - policy.rate_inertia) * target,
                                         0.0, policy.maximum_policy_rate);
        break;
    }
    }
    return Status::success();
...
void apply_fiscal_policy(M4Runtime &real, const M5Runtime &runtime) noexcept {
    real.rules.government_consumption_share =
        runtime.policy.government_consumption_share;
    real.rules.government_deficit_target = runtime.policy.government_deficit_target;
    ...
    real.rules.benefit_income_floor = runtime.policy.benefit_income_floor;
    real.rules.minimum_wage = runtime.policy.minimum_wage;
    real.rules.job_guarantee = runtime.policy.job_guarantee;
    real.rules.job_guarantee_wage_ratio = runtime.policy.job_guarantee_wage_ratio;
    real.rules.job_guarantee_public_works_share =
        runtime.policy.job_guarantee_public_works_share;
}

void open_financial_books(const core::RootState &state, const M4TickScratch &real,
                          M5Runtime &runtime, M5TickScratch &scratch, Tick tick) {
    scratch.loans_ = state.loans.records();
    scratch.interbank_ = state.interbank.records();
    scratch.central_bank_operations_ = state.central_bank_operations.records();
    ...
    scratch.working_metrics_.inflation_sensor = runtime.inflation_sensor;
    std::fill(scratch.reusable_loan_by_account_.begin(),
              scratch.reusable_loan_by_account_.end(), LoanId{});
    std::fill(scratch.relationship_loan_by_account_.begin(),
              scratch.relationship_loan_by_account_.end(), LoanId{});
    for (const auto &loan : scratch.loans_) {
        const auto slot = account_index(loan.borrower_account);
        if (loan.active && loan.purpose == core::LoanPurpose::general &&
            loan.borrower.kind() == core::OwnerKind::firm) {
            scratch.working_metrics_.firm_credit_existing_principal +=
                loan.principal.value();
        }
        if (loan.active && loan.principal.value() > algorithms::kEconomicEpsilon &&
            slot < scratch.relationship_loan_by_account_.size() &&
            !scratch.relationship_loan_by_account_[slot].valid()) {
            scratch.relationship_loan_by_account_[slot] = loan.id;
        }
        if (loan.active && loan.purpose == core::LoanPurpose::general &&
            slot < scratch.reusable_loan_by_account_.size()) {
            scratch.reusable_loan_by_account_[slot] = loan.id;
        }
    }
...
[[nodiscard]] Status run_deposit_competition(const core::RootState &state,
                                             const M5Runtime &runtime,
                                             M4TickScratch &real,
                                             M5TickScratch &scratch) {
    if (!runtime.rules.interbank || runtime.rules.deposit_spread_dispersion <= 0.0 ||
        state.banks.alive_count() <= 1) {
        return Status::success();
    }
    refresh_aggregates(state, real, scratch);
    double total = std::accumulate(scratch.deposits_by_bank_.begin(),
                                   scratch.deposits_by_bank_.end(), 0.0);
    total = std::max(1.0, total);
    state.households.for_each_alive([&state, &runtime, &real, &scratch,
                                     total](HouseholdId,
                                            const core::HouseholdComponent &household) {
        const auto account = account_index(household.primary_account);
        if (scratch.debt_by_account_[account] > algorithms::kEconomicEpsilon) {
            return;
        }
        const auto current = bank_for_node(scratch, real.account_nodes_[account]);
        BankId best = current;
        const auto effective = [&state, &runtime, &scratch, total](BankId bank) {
            const auto *component = state.banks.get(bank);
            const auto index = bank_index(bank);
            const double congestion = runtime.rules.deposit_spread_dispersion *
        ...
        state.banks.for_each_alive(
            [&best, &best_rate, &visited, &runtime,
             &effective](BankId candidate, const core::BankComponent &bank) {
                if (!bank.alive || visited >= runtime.rules.deposit_search_count) {
                    return;
                }
                ++visited;
        ...
        if (best != current) {
            static_cast<void>(migrate_account(state, real, scratch,
                                              household.primary_account, best, true));
        }
    });
    refresh_aggregates(state, real, scratch);
    return Status::success();
...
[[nodiscard]] Status run_omo(const core::RootState &state, const M5Runtime &runtime,
                             M4TickScratch &real, M5TickScratch &scratch, Tick tick) {
    if (!runtime.policy.open_market_operations || !runtime.rules.interbank) {
        return Status::success();
    }
    double target = runtime.policy.reserve_target * runtime.reserve_genesis;
    if (runtime.policy.reserve_target_indexes_deposits) {
        ...
        for (const auto &account : state.postings.records()) {
            if (account.key.kind == core::AccountKind::deposit) {
                deposits += std::max(0.0, real.balances_[account_index(account.id)]);
            }
        }
        target = runtime.policy.reserve_target * runtime.policy.reserve_floor_fraction *
                 deposits;
    }
    double current = 0.0;
    state.banks.for_each_alive(
        [&real, &scratch, &current](BankId id, const core::BankComponent &bank) {
            if (bank.alive && scratch.bank_alive_[bank_index(id)] != 0U) {
                current += real.reserve_balances_[node_index(bank.settlement_node)];
            }
        });
    const double move = runtime.policy.reserve_gap_close * (current - target);
    if (move > algorithms::kEconomicEpsilon) {
        double positive = 0.0;
        state.banks.for_each_alive([&real, &positive](BankId,
                                                      const core::BankComponent &bank) {
            positive +=
                std::max(0.0, real.reserve_balances_[node_index(bank.settlement_node)]);
        });
        ...
        double drained = 0.0;
        state.banks.for_each_alive(
            [&real, &scratch, &runtime, tick, move, positive,
             &drained](BankId id, const core::BankComponent &bank) {
                const auto node = node_index(bank.settlement_node);
                const double available = std::max(0.0, real.reserve_balances_[node]);
                const double amount = std::min(available, move * available / positive);
                real.reserve_balances_[node] -= amount;
                scratch.reserve_stock_ -= amount;
                add_cb_operation(scratch,
                                 core::CentralBankOperationKind::reserve_absorption, id,
                                 amount, runtime.policy_rate, tick);
                drained += amount;
            });
        ...
        for (const auto &operation : scratch.central_bank_operations_) {
            if (operation.active &&
                operation.kind == core::CentralBankOperationKind::reserve_absorption) {
        ...
        for (auto &operation : scratch.central_bank_operations_) {
            ...
            const auto *bank = state.banks.get(operation.counterparty);
            if (bank == nullptr ||
                scratch.bank_alive_[bank_index(operation.counterparty)] == 0U) {
                continue;
            }
            const double amount = std::min(remaining, operation.principal.value());
            real.reserve_balances_[node_index(bank->settlement_node)] += amount;
            scratch.reserve_stock_ += amount;
            operation.principal = Money(operation.principal.value() - amount);
            operation.active =
                operation.principal.value() > algorithms::kEconomicEpsilon;
            remaining -= amount;
    ...
    if (runtime.rules.household_credit) {
        for (std::size_t index = 0; index < real.household_ids_.size(); ++index) {
            ...
            const double requested =
                std::max(0.0, real.household_work_[index].consumption_budget -
...
[[nodiscard]] Status service_debt(const core::RootState &state,
                                  const M5Runtime &runtime, M4TickScratch &real,
                                  M5TickScratch &scratch, bool full_pnl) {
    for (auto &loan : scratch.loans_) {
        if (!loan.active || (loan.principal.value() <= algorithms::kEconomicEpsilon &&
                             loan.interest_arrears <= algorithms::kEconomicEpsilon)) {
            continue;
        }
        const auto *account_record = state.postings.get(loan.borrower_account);
        ...
        const bool household =
            account_record->key.owner.kind() == core::OwnerKind::household;
        const double accrued_interest =
            loan_service_rate(state, runtime, loan) * loan.principal.value();
        const double opening_interest_arrears =
            household && runtime.rules.household_interest_arrears
                ? loan.interest_arrears
                : 0.0;
        const double interest_due = accrued_interest + opening_interest_arrears;
        if (household && runtime.rules.household_interest_arrears) {
            scratch.working_metrics_.household_interest_arrears_opening +=
                opening_interest_arrears;
            scratch.working_metrics_.household_interest_accrued += accrued_interest;
        }
        double interest = 0.0;
        double principal = 0.0;
        if (full_pnl) {
            interest = std::min(interest_due, std::max(0.0, real.balances_[account]));
            principal = std::min(amortization * loan.principal.value(),
                                 std::max(0.0, real.balances_[account] - interest));
        } else {
            principal = std::min(amortization * loan.principal.value(),
                                 std::max(0.0, real.balances_[account]));
            interest = std::min(interest_due,
                                std::max(0.0, real.balances_[account] - principal));
        }
        if (interest > algorithms::kEconomicEpsilon) {
            const auto status = transfer(state, real, loan.borrower_account,
                                         lender->cash_account, interest);
            if (!status.ok()) {
                return status;
            }
            ...
            if (household) {
                ...
                if (runtime.rules.household_interest_arrears) {
                    scratch.working_metrics_.household_interest_arrears_cash_paid +=
                        interest;
                }
            } else if (full_pnl) {
        ...
        if (household && runtime.rules.household_interest_arrears) {
            loan.interest_arrears = std::max(0.0, interest_due - interest);
        }
        if (principal > algorithms::kEconomicEpsilon) {
            real.balances_[account] -= principal;
            loan.principal = Money(loan.principal.value() - principal);
            if (loan.principal.value() <= algorithms::kEconomicEpsilon) {
                loan.principal = Money(0.0);
                loan.active = false;
                const auto slot = account_index(loan.borrower_account);
                if (loan.purpose == core::LoanPurpose::general &&
                    slot < scratch.reusable_loan_by_account_.size() &&
                    scratch.reusable_loan_by_account_[slot] == loan.id) {
                    scratch.reusable_loan_by_account_[slot] = LoanId{};
                }
            }
...
[[nodiscard]] core::InterbankRecord *
find_interbank(M5TickScratch &scratch, BankId lender, BankId borrower) noexcept {
    for (auto &record : scratch.interbank_) {
        if (record.active && record.lender == lender && record.borrower == borrower) {
            return &record;
        }
    }
    return nullptr;
...
void add_interbank(M5TickScratch &scratch, BankId lender, BankId borrower,
                   double principal, double rate, Tick tick) {
    if (auto *record = find_interbank(scratch, lender, borrower); record != nullptr) {
        const double old = record->principal.value();
        const double total = old + principal;
        record->rate =
            Rate(total > 0.0 ? (old * record->rate.value() + principal * rate) / total
                             : 0.0);
...
[[nodiscard]] Status default_interbank_borrower(const core::RootState &state,
                                                M4TickScratch &real,
                                                M5TickScratch &scratch,
                                                BankId borrower) {
...
[[nodiscard]] Status run_interbank(const core::RootState &state, M5Runtime &runtime,
    ...
    for (auto &record : scratch.interbank_) {
        ...
        const double due = record.accrued_interest.value() +
                           record.principal.value() * record.rate.value();
        const double interest = std::min({
            due,
            std::max(0.0, real.balances_[account_index(borrower->cash_account)]),
            std::max(0.0, real.reserve_balances_[borrower_node]),
        });
        if (interest > algorithms::kEconomicEpsilon) {
            const auto status = transfer(state, real, borrower->cash_account,
                                         lender->cash_account, interest);
            if (!status.ok()) {
                return status;
            }
            pnl_for(scratch, record.borrower)->interbank_interest_expense += interest;
            pnl_for(scratch, record.lender)->interbank_interest_income += interest;
        }
        const double principal =
            std::min(record.principal.value(),
                     std::max(0.0, real.reserve_balances_[borrower_node]));
        if (principal > algorithms::kEconomicEpsilon) {
            const auto status = move_reserves(real, borrower->settlement_node,
                                              lender->settlement_node, principal);
            if (!status.ok()) {
                return status;
            }
        }
        record.principal = Money(record.principal.value() - principal);
        record.accrued_interest = Money(std::max(0.0, due - interest));
        record.active = record.principal.value() > algorithms::kEconomicEpsilon ||
                        record.accrued_interest.value() > algorithms::kEconomicEpsilon;
        record.maturity_tick = Tick(tick.value() + 1);
    }
    double deficit = 0.0;
    double surplus = 0.0;
    state.banks.for_each_alive([&real, &scratch, &deficit,
                                &surplus](BankId id, const core::BankComponent &bank) {
        if (!bank.alive || scratch.bank_alive_[bank_index(id)] == 0U) {
            return;
        }
        const double reserve = real.reserve_balances_[node_index(bank.settlement_node)];
...
[[nodiscard]] double bank_health(const core::RootState &state, const M5Runtime &runtime,
                                 const M5TickScratch &scratch, BankId bank) noexcept {
    const auto index = bank_index(bank);
    ...
    const double book_health = std::min(
        1.0, (capital / exposure) / std::max(algorithms::kEconomicEpsilon,
                                             runtime.rules.run_health_reference));
    const auto market_index = bank_index(bank);
    const double market_health =
        market_index < runtime.bank_market_health.size()
            ? std::clamp(runtime.bank_market_health[market_index], 0.0, 1.0)
            : 1.0;
    return runtime.rules.run_market_weight * market_health +
           (1.0 - runtime.rules.run_market_weight) * book_health;
}

void add_lolr(M5Runtime &runtime, M5TickScratch &scratch, M4TickScratch &real,
              const core::BankComponent &bank, BankId bank_id, double amount,
              Tick tick) {
    real.reserve_balances_[node_index(bank.settlement_node)] += amount;
    scratch.reserve_stock_ += amount;
...
void extinguish_household_interest_arrears(M5TickScratch &scratch,
                                           core::LoanRecord &loan) noexcept {
    if (loan.borrower.kind() != core::OwnerKind::household ||
        loan.interest_arrears <= algorithms::kEconomicEpsilon) {
        return;
    }
...
[[nodiscard]] Status resolve_bank(const core::RootState &state, M5Runtime &runtime,
    ...
    auto status = default_interbank_borrower(state, real, scratch, failed);
    if (!status.ok()) {
        return status;
    }
    scratch.alive_banks_.clear();
    state.banks.for_each_alive([&scratch](BankId id, const core::BankComponent &) {
    ...
    for (auto &record : scratch.interbank_) {
        ...
        const auto receiver_it = std::find_if(
            scratch.alive_banks_.begin(), scratch.alive_banks_.end(),
            [&record](BankId candidate) { return candidate != record.borrower; });
        ...
        const double principal = record.principal.value();
        real.balances_[account_index(failed_bank->cash_account)] -= principal;
        real.balances_[account_index(receiver_bank->cash_account)] += principal;
    ...
    if (runtime.policy.migrate_relationships_on_failure &&
        ...
        for (auto &account : state.postings.records()) {
            ...
            status = migrate_account(state, real, scratch, account.id, target, true);
            if (!status.ok()) {
                return status;
            }
        }
    ...
    if (real.balances_[failed_account] > algorithms::kEconomicEpsilon) {
        ...
        if (!scratch.alive_banks_.empty()) {
            ...
            receiver_account = state.banks.get(receiver_bank)->cash_account;
        }
        const double amount = real.balances_[failed_account];
        status =
            transfer(state, real, failed_bank->cash_account, receiver_account, amount);
        if (!status.ok()) {
            return status;
        }
    ...
    if (hole > algorithms::kEconomicEpsilon && allow_state_support &&
        ...
        status = transfer(state, real, state.institutions.treasury_account,
                          failed_bank->cash_account, hole);
        if (!status.ok()) {
            return status;
        }
        pnl_for(scratch, failed)->resolution_flow += hole;
        scratch.working_metrics_.resolution_cost += hole;
    } else if (hole > algorithms::kEconomicEpsilon && !scratch.alive_banks_.empty()) {
        double available = 0.0;
        for (const auto bank : scratch.alive_banks_) {
            const auto *component = state.banks.get(bank);
            available += std::max(
                0.0, real.reserve_balances_[node_index(component->settlement_node)]);
        }
        for (const auto bank : scratch.alive_banks_) {
            ...
            const double reserve = std::max(
                0.0, real.reserve_balances_[node_index(component->settlement_node)]);
            const double amount = std::min(hole, hole * reserve / available);
            status = transfer(state, real, component->cash_account,
                              failed_bank->cash_account, amount);
            if (!status.ok()) {
                return status;
            }
...
[[nodiscard]] Status force_default(const core::RootState &state, M4TickScratch &real,
    ...
    for (auto &loan : scratch.loans_) {
        ...
        extinguish_household_interest_arrears(scratch, loan);
...
[[nodiscard]] Status run_bank_runs(const core::RootState &state, M5Runtime &runtime,
    ...
    BankId safe{};
    double safest = -1.0;
    state.banks.for_each_alive([&state, &runtime, &scratch, &safe,
                                &safest](BankId id, const core::BankComponent &bank) {
        if (!bank.alive || scratch.bank_alive_[bank_index(id)] == 0U) {
            return;
        }
        const double health = bank_health(state, runtime, scratch, id);
        if (health > safest ||
            (std::abs(health - safest) <= algorithms::kEconomicEpsilon && id < safe)) {
            safe = id;
            safest = health;
        }
    });
    ...
    state.banks.for_each_alive([&state, &runtime, &real, &scratch, tick, &rng, forced,
        ...
        const double intensity =
            forced == id ? 1.0
                         : std::min(1.0, runtime.rules.run_sensitivity * pressure +
                                             runtime.bank_fear);
        ...
        for (std::size_t index = 0; index < real.household_ids_.size(); ++index) {
            const auto *household = state.households.get(real.household_ids_[index]);
            const auto account = account_index(household->primary_account);
            if (real.account_nodes_[account] == bank.settlement_node &&
                scratch.debt_by_account_[account] <= algorithms::kEconomicEpsilon &&
                rng.uniform_closed_open() < intensity) {
                scratch.household_order_.push_back(index);
            }
        }
        static_cast<void>(rng.shuffle(std::span(scratch.household_order_)));
        ...
        for (const auto index : scratch.household_order_) {
            ...
            if (reserves + kTolerance >= withdrawal) {
                static_cast<void>(migrate_account(
                    state, real, scratch, household->primary_account, safe, true));
                scratch.working_metrics_.run_flight_volume += withdrawal;
            } else {
                suspended = true;
                break;
            }
        ...
        if (suspended) {
            ...
            runtime.bank_fear = std::min(1.0, runtime.bank_fear + 0.25);
        } else if (!scratch.household_order_.empty()) {
            runtime.bank_fear = std::min(
                1.0, runtime.bank_fear +
                         0.02 * static_cast<double>(scratch.household_order_.size()) /
                             static_cast<double>(
                                 std::max<std::size_t>(1, real.household_ids_.size())));
        }
    });
...
[[nodiscard]] Status resolve_insolvent_banks(const core::RootState &state,
    ...
    while (changed) {
        ...
        scratch.failed_banks_.clear();
        state.banks.for_each_alive(
            [&real, &scratch](BankId id, const core::BankComponent &bank) {
                if (scratch.bank_alive_[bank_index(id)] != 0U &&
                    real.balances_[account_index(bank.cash_account)] < -kTolerance) {
                    scratch.failed_banks_.push_back(id);
                }
            });
        for (const auto bank : scratch.failed_banks_) {
...
[[nodiscard]] Status distribute_bank_payout(const core::RootState &state,
                                            const M5Runtime &runtime,
                                            M4TickScratch &real, M5TickScratch &scratch,
                                            core::BankPnlRecord &pnl, double payable) {
    ...
    for (const auto household_id : real.household_ids_) {
        ...
        if (!runtime.rules.interest_by_deposits ||
            bank_for_node(scratch, real.account_nodes_[account]) == pnl.bank) {
    ...
    for (const auto household_id : real.household_ids_) {
        ...
        const auto status = transfer(state, real, bank->cash_account,
                                     household->primary_account, amount);
        if (!status.ok()) {
            return status;
        }
...
[[nodiscard]] Status close_legacy_bank_payout(const core::RootState &state,
                                              const M5Runtime &runtime,
                                              M4TickScratch &real,
                                              M5TickScratch &scratch) {
    ...
    for (auto &pnl : scratch.bank_pnl_) {
        ...
        double payable =
            std::min(runtime.rules.bank_payout_ratio * std::max(0.0, pnl.loan_interest),
                     real.balances_[cash]);
        if (runtime.policy.bank_target_capital_ratio > 0.0) {
            const double target = runtime.policy.bank_target_capital_ratio *
                                  scratch.exposure_by_bank_[bank_index(pnl.bank)];
            payable = std::max(0.0, real.balances_[cash] - target);
        }
        const auto status =
            distribute_bank_payout(state, runtime, real, scratch, pnl, payable);
        if (!status.ok()) {
            return status;
        }
    }
...
[[nodiscard]] Status pay_deposit_cost_and_close_pnl(const core::RootState &state,
                                                    const M5Runtime &runtime,
                                                    M4TickScratch &real,
                                                    M5TickScratch &scratch, Tick tick) {
    const double rate =
        runtime.rules.realized_bank_pnl
            ? std::max(runtime.rules.deposit_rate, runtime.policy.deposit_rate_floor)
            : 0.0;
    if (runtime.rules.realized_bank_pnl &&
        ...
        for (std::size_t index = 0; index < real.household_ids_.size(); ++index) {
            ...
            const auto account = account_index(household->primary_account);
            const auto bank = bank_for_node(scratch, real.account_nodes_[account]);
            if (!bank.valid() || scratch.bank_alive_[bank_index(bank)] == 0U) {
                continue;
            }
            const auto *bank_component_value = state.banks.get(bank);
            const auto cash = account_index(bank_component_value->cash_account);
            const double due = rate * std::max(0.0, real.balances_[account]);
            const double paid = std::min(due, std::max(0.0, real.balances_[cash]));
            if (paid > algorithms::kEconomicEpsilon) {
                const auto status =
                    transfer(state, real, bank_component_value->cash_account,
                             household->primary_account, paid);
                if (!status.ok()) {
                    return status;
                }
        ...
        if (runtime.rules.deposit_interest_arrears) {
            for (auto &capital : scratch.bank_capital_) {
                if (!capital.alive ||
                    capital.deposit_interest_arrears <= algorithms::kEconomicEpsilon) {
                    continue;
                }
                const auto *bank = state.banks.get(capital.bank);
                const auto cash = account_index(bank->cash_account);
                const double budget = std::min(capital.deposit_interest_arrears,
                ...
                for (const auto household_id : real.household_ids_) {
                    const auto *household = state.households.get(household_id);
                    const auto account = account_index(household->primary_account);
                    if (bank_for_node(scratch, real.account_nodes_[account]) ==
                        capital.bank) {
                        deposits += std::max(0.0, real.balances_[account]);
                    }
                }
                ...
                for (const auto household_id : real.household_ids_) {
                    ...
                    const double amount =
                        budget * std::max(0.0, real.balances_[account]) / deposits;
                    const auto status = transfer(state, real, bank->cash_account,
                                                 household->primary_account, amount);
                    if (!status.ok()) {
                        return status;
                    }
                    pnl_for(scratch, capital.bank)->deposit_funding_cost += amount;
                    scratch.working_metrics_.deposit_interest_paid += amount;
                    paid_total += amount;
                }
                capital.deposit_interest_arrears =
                    std::max(0.0, capital.deposit_interest_arrears - paid_total);
            }
        }
    }
    for (auto &pnl : scratch.bank_pnl_) {
        ...
        if (runtime.rules.realized_bank_pnl &&
            ...
            real.balances_[cash] > algorithms::kEconomicEpsilon) {
            double payable = std::min(runtime.rules.bank_payout_ratio * pnl.net_income,
                                      real.balances_[cash]);
            if (runtime.policy.bank_target_capital_ratio > 0.0) {
                const double target = runtime.policy.bank_target_capital_ratio *
                                      scratch.exposure_by_bank_[bank_index(pnl.bank)];
                payable = std::min(pnl.net_income,
        ...
        capital->closing_capital = real.balances_[account_index(bank->cash_account)];
        capital->last_closed_tick = tick;
        capital->alive = scratch.bank_alive_[bank_index(pnl.bank)] != 0U;
        scratch.bank_capital_live_[bank_index(pnl.bank)] = capital->closing_capital;
    }
    return Status::success();
}
...
[[nodiscard]] Status validate_projection(const core::RootState &state,
                                         const M4TickScratch &real,
                                         const M5TickScratch &scratch) noexcept {
    if (!all_finite(real.balances_) || !all_finite(real.reserve_balances_) ||
        !finite(scratch.reserve_stock_) || scratch.reserve_stock_ < -kTolerance) {
        return Status(ErrorCode::invariant_violation,
                      "M5 financial projection is not finite");
    }
    double deposits = 0.0;
    for (const auto &account : state.postings.records()) {
        const double balance = real.balances_[account_index(account.id)];
    ...
    for (std::size_t index = 0; index < scratch.loans_.size(); ++index) {
        ...
        if (loan.id.value() != index + 1 || !finite(loan.principal.value()) ||
            loan.principal.value() < -kTolerance || !finite(loan.interest_arrears) ||
            loan.interest_arrears < -kTolerance || !valid_loan_purpose(loan.purpose) ||
    ...
    const double scale =
        std::max({1.0, std::abs(deposits), std::abs(state.genesis_money.value())});
    if (std::abs(a5 - state.genesis_money.value()) > 1.0e-9 * scale) {
        return Status(ErrorCode::invariant_violation,
                      "invariant.m5.net-financial-worth");
    }
    const double reserves = std::accumulate(real.reserve_balances_.begin(),
                                            real.reserve_balances_.end(), 0.0);
    if (std::abs(reserves - scratch.reserve_stock_) >
        1.0e-9 *
            std::max({1.0, std::abs(reserves), std::abs(scratch.reserve_stock_)})) {
        return Status(ErrorCode::invariant_violation,
    ...
    for (std::size_t index = 0; index < scratch.interbank_.size(); ++index) {
        ...
        if (record.id.value() != index + 1 || record.lender == record.borrower ||
            ...
            return Status(ErrorCode::invariant_violation, "invariant.m5.interbank");
        }
    }
    for (const auto &operation : scratch.central_bank_operations_) {
    ...
    for (const auto &pnl : scratch.bank_pnl_) {
        const double expected = pnl.loan_interest + pnl.interbank_interest_income -
                                pnl.interbank_interest_expense -
                                pnl.deposit_funding_cost - pnl.realized_loan_losses -
        ...
        if (!finite(pnl.net_income) ||
            ...
            return Status(ErrorCode::invariant_violation, "invariant.m5.bank-pnl");
        }
        ...
        const double roll_forward = capital->opening_capital + pnl.net_income +
                                    pnl.resolution_flow - pnl.distributions;
        if (std::abs(capital->closing_capital - roll_forward) >
            1.0e-8 * std::max({1.0, std::abs(roll_forward),
                               std::abs(capital->closing_capital)})) {
            return Status(ErrorCode::invariant_violation,
...
class M5Extension final : public M4TickExtension {
  public:
    M5Extension(M5Runtime &runtime, M5TickScratch &scratch,
                const M5AdvanceOptions &options, M5TickExtension *extension) noexcept
    ...
    Status prepare_tick(const core::RootState &state, M4Runtime &real_runtime,
        ...
        tick_active_ = true;
        scratch_.synchronize_topology(state, real);
        ...
        status = run_omo(state, runtime_, real, scratch_, tick);
        if (!status.ok() || extension_ == nullptr) {
            return status;
        }
        return extension_->prepare_tick(state, real_runtime, real, runtime_, scratch_,
                                        tick, rng);
    }
    ...
    Status after_planning(const core::RootState &state, M4Runtime &real_runtime,
        ...
        return extension_->after_planning(state, real_runtime, real, runtime_, scratch_,
                                          tick, rng);
    }
    ...
    Status before_settlement(const core::RootState &state, M4Runtime &real_runtime,
        ...
        if (runtime_.rules.full_firm_pnl) {
            status = service_debt(state, runtime_, real, scratch_, true);
            if (status.ok()) {
                status = close_legacy_bank_payout(state, runtime_, real, scratch_);
            }
        }
        if (!status.ok() || extension_ == nullptr) {
            return status;
    ...
    Status after_settlement(const core::RootState &state, M4Runtime &real_runtime,
        ...
        auto status = Status::success();
        if (!runtime_.rules.full_firm_pnl) {
            status = service_debt(state, runtime_, real, scratch_, false);
            if (status.ok()) {
                status = close_legacy_bank_payout(state, runtime_, real, scratch_);
            }
        }
        ...
        return extension_->after_settlement(state, real_runtime, real, runtime_,
                                            scratch_, tick, rng);
    }

    Status close_institutions(const core::RootState &state, M4Runtime &real_runtime,
                              M4TickScratch &real, Tick tick, PhiloxRng &rng) override {
        auto status = run_interbank(state, runtime_, real, scratch_, tick);
        if (!status.ok()) {
            return status;
        }
        status =
            run_bank_runs(state, runtime_, real, scratch_, tick, rng,
                          options_.force_run_bank, government_enabled(real_runtime));
        if (!status.ok()) {
            return status;
        }
        if (options_.force_default_account.has_value()) {
            status =
                force_default(state, real, scratch_, *options_.force_default_account);
            if (!status.ok()) {
                return status;
            }
        }
        if (extension_ != nullptr) {
            status = extension_->before_bank_resolution(state, real_runtime, real,
        ...
        status = pay_deposit_cost_and_close_pnl(state, runtime_, real, scratch_, tick);
        if (!status.ok()) {
            return status;
        }
        if (extension_ != nullptr) {
            status = extension_->close_institutions(state, real_runtime, real, runtime_,
                                                    scratch_, tick, rng);
            if (!status.ok()) {
                return status;
            }
    ...
    Status validate(const core::RootState &state, const M4Runtime &real_runtime,
                    const M4TickScratch &real, Tick tick) const override {
        const auto status = validate_projection(state, real, scratch_);
        if (!status.ok() || extension_ == nullptr) {
            return status;
        }
        return extension_->validate(state, real_runtime, real, runtime_, scratch_,
                                    tick);
    }

    void commit(core::RootState &state, M4Runtime &real_runtime, M4TickScratch &real,
                Tick closed_tick, const M4Metrics &metrics) noexcept override {
        for (auto &account : state.postings.records()) {
        ...
        state.reserves.commit_projected_stock(Money(scratch_.reserve_stock_));
        state.banks.for_each_alive([this](BankId id, core::BankComponent &bank) {
        ...
        if (runtime_.previous_price_index <= algorithms::kEconomicEpsilon) {
        ...
        if (extension_ != nullptr) {
            extension_->commit(state, real_runtime, real, runtime_, scratch_,
                               closed_tick, runtime_.last_metrics);
        }
        tick_active_ = false;
    }
...
double stage_m5_firm_plan_credit(const core::RootState &state,
    ...
    if (request > algorithms::kEconomicEpsilon) {
        ++scratch.working_metrics_.firm_credit_applications;
        scratch.working_metrics_.firm_credit_requested += request;
        if (runtime.rules.direct_monetary_transmission) {
            scratch.working_metrics_.firm_credit_min_dscr_applied =
    ...
    scratch.working_metrics_.firm_credit_originated += granted;
...
void M5TickScratch::reserve(const core::RootState &state) {
    ...
    interbank_.reserve(state.interbank.size() +
                       state.banks.alive_count() * state.banks.alive_count());
    central_bank_operations_.reserve(state.central_bank_operations.size() +
    ...
    firm_index_by_id_.assign(
        static_cast<std::size_t>(state.firms.allocator_state().next_id), kNoIndex);
    std::size_t dense = 0;
    state.firms.for_each_alive([this, &dense](FirmId id, const core::FirmComponent &) {
...
void M5TickScratch::synchronize_topology(const core::RootState &state,
    ...
    bank_capital_live_.resize(next_bank_id);
    bank_alive_.resize(next_bank_id);
    bank_by_node_.assign(state.reserves.size() + 1, BankId{});
    for (const auto &reserve : state.reserves.records()) {
        bank_by_node_[node_index(reserve.node)] = reserve.bank;
    }
    firm_index_by_id_.assign(
        static_cast<std::size_t>(state.firms.allocator_state().next_id), kNoIndex);
    for (std::size_t dense = 0; dense < real_economy_scratch.firm_ids_.size();
         ++dense) {
        const auto id = real_economy_scratch.firm_ids_[dense];
        if (id.value() < firm_index_by_id_.size()) {
            firm_index_by_id_[id.value()] = dense;
        }
    }
...
std::uint64_t M5TickScratch::capacity_signature() const noexcept {
    ...
    const std::array capacities{
        ...
        reusable_loan_by_account_.capacity(),
...
Result<M5CreditQuote> quote_m5_credit(const core::RootState &state,
                                      ...
                                      double credit_supply_multiplier) {
    if (!runtime.rules.banking_enabled) {
        return Status(ErrorCode::insufficient_funds, "M5 credit banking is disabled");
    }
...
Result<LoanId> stage_m5_credit(const core::RootState &state,
                               ...
                               Tick tick, core::LoanPurpose purpose) {
    if (!runtime.rules.banking_enabled) {
        return Status(ErrorCode::insufficient_funds, "M5 credit banking is disabled");
    }
    ...
    LoanId id{};
    if (purpose == core::LoanPurpose::general &&
        quote.expected_existing_debt > algorithms::kEconomicEpsilon &&
        account_slot < scratch.reusable_loan_by_account_.size()) {
        const auto candidate = scratch.reusable_loan_by_account_[account_slot];
        if (candidate.valid() && candidate.value() <= scratch.loans_.size()) {
            auto &loan =
                scratch.loans_[static_cast<std::size_t>(candidate.value() - 1U)];
            if (loan.id == candidate && loan.active &&
                loan.purpose == core::LoanPurpose::general &&
    ...
    if (!id.valid()) {
        ...
        id = LoanId(static_cast<std::uint64_t>(scratch.loans_.size()) + 1U);
        ...
        if (purpose == core::LoanPurpose::general &&
            account_slot < scratch.reusable_loan_by_account_.size()) {
            scratch.reusable_loan_by_account_[account_slot] = id;
        }
        if (account_slot < scratch.relationship_loan_by_account_.size() &&
            !scratch.relationship_loan_by_account_[account_slot].valid()) {
            scratch.relationship_loan_by_account_[account_slot] = id;
        }
    }
    if (quote.expected_existing_debt <= algorithms::kEconomicEpsilon) {
        ...
        if (!status.ok()) {
            ...
            if (purpose == core::LoanPurpose::general &&
                account_slot < scratch.reusable_loan_by_account_.size() &&
                scratch.reusable_loan_by_account_[account_slot] == id) {
                scratch.reusable_loan_by_account_[account_slot] = LoanId{};
            }
            if (account_slot < scratch.relationship_loan_by_account_.size() &&
                scratch.relationship_loan_by_account_[account_slot] == id) {
                scratch.relationship_loan_by_account_[account_slot] = LoanId{};
...
Status stage_m5_loan_repayment(const core::RootState &state,
    ...
    auto &loan = scratch.loans_[static_cast<std::size_t>(loan_id.value() - 1U)];
    if (loan.id != loan_id || !loan.active ||
        loan.principal.value() <= algorithms::kEconomicEpsilon ||
        loan.borrower_account != payer) {
        return Status(ErrorCode::contract_violation,
                      "M5 loan repayment does not match an active loan");
    }
    const auto account_slot = account_index(payer);
    const auto lender_slot = bank_index(loan.lender);
    const auto *account = state.postings.get(payer);
    ...
    if (real_economy.balances_[account_slot] + kTolerance < principal) {
        return Status(ErrorCode::insufficient_funds,
                      "M5 loan repayment exceeds available funds");
    }
    real_economy.balances_[account_slot] -= principal;
    loan.principal = Money(std::max(0.0, loan.principal.value() - principal));
    loan.active = loan.principal.value() > algorithms::kEconomicEpsilon ||
                  loan.interest_arrears > algorithms::kEconomicEpsilon;
    if (!loan.active && loan.purpose == core::LoanPurpose::general &&
        account_slot < scratch.reusable_loan_by_account_.size() &&
        scratch.reusable_loan_by_account_[account_slot] == loan.id) {
        scratch.reusable_loan_by_account_[account_slot] = LoanId{};
    }
    scratch.debt_by_account_[account_slot] =
        std::max(0.0, scratch.debt_by_account_[account_slot] - principal);
    scratch.exposure_by_bank_[lender_slot] =
        std::max(0.0, scratch.exposure_by_bank_[lender_slot] - principal);
    scratch.working_metrics_.principal_repaid += principal;
    return Status::success();
}
...
Status validate_m5_policy(const M5PolicyState &policy) noexcept {
    ...
    if (!all_finite(values) ||
        ...
        policy.income_tax_rate > 1.0 || policy.income_allowance < 0.0 ||
        policy.consumption_tax_rate < 0.0 || policy.consumption_tax_rate > 1.0 ||
        ...
        policy.benefit_income_floor < 0.0 || policy.minimum_wage < 0.0 ||
        ...
        policy.firm_leverage_limit < 0.0 || policy.firm_minimum_dscr < 0.0 ||
        policy.household_credit_limit < 0.0 || policy.bank_leverage_cap < 0.0 ||
        policy.bank_exposure_limit < 0.0 || policy.bank_exposure_limit > 1.0 ||
        policy.bank_target_capital_ratio < 0.0 || policy.deposit_rate_floor < 0.0) {
        return Status(ErrorCode::invalid_argument,
                      "M5 policy contains an invalid value");
    }
    const bool manual = policy.monetary_regime == MonetaryRegime::manual;
    if (manual != policy.manual_policy_rate.has_value() ||
        (manual &&
         (!finite(*policy.manual_policy_rate) || *policy.manual_policy_rate < 0.0))) {
        return Status(ErrorCode::invalid_argument,
...
Status validate_m5_spec(const M5SimulationSpec &spec) noexcept {
    ...
    if (spec.real_economy.vertical != M4Vertical::capital_fiscal ||
        ...
        spec.rules.interbank_rate_base < 0.0 || spec.rules.interbank_tightness < 0.0 ||
        spec.rules.deposit_spread_dispersion < 0.0 || spec.rules.deposit_rate < 0.0 ||
        spec.rules.firm_amortization < 0.0 || spec.rules.firm_amortization > 1.0 ||
        ...
        spec.rules.run_health_reference <= 0.0 || spec.rules.run_market_weight < 0.0 ||
        spec.rules.run_market_weight > 1.0 || spec.rules.run_fear_persistence < 0.0 ||
        spec.rules.run_fear_persistence > 1.0 || spec.rules.bank_payout_ratio < 0.0 ||
        spec.rules.bank_payout_ratio > 1.0 || spec.initial_policy_rate < 0.0) {
        return Status(ErrorCode::invalid_argument,
                      "M5 simulation specification is invalid");
    }
    auto base = spec.real_economy;
    base.settlement_banks = spec.rules.bank_count;
    base.rules.initial_bank_capital = spec.rules.opening_capital_per_bank;
    return validate_spec(base);
}

Status validate_m5_state(const core::RootState &state,
                         const M4Runtime &real_economy_runtime,
    ...
    if (!validate_m5_policy(runtime.policy).ok() ||
        ...
        return Status(ErrorCode::invariant_violation, "M5 persistent state is invalid");
    }
    auto base_runtime = real_economy_runtime;
    base_runtime.rules.initial_bank_capital = runtime.rules.opening_capital_per_bank;
    return validate_m4_state(state, base_runtime, tick);
}

Result<M5Initialization> build_m5_genesis(const M5SimulationSpec &spec) {
    ...
    const double denominator =
        static_cast<double>(std::max<std::uint64_t>(1, spec.rules.bank_count - 1));
    value.root.banks.for_each_alive([&spec, &value, &pnl, &capital, &bank_ids,
                                     denominator](BankId id,
                                                  core::BankComponent &bank) {
        bank_ids.push_back(id);
        const double centered =
            spec.rules.bank_count <= 1
                ? 0.0
                : 2.0 * static_cast<double>(id.value() - 1) / denominator - 1.0;
        bank.leverage_appetite =
            std::max(1.0, spec.rules.bank_leverage_mean *
                              (1.0 + spec.rules.bank_leverage_dispersion * centered));
        bank.loan_spread = spec.rules.loan_spread_dispersion * centered;
        bank.deposit_spread = spec.rules.deposit_spread_dispersion * centered;
        bank.alive = true;
        const double opening =
            value.root.postings.get(bank.cash_account)->balance.value();
    ...
    borrowers.reserve(value.root.firms.alive_count() +
    ...
    value.root.households.for_each_alive(
        [&borrowers](HouseholdId, const core::HouseholdComponent &household) {
    ...
    if (spec.rules.assign_banks_by_size) {
        ...
        for (std::size_t index = 0; index < borrowers.size(); ++index) {
            ...
            const auto *bank = value.root.banks.get(bank_ids[index % bank_ids.size()]);
            account->key.settlement_node = bank->settlement_node;
        }
    } else {
        PhiloxRng assignment_rng(
            {static_cast<std::uint32_t>(spec.real_economy.seed) ^ 0x42414e4bU,
             static_cast<std::uint32_t>(spec.real_economy.seed >> 32U) ^ 0x4153534eU});
        for (const auto borrower : borrowers) {
            auto draw = assignment_rng.uniform_index(bank_ids.size());
            if (!draw.ok()) {
                return draw.status();
            }
            auto *account = value.root.postings.get(borrower);
            const auto *bank = value.root.banks.get(bank_ids[*draw.get_if()]);
            account->key.settlement_node = bank->settlement_node;
        }
    }
    ...
    runtime.bank_market_health.assign(bank_ids.size() + 1U, 1.0);
    ...
    runtime.last_metrics.reserve_stock = value.root.reserves.reserve_stock().value();
    ...
    const auto state_validation =
        validate_m5_state(value.root, value.runtime, runtime, Tick(0));
...
Result<M5AdvanceResult>
advance_m5_ticks_impl(core::RootState &state, M4Runtime &real_economy_runtime,
    ...
    M5Extension extension(runtime, scratch, options, tick_extension);
    auto result =
        advance_ticks_extended(state, real_economy_runtime, real_economy_scratch, tick,
                               count, extension, options.base);
    if (!result.ok()) {
...
Result<M5AdvanceResult> advance_m5_ticks(core::RootState &state,
                                         M4Runtime &real_economy_runtime,
                                         M4TickScratch &real_economy_scratch,
                                         M5Runtime &runtime, M5TickScratch &scratch,
                                         Tick &tick, std::uint64_t count,
                                         const M5AdvanceOptions &options) {
    return advance_m5_ticks_impl(state, real_economy_runtime, real_economy_scratch,
                                 runtime, scratch, tick, count, nullptr, options);
}
...
advance_m5_ticks_extended(core::RootState &state, M4Runtime &real_economy_runtime,
                          M4TickScratch &real_economy_scratch, M5Runtime &runtime,
                          M5TickScratch &scratch, Tick &tick, std::uint64_t count,
                          M5TickExtension &extension, const M5AdvanceOptions &options) {
    return advance_m5_ticks_impl(state, real_economy_runtime, real_economy_scratch,
                                 runtime, scratch, tick, count, &extension, options);
}
