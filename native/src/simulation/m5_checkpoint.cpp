    ...
    return Json::array(
        {
            ...
            value.rate_inertia,
            value.manual_policy_rate.has_value()
                ? Json(*value.manual_policy_rate)
                : Json(nullptr),
            value.neutral_rate,
...
[[nodiscard]] M5PolicyState decode_policy(const Json& row) {
    ...
    value.monetary_regime =
        static_cast<MonetaryRegime>(row[i++].get<std::uint8_t>());
...
[[nodiscard]] Json encode_state(
    ...
    root.banks.for_each_alive(
        [&output](BankId id, const core::BankComponent& bank) {
            output["banks"].push_back(
                Json::array(
                    {
                        id.value(),
    ...
    for (const auto& row : root.central_bank_operations.records()) {
        output["central_bank_operations"].push_back(
            Json::array(
                {
                    row.id.value(),
        ...
        operations.push_back(
            {
                ...
                static_cast<core::CentralBankOperationKind>(
...
Result<std::vector<std::uint8_t>> save_m5_checkpoint(
    const core::RootState& root,
    ...
    bytes.reserve(
        kMagic.size() + 4 + 8 + base.get_if()->size()
        + 8 + encoded.size() + kDigestBytes
    );
...
Result<M5Checkpoint> load_m5_checkpoint(
    std::span<const std::uint8_t> bytes
) {
    if (bytes.size() > kMaximumCheckpointBytes
