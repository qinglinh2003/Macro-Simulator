...
[[nodiscard]] Status move_reserves(
    ...
    if (!finite(amount) || amount < 0.0) {
        return Status(
            ...
            "M6 reserve movement is invalid"
        );
    }
...
[[nodiscard]] Status transfer(
    ...
    if (source_index >= scratch.balances_.size()
        ...
        return Status(
            ...
            "M6 posting projection is stale"
        );
    }
...
[[nodiscard]] AccountId owner_account(
    const core::RootState& state,
    core::OwnerId owner
) noexcept {
    switch (owner.kind) {
        case core::OwnerKind::household: {
            const auto* household = state.households.get(
...
[[nodiscard]] FirmLifecycleRecord* firm_record(
    std::vector<FirmLifecycleRecord>& records,
    FirmId firm
) noexcept {
    const auto index = static_cast<std::size_t>(firm.value());
    return index < records.size() && records[index].firm == firm
        ? &records[index]
        : nullptr;
}

[[nodiscard]] const FirmLifecycleRecord* firm_record(
    const std::vector<FirmLifecycleRecord>& records,
    FirmId firm
) noexcept {
    const auto index = static_cast<std::size_t>(firm.value());
    return index < records.size() && records[index].firm == firm
...
[[nodiscard]] std::span<const EquityId> household_watchlist(
    ...
    return std::span<const EquityId>(
        runtime.watchlist_equities.data() + row.offset,
        row.count
    );
}
...
[[nodiscard]] std::optional<EquityId> firm_equity(
    const core::SecurityBook& book,
    FirmId firm
) noexcept {
    for (const auto security : book.securities_for_issuer(
             core::OwnerId::firm(firm))) {
...
[[nodiscard]] std::optional<EquityId> bank_equity(
    const core::SecurityBook& book,
    BankId bank
) noexcept {
    for (const auto security : book.securities_for_issuer(
             core::OwnerId::bank(bank))) {
        if (security.kind != core::SecurityKind::equity) {
            continue;
        }
        const auto id = EquityId(security.value);
...
[[nodiscard]] double household_security_value(
    const core::SecurityBook& book,
    core::OwnerId holder
) noexcept {
    double value = 0.0;
...
[[nodiscard]] double household_bond_value(
    const core::SecurityBook& book,
    core::OwnerId holder,
    Tick tick,
    double rate
...
[[nodiscard]] double bank_bond_face(
    const core::SecurityBook& book,
    BankId bank
) noexcept {
    double value = 0.0;
...
void open_bank_security_books(
    const core::RootState& state,
    const core::SecurityBook& book,
    M5TickScratch& monetary
) noexcept {
    state.banks.for_each_alive(
        [&](BankId bank, const core::BankComponent&) {
            const double securities_value =
...
void close_bank_security_books(
    const core::RootState& state,
    ...
    state.banks.for_each_alive(
        [&](BankId bank, const core::BankComponent& component) {
            auto* capital = capital_for(monetary, bank);
            if (capital == nullptr) {
                return;
            }
            capital->closing_capital =
                projected_balance(real, component.cash_account)
                + bank_bond_face(book, bank);
            const auto index = bank_index(bank);
            if (index < monetary.bank_capital_live_.size()) {
                monetary.bank_capital_live_[index] =
                    capital->closing_capital;
            }
        }
    );
...
[[nodiscard]] double outstanding_margin(
    const M6TickScratch& scratch,
    AccountId account
) noexcept {
    const auto index = account_index(account);
    return index < scratch.margin_by_account_.size()
        ? scratch.margin_by_account_[index]
        : 0.0;
}

void rebuild_debt_views(
    const core::RootState& state,
    ...
    for (const auto loan_id : scratch.margin_loans_) {
        ...
        const auto& loan = monetary.loans_[
            static_cast<std::size_t>(loan_id.value() - 1)
        ];
        if (!loan.active) {
            continue;
        }
...
[[nodiscard]] Status run_bond_open(
    const core::RootState& state,
    M4TickScratch& real,
    M5TickScratch& monetary,
    M6TickScratch& scratch,
    ...
    for (const auto& bond : scratch.securities_.bonds()) {
        ...
        for (const auto lot_id :
             scratch.securities_.lots_for_security(security)) {
            ...
            const double coupon =
                lot->units * bond.coupon_rate.value();
    ...
    for (const auto bond_id : matured) {
        ...
        std::vector<SecurityLotId> stable_lots(
            lot_ids.begin(),
            lot_ids.end()
        );
...
void build_firm_statements(
    ...
    for (std::size_t index = 0; index < real.firm_ids_.size(); ++index) {
        ...
        statement.debt =
            debt_index < scratch.debt_by_account_.size()
        ...
        statement.output_inventory_units =
            std::max(0.0, work.closing_inventory);
        const double unit_cost =
            work.produced > kEconomicEpsilon
            ? work.wage_bill / work.produced
            : work.posted_price;
        statement.output_inventory_unit_price = std::max(
            0.0,
            std::min(work.posted_price, std::max(0.0, unit_cost))
        );
        statement.output_inventory_value =
            statement.output_inventory_units
            * statement.output_inventory_unit_price;
        statement.inventory_value =
            statement.output_inventory_value
            + statement.work_in_progress_value
            + statement.input_inventory_value;
        statement.gross_assets =
            statement.cash + statement.capital_value
            + statement.inventory_value;
        statement.book_equity =
            statement.gross_assets - statement.debt
            - statement.interest_arrears;
        ...
        statement.borrowing_base_headroom = std::max(
            0.0,
            statement.borrowing_base_proxy - statement.debt
        );
        ...
        if (lifecycle_index < scratch.firm_return_.size()) {
            scratch.firm_return_[lifecycle_index] =
                statement.earnings / denominator;
        }
        scratch.working_metrics_.total_firm_book_equity +=
            statement.book_equity;
    }
}

[[nodiscard]] Status update_equity_fundamentals(
    const core::RootState& state,
    ...
    const double discount = equity_discount_rate(
        monetary_runtime,
        runtime.rules
    );
    auto status = scratch.securities_.begin_batch();
    ...
    for (const auto& contract : scratch.securities_.equities()) {
        ...
        if (contract.issuer_kind == core::EquityIssuerKind::firm) {
            ...
            const double q =
                lifecycle->statement.book_equity
                    > kEconomicEpsilon
                ? contract.price.value() * contract.outstanding_shares
                    / lifecycle->statement.book_equity
                : 1.0;
            lifecycle->tobin_q_ema += runtime.rules.q_smoothing
                * (q - lifecycle->tobin_q_ema);
        } else {
...
[[nodiscard]] double bank_exposure(
    const M5TickScratch& monetary,
    BankId bank
) noexcept {
    double result = 0.0;
...
[[nodiscard]] double originate_margin(
    const core::RootState& state,
    M4TickScratch& real,
    ...
    const auto bank = bank_for_node(
        state,
        real.account_nodes_[account_position]
    );
    const auto* capital = capital_for(monetary, bank);
    ...
    if (monetary_runtime.policy.bank_capital_constraint
        ...
        headroom = std::min(
            ...
            std::max(
                0.0,
                monetary_runtime.policy.bank_leverage_cap
    ...
    const double current_debt =
        account_position < scratch.debt_by_account_.size()
    ...
    headroom = std::min(
        ...
        std::max(
            0.0,
            monetary_runtime.policy.household_credit_limit * income
                - current_debt
        )
    );
    ...
    const auto id = LoanId(
        static_cast<std::uint64_t>(monetary.loans_.size()) + 1
    );
...
void generate_firm_equity_orders(
    const core::RootState& state,
    M4TickScratch& real,
    M5Runtime& monetary_runtime,
    M5TickScratch& monetary,
    ...
    state.households.for_each_alive(
        [&](HouseholdId household_id, const core::HouseholdComponent& household) {
            ...
            for (const auto equity_id : watch) {
                ...
                equity_value += scratch.securities_.units_held(
                ...
                attractiveness_total += std::max(0.0, 1.0 + response);
            }
            if (attractiveness_total
                <= kEconomicEpsilon) {
                return;
            }
            const double margin = outstanding_margin(
                scratch,
                household.primary_account
            );
            ...
            if (runtime.rules.margin_credit) {
                target_share = std::min(
                    runtime.policy.margin_max,
                    target_share
                );
            }
            const double target_equity =
                std::max(0.0, target_share * net_worth);
            double buy_cash = 0.0;
            for (const auto equity_id : watch) {
                const auto* equity = scratch.securities_.get(equity_id);
                if (equity == nullptr || !equity->active
                ...
                const double attractiveness =
                    std::max(0.0, 1.0 + response);
                const double desired =
                    target_equity
                    * attractiveness / attractiveness_total
                    / equity->price.value();
                ...
                const double delta =
                    (desired - current)
                    * runtime.rules.portfolio_adjustment;
                if (delta > 0.0) {
            ...
            if (runtime.rules.margin_credit && equity_value > 0.0) {
                const double secured_room = std::max(
                    0.0,
                    runtime.policy.margin_ltv * equity_value - margin
                );
                const double desired_margin = std::max(
                    0.0,
                    std::min(buy_cash, deposits + secured_room) - deposits
                );
                budget += originate_margin(
                    state,
                    real,
                    monetary_runtime,
                    monetary,
            ...
            const double scale =
                buy_cash > kEconomicEpsilon
                ? std::min(1.0, budget / buy_cash)
                : 1.0;
            for (const auto equity_id : watch) {
                const auto* equity = scratch.securities_.get(equity_id);
                if (equity == nullptr || !equity->active
                ...
                delta = delta > 0.0
                    ? delta * scale
                    : std::max(delta, -current);
                if (std::abs(delta)
                    > kEconomicEpsilon) {
                    scratch.orders_.push_back(
                        {equity_id, household_id, delta, ordinal++}
                    );
                }
            }
        }
...
void generate_bank_equity_orders(
    const core::RootState& state,
    M4TickScratch& real,
    const M6Runtime& runtime,
    M6TickScratch& scratch,
    ...
    std::vector<EquityId> bank_equities;
    bank_equities.reserve(state.banks.alive_count());
    state.banks.for_each_alive(
        [&](BankId bank, const core::BankComponent& component) {
            if (!component.alive) {
                return;
    ...
    state.households.for_each_alive(
        [&](HouseholdId household_id, const core::HouseholdComponent& household) {
            ...
            double value = 0.0;
            double attractiveness_total = 0.0;
            for (const auto equity_id : bank_equities) {
                const auto* equity = scratch.securities_.get(equity_id);
                if (equity == nullptr || equity->price.value()
                        <= kEconomicEpsilon) {
                    continue;
                }
                value += scratch.securities_.units_held(
                    core::SecurityId::equity(equity_id),
                    holder
                ) * equity->price.value();
                const double response =
            ...
            const double target = runtime.rules.bank_equity_target
                * std::max(0.0, deposits + value);
            double buy_cash = 0.0;
            for (const auto equity_id : bank_equities) {
                const auto* equity = scratch.securities_.get(equity_id);
                const double response =
                ...
                const double desired =
                    target * weight / equity->price.value();
                const double current = scratch.securities_.units_held(
                    core::SecurityId::equity(equity_id),
                    holder
                );
...
[[nodiscard]] double planned_primary_issue(
    const core::EquityContract& contract,
    ...
    if (contract.issuer_kind == core::EquityIssuerKind::firm) {
        ...
        const double issue_cash = std::min(
            runtime.rules.equity_issue_lambda
                * (firm->tobin_q_ema - 1.0)
                * firm->statement.book_equity,
            0.5 * firm->statement.book_equity
        );
        return std::min(
            issue_cash / contract.price.value(),
...
[[nodiscard]] Status clear_equity_orders(
    const core::RootState& state,
    M4TickScratch& real,
    M5TickScratch& monetary,
    M6Runtime& runtime,
    ...
    while (first < scratch.orders_.size()) {
        ...
        while (last < scratch.orders_.size()
               && scratch.orders_[last].equity
                   == scratch.orders_[first].equity) {
            ++last;
        }
        ...
        if (contract->issuer_kind == core::EquityIssuerKind::firm) {
            firm = firm_record(
                scratch.firms_,
                FirmId(contract->issuer.value)
            );
        } else {
            bank_capital = capital_for(
                monetary,
                BankId(contract->issuer.value)
            );
        }
        const double primary = planned_primary_issue(
            *contract,
            firm,
            bank_capital,
            runtime
        ...
        const double buy_scale =
            buy > kEconomicEpsilon
            ? executed / buy
            : 0.0;
        const double sell_scale =
            sell_total > kEconomicEpsilon
            ? executed / sell_total
            : 0.0;
        ...
        if (primary_executed > kEconomicEpsilon) {
            status = transfer(
                ...
                real,
                clearing,
                contract->issuer_account,
                primary_executed * price
            );
            if (!status.ok()) {
                return status;
            }
            scratch.working_metrics_.primary_equity_raised +=
                primary_executed * price;
        ...
        for (const auto& seller : scratch.sellers_) {
            ...
            while (remaining > kEconomicEpsilon
                ...
                status = scratch.securities_.transfer_units(
                    ...
                    core::OwnerId::household(buyer.household),
                    take,
                    Money(take * price)
                );
                if (!status.ok()) {
        ...
        const double next_price = std::max(
            kEconomicEpsilon,
            price * (1.0
                + runtime.rules.equity_price_adjustment * bounded)
        );
        const double price_return =
            price > kEconomicEpsilon
            ? (next_price - price) / price
            : 0.0;
        const double next_trend =
            contract->trend
            + runtime.rules.equity_trend_lambda
                * (price_return - contract->trend);
        const double next_peak = std::max(
            contract->peak_price.value(),
            next_price
        );
        status = scratch.securities_.update_equity_valuation(
            equity_id,
    ...
    scratch.working_metrics_.clearing_residual =
        projected_balance(real, clearing);
    return Status::success();
}

[[nodiscard]] Status service_margin(
    const core::RootState& state,
    M4TickScratch& real,
    M5TickScratch& monetary,
    M6Runtime& runtime,
    ...
    state.households.for_each_alive(
        [&](HouseholdId household_id, const core::HouseholdComponent& household) {
            ...
            double call = std::max(
                0.0,
                margin - runtime.policy.margin_ltv * equity
            );
            double available = projected_balance(real, account);
            double repayment = std::min(call, available);
            for (const auto loan_id : scratch.margin_loans_) {
                if (repayment <= kEconomicEpsilon
                    || loan_id.value() == 0
                    || loan_id.value() > monetary.loans_.size()) {
                    continue;
                }
                auto& loan = monetary.loans_[
                    static_cast<std::size_t>(loan_id.value() - 1)
                ];
            ...
            rebuild_debt_views(state, monetary, scratch);
            const double remaining_margin = outstanding_margin(
                scratch,
                account
            );
            if (!runtime.policy.household_bankruptcy
                || remaining_margin <= kEconomicEpsilon
                || projected_balance(real, account) + equity
...
[[nodiscard]] Status resolve_firm_exit(
    const core::RootState& state,
    M4TickScratch& real,
    M5TickScratch& monetary,
    M6TickScratch& scratch,
    ...
    if (firm == nullptr || lifecycle == nullptr || !lifecycle->active) {
        return Status(
            ErrorCode::not_found,
            "firm exit references an absent firm"
        );
    }
    ...
    if (cash > kEconomicEpsilon) {
        if (!defaulted && equity.has_value()) {
            ...
            if (shares > kEconomicEpsilon) {
                ...
                for (const auto lot_id : lot_ids) {
                    ...
                    const auto recipient = owner_account(
                        state,
                        lot->holder
                    );
        ...
        if (cash > kEconomicEpsilon) {
            const auto node =
                real.account_nodes_[account_index(account)];
...
[[nodiscard]] Status run_firm_exits(
    const core::RootState& state,
    M4TickScratch& real,
    M5TickScratch& monetary,
    M6Runtime& runtime,
    ...
    for (const auto firm_id : real.firm_ids_) {
        ...
        const bool insolvent =
            lifecycle->statement.book_equity < -kTolerance;
        lifecycle->insolvent_days = insolvent
            ? lifecycle->insolvent_days + 1
            : 0;
        const bool shell =
            ...
            && lifecycle->statement.output_inventory_units
                <= kEconomicEpsilon;
        lifecycle->shell_days = shell
            ? lifecycle->shell_days + 1
            : 0;
        const bool forced =
            options.force_firm_exit.has_value()
            && *options.force_firm_exit == firm_id;
        const bool defaulted =
            forced || lifecycle->insolvent_days
                >= runtime.rules.bankrupt_persistence;
        const bool voluntary =
            lifecycle->shell_days >= runtime.rules.shell_exit_days;
        ...
        const auto status = resolve_firm_exit(
            state,
            real,
            monetary,
            scratch,
...
void run_sector_switching(
    ...
    if (!runtime.rules.consumption_strata
        || !runtime.rules.sector_switching) {
        return;
    }
    ...
    for (const auto firm_id : real.firm_ids_) {
        ...
        const double value =
            index < scratch.firm_return_.size()
            ? scratch.firm_return_[index]
            : 0.0;
    ...
    for (std::size_t index = 0; index < real.firm_ids_.size(); ++index) {
        ...
        const double own =
            lifecycle->stratum == ConsumptionStratum::necessity
            ? necessity_return
            : luxury_return;
        const double other =
            lifecycle->stratum == ConsumptionStratum::necessity
            ? luxury_return
            : necessity_return;
        if (other > own * (1.0 + runtime.rules.switch_return_gap)
            && other > 0.0) {
            ++lifecycle->switch_pressure_days;
        } else {
            lifecycle->switch_pressure_days = 0;
        }
        if (lifecycle->switch_pressure_days
                < runtime.rules.switch_pressure_days) {
            continue;
        }
        const double draw = unit_draw(
            state.seed,
            lifecycle_counter++,
            0x535749544348ULL
        );
        ...
        const double loss =
            work.closing_capital * runtime.rules.switch_retool_loss;
        work.closing_capital = std::max(
            0.0,
            work.closing_capital - loss
        );
        lifecycle->stratum =
            lifecycle->stratum == ConsumptionStratum::necessity
...
[[nodiscard]] std::optional<HouseholdId> pick_founder(
    const core::RootState& state,
    const M4TickScratch& real,
    double need,
    std::uint64_t seed,
...
[[nodiscard]] Status stage_firm_entries(
    const core::RootState& state,
    M4TickScratch& real,
    M6Runtime& runtime,
    M6TickScratch& scratch,
    ...
    if (!runtime.rules.firm_dynamics
        || runtime.rules.entry_max == 0) {
        return Status::success();
    }
    ...
    for (std::uint32_t entry = 0; entry < entries; ++entry) {
        ...
        const auto predicted_firm = FirmId(
        ...
        component.total_factor_productivity =
            ...
            ? 1.0
            : state.firms.get(
                real.firm_ids_.front()
            )->total_factor_productivity;
        component.posted_price = Price(
            std::max(
                kEconomicEpsilon,
                runtime.last_metrics.economy.economy.price_index
            )
        );
        component.posted_wage = Money(
            real.firm_ids_.empty()
            ? 1.0
            : state.firms.get(
                real.firm_ids_.front()
        ...
        if (scratch.firms_.size() <= predicted_firm.value()) {
            scratch.firms_.resize(
                static_cast<std::size_t>(predicted_firm.value()) + 1
            );
        }
        scratch.firms_[
            static_cast<std::size_t>(predicted_firm.value())
        ] = lifecycle;
        ...
        const double price = runtime.rules.startup_deposits
        ...
        const std::array holding{
            core::InitialSecurityHolding{
                core::OwnerId::household(*founder),
                runtime.rules.shares_per_firm,
                Money(runtime.rules.startup_deposits),
            }
        };
        const auto created = scratch.securities_.create_equity(
            equity,
            holding
        );
        if (!created.ok()) {
        ...
        scratch.firm_entries_.push_back(
            {
                ...
                *founder,
                ...
                real.account_nodes_[
                    account_index(founder_component->primary_account)
                ],
                runtime.rules.startup_deposits,
                component,
                lifecycle,
...
[[nodiscard]] Status resolve_dead_bank_equity(
    const core::RootState& state,
    M5TickScratch& monetary,
    M6TickScratch& scratch
) {
...
[[nodiscard]] Status stage_bank_entries(
    const core::RootState& state,
    M4TickScratch& real,
    const M5Runtime& monetary_runtime,
    M5TickScratch& monetary,
    ...
    for (const auto& capital : monetary.bank_capital_) {
        ...
        if (pnl != nullptr && pnl->net_income > 0.0
    ...
    if (profitable_capital > kEconomicEpsilon) {
        ...
        probability = std::clamp(
            runtime.rules.bank_entry_beta * (roe / rate - 1.0),
            0.0,
            1.0
        );
        probability *= std::max(
            0.0,
            1.0 - static_cast<double>(alive)
        ...
        probability = std::min(
            1.0,
            5.0 * runtime.rules.bank_entry_beta
        );
    }
    if (force) {
        probability = 1.0;
    ...
    for (std::uint32_t entry = 0;
         entry < runtime.rules.bank_entry_max;
         ++entry) {
        if (unit_draw(
                state.seed,
                lifecycle_counter++,
                0x42414e4b454e5452ULL)
            >= probability) {
            continue;
        }
        const auto founder = pick_founder(
        ...
        const auto* founder_component = state.households.get(*founder);
        const auto predicted_bank = BankId(
            state.banks.allocator_state().next_id
                + scratch.bank_entries_.size()
        );
        const auto predicted_node = SettlementNodeId(
            static_cast<std::uint64_t>(state.reserves.size())
        ...
        component.leverage_appetite =
            monetary_runtime.rules.bank_leverage_mean;
        ...
        const double price = runtime.policy.bank_minimum_capital
...
[[nodiscard]] Status run_bond_issuance(
    const core::RootState& state,
    M4TickScratch& real,
    const M5TickScratch& monetary,
    M6Runtime& runtime,
    ...
    if (!runtime.rules.bonds
        || runtime.policy.bond_finance_fraction <= 0.0) {
        return Status::success();
    }
    const auto treasury = state.institutions.treasury_account;
    const double outstanding =
        scratch.securities_.total_bond_face().value();
    const double government_debt = std::max(
        0.0,
        outstanding - projected_balance(real, treasury)
    );
    const double target =
        runtime.policy.bond_finance_fraction * government_debt;
    ...
    demands.reserve(
        state.households.alive_count() + state.banks.alive_count()
    );
    ...
    state.households.for_each_alive(
        [&](HouseholdId id, const core::HouseholdComponent& household) {
            ...
            const double room = std::max(
                0.0,
                runtime.policy.household_bond_target
                    * (deposits + bonds) - bonds
            );
            const double demand = std::min(
                std::max(0.0, deposits - buffer),
                room
            );
            if (demand > kEconomicEpsilon) {
                demands.push_back(
                    {holder, household.primary_account, demand}
                );
                total_demand += demand;
            }
        }
    ...
    state.banks.for_each_alive(
        [&](BankId id, const core::BankComponent& bank) {
            if (!bank.alive
                || runtime.policy.bank_bond_appetite <= 0.0) {
                return;
            }
            const auto reserve_position = node_index(bank.settlement_node);
            const double reserve =
                reserve_position < real.reserve_balances_.size()
                ? real.reserve_balances_[reserve_position]
                : 0.0;
            const auto* capital = capital_for(monetary, id);
            const double current = std::accumulate(
                scratch.securities_.bank_lots(id).begin(),
                scratch.securities_.bank_lots(id).end(),
                0.0,
            ...
            double demand = runtime.policy.bank_bond_appetite
                * std::max(0.0, reserve);
            if (runtime.policy.bank_bond_duration_limit > 0.0
                && capital != nullptr) {
                demand = std::min(
                    demand,
                    std::max(
                        0.0,
                        runtime.policy.bank_bond_duration_limit
                            * std::max(0.0, capital->closing_capital)
                            - current
                    )
                );
    ...
    std::uint64_t maturity = tick.value()
        + runtime.policy.bond_maturity_days;
    ...
    contract.issuer = core::OwnerId::institutional(
    ...
    const auto clearing_owner = core::OwnerId::institutional(
        core::OwnerKind::institution,
        2
    );
    const auto bond = scratch.securities_.issue_bond(
        contract,
        clearing_owner,
        Money(issue)
    );
    ...
    for (std::size_t index = 0; index < demands.size(); ++index) {
        ...
        status = scratch.securities_.transfer_units(
            core::SecurityId::bond(*bond.get_if()),
            clearing_owner,
            demands[index].holder,
            amount,
            Money(amount)
        );
        if (!status.ok()) {
...
void measure_m6(
    ...
    for (const auto& equity : scratch.securities_.equities()) {
        ...
        const double market_cap =
            equity.outstanding_shares * equity.price.value();
    ...
    for (const auto loan_id : scratch.margin_loans_) {
        if (loan_id.value() > 0
            ...
            if (loan.active) {
                scratch.working_metrics_.margin_principal +=
                    loan.principal.value();
            }
        }
    }
    const double shares = std::accumulate(
        scratch.securities_.equities().begin(),
        scratch.securities_.equities().end(),
        0.0,
        [](double sum, const core::EquityContract& equity) {
            return sum
                + (equity.active ? equity.outstanding_shares : 0.0);
        }
    );
    scratch.working_metrics_.equity_turnover =
        shares > kEconomicEpsilon
        ? scratch.working_metrics_.equity_turnover / shares
        : 0.0;
    static_cast<void>(state);
}
...
[[nodiscard]] Status validate_projection(
    ...
    const auto security_status = scratch.securities_.validate(
    ...
    if (!finite(scratch.working_metrics_.clearing_residual)
        || std::abs(scratch.working_metrics_.clearing_residual)
            > 1.0e-6) {
        return Status(
            ErrorCode::invariant_violation,
            "invariant.m6.clearing-zero"
        );
    }
...
void append_founder_watchlist(
    ...
    for (std::size_t index = 1;
         index < runtime.watchlist_rows.size();
         ++index) {
        ...
        const auto existing = household_watchlist(
            runtime,
            old.household
        );
        values.insert(values.end(), existing.begin(), existing.end());
        if (old.household == founder
            && std::find(existing.begin(), existing.end(), equity)
                == existing.end()) {
            values.push_back(equity);
        }
        row.count = static_cast<std::uint32_t>(
            values.size() - row.offset
        );
        rows[index] = row;
    }
...
void commit_lifecycle(
    ...
    for (const auto& exit : scratch.firm_exits_) {
        ...
        for (const auto& lot : state.ownership.records()) {
            if (lot.active
                && lot.asset.kind == core::AssetKind::firm_equity
                && lot.asset.value == exit.firm.value()) {
                if (!transaction.mutate_ownership(
                        lot.id,
                        lot.owner,
                        0.0
                    ).ok()) {
    ...
    for (const auto& entry : scratch.bank_entries_) {
        ...
        auto node = state.reserves.create_position(
            entry.bank,
            Money(0.0)
        );
...
public:
    ...
    Status prepare_tick(
        ...
        open_bank_security_books(
            state,
            scratch_.securities_,
            monetary
        );
        return run_bond_open(
            state,
            real,
            monetary,
            scratch_,
    ...
    Status before_bank_resolution(
        ...
        auto status = update_equity_fundamentals(
            state,
            monetary_runtime,
            monetary,
            runtime_,
        ...
        if (runtime_.rules.bank_equity
            && runtime_.rules.bank_equity_trading) {
            generate_bank_equity_orders(
                state,
                real,
                runtime_,
                scratch_,
    ...
    Status close_institutions(
        ...
        auto status = resolve_dead_bank_equity(
            state,
            monetary,
            scratch_
        );
        ...
        status = stage_bank_entries(
            ...
            scratch_,
            lifecycle_counter_,
            options_.force_bank_entry
        );
        if (!status.ok()) {
            return status;
        }
        status = run_bond_issuance(
            state,
            real,
            monetary,
            runtime_,
...
[[nodiscard]] core::EquityContract initial_firm_equity(
    const core::RootState& state,
    const M6Rules& rules,
    FirmId firm_id,
...
void build_genesis_watchlists(
    const core::RootState& state,
    const std::vector<FirmId>& firms,
    const M6Rules& rules,
    M6Runtime& runtime
) {
    ...
    const auto count = std::min<std::size_t>(
        rules.watchlist_size,
        firms.size()
    );
    runtime.watchlist_equities.reserve(
        state.households.alive_count() * count
    );
    state.households.for_each_alive(
        [&](HouseholdId household, const core::HouseholdComponent&) {
            std::vector<std::size_t> candidates(firms.size());
            std::iota(candidates.begin(), candidates.end(), 0);
            std::uint64_t random_state = splitmix64(
                state.seed ^ household.value()
                    ^ 0x57415443484c4953ULL
            );
            for (std::size_t index = 0; index < count; ++index) {
                random_state = splitmix64(random_state);
                const auto selected = index
                    + static_cast<std::size_t>(
                        random_state % (candidates.size() - index)
                    );
                std::swap(candidates[index], candidates[selected]);
            }
            auto& row = runtime.watchlist_rows[
                static_cast<std::size_t>(household.value())
            ];
            row.household = household;
            row.offset = static_cast<std::uint32_t>(
                runtime.watchlist_equities.size()
            );
            row.count = static_cast<std::uint32_t>(count);
            for (std::size_t index = 0; index < count; ++index) {
                runtime.watchlist_equities.push_back(
                    EquityId(candidates[index] + 1)
                );
            }
        }
    );
...
double bond_price(
    ...
    if (!finite(face) || !finite(required_return) || !finite(coupon_rate)
        || face < 0.0 || coupon_rate < 0.0
        || required_return <= -1.0
        || remaining_days == 0) {
        return std::max(0.0, face);
    }
    if (remaining_days == 1
        && coupon_rate <= kEconomicEpsilon) {
        return face;
    }
    ...
    const double coupons =
        std::abs(required_return) <= 1.0e-12
...
double equity_discount_rate(
    const M5Runtime& monetary_runtime,
    const M6Rules& rules
) noexcept {
    return std::max(
        rules.valuation_discount_floor,
        std::max(0.0, monetary_runtime.policy_rate)
            + rules.valuation_risk_premium
    );
}

double residual_income_fundamental(
    double book_value,
    double residual_income,
    double shares,
    double discount_rate
) noexcept {
    if (!finite(book_value) || !finite(residual_income)
        || !finite(shares) || !finite(discount_rate)
        || shares <= kEconomicEpsilon) {
        return 0.0;
    }
    ...
    return std::max(
        0.0,
        (book_value + std::max(0.0, residual_income) / discount)
            / shares
    );
}
...
Status validate_m6_policy(const M6PolicyState& policy) noexcept {
    ...
    if (!all_finite(values)
        || policy.bond_finance_fraction < 0.0
        || policy.bond_finance_fraction > 1.0
        || policy.bond_coupon_rate < 0.0
        || policy.bond_maturity_days == 0
        || policy.household_bond_target < 0.0
        || policy.household_bond_target > 1.0
        || policy.bank_bond_appetite < 0.0
        || policy.bank_bond_appetite > 1.0
        || policy.bank_bond_duration_limit < 0.0
        || policy.margin_ltv < 0.0
        || policy.margin_ltv >= 1.0
        || policy.margin_max < 1.0
        || policy.bank_minimum_capital < 0.0) {
        return Status(
            ErrorCode::invalid_argument,
            "M6 policy is invalid"
        );
    }
    return Status::success();
}
...
Status validate_m6_spec(const M6SimulationSpec& spec) noexcept {
    ...
    if (!all_finite(values) || spec.rules.bond_maturity_bucket == 0
        ...
        || spec.rules.watchlist_size > 100'000
        || spec.rules.genesis_founder_pool <= 0.0
        || spec.rules.genesis_founder_pool > 1.0
        || spec.rules.equity_price_adjustment < 0.0
        || spec.rules.equity_trend_lambda < 0.0
        || spec.rules.equity_trend_lambda > 1.0
        || spec.rules.residual_income_lambda < 0.0
        || spec.rules.residual_income_lambda > 1.0
        || spec.rules.q_smoothing < 0.0
        || spec.rules.q_smoothing > 1.0
        || spec.rules.fundamental_weight < 0.0
        || spec.rules.chartist_weight < 0.0
        ...
        || spec.rules.equity_issue_lambda < 0.0
        || spec.rules.valuation_discount_floor <= 0.0
        || spec.rules.valuation_risk_premium < 0.0
        || spec.rules.capital_haircut < 0.0
        || spec.rules.capital_haircut > 1.0
        || spec.rules.inventory_haircut < 0.0
        || spec.rules.inventory_haircut > 1.0
        || spec.rules.bankrupt_persistence == 0
        || spec.rules.shell_exit_days == 0
        || spec.rules.entry_hurdle < 0.0
        || spec.rules.entry_beta < 0.0
        || spec.rules.startup_deposits < 0.0
        || spec.rules.startup_capital < 0.0
        || spec.rules.switch_return_gap < 0.0
        || spec.rules.switch_pressure_days == 0
        || spec.rules.switch_hazard < 0.0
        || spec.rules.switch_hazard > 1.0
        || spec.rules.switch_retool_loss < 0.0
        || spec.rules.switch_retool_loss > 1.0
        || spec.rules.bank_shares <= 0.0
        || spec.rules.bank_equity_lambda < 0.0
        || spec.rules.bank_equity_lambda > 1.0
        || spec.rules.bank_equity_target < 0.0
        || spec.rules.bank_equity_target > 1.0
        ...
        || (spec.rules.sector_switching
            && !spec.rules.consumption_strata)) {
        return Status(
            ErrorCode::invalid_argument,
            "M6 rules are invalid"
        );
    }
    return Status::success();
}

Status validate_m6_state(
    const core::RootState& state,
    ...
    if (!validate_m6_policy(runtime.policy).ok()
        || !runtime.securities.validate(state.accounting_tolerance).ok()
        || !finite(runtime.replacement_capital_price)
        || runtime.replacement_capital_price <= 0.0) {
        return Status(
            ErrorCode::invariant_violation,
            "M6 persistent state is invalid"
        );
    }
    bool firms_valid = true;
    state.firms.for_each_alive(
        [&](FirmId id, const core::FirmComponent&) {
            const auto* record = firm_record(runtime.firms, id);
            firms_valid = firms_valid && record != nullptr
                && record->active && record->firm == id
                && record->statement.firm == id
                && finite(record->statement.book_equity)
                && finite(record->residual_income_ema)
                && finite(record->tobin_q_ema);
        }
    );
    ...
    for (std::size_t index = 1;
        ...
        for (const auto equity : household_watchlist(
                 runtime,
                 row.household)) {
    ...
    for (const auto loan : runtime.margin_loans) {
        if (loan.value() == 0
            || loan.value() > state.loans.records().size()) {
            return Status(
                ErrorCode::invariant_violation,
...
Result<M6Initialization> build_m6_genesis(
    ...
    runtime.firms.resize(
        static_cast<std::size_t>(
            value.root.firms.allocator_state().next_id
        )
    );
    ...
    value.root.firms.for_each_alive(
        [&](FirmId id, const core::FirmComponent& firm) {
            ...
            lifecycle.stratum =
                ...
                : ConsumptionStratum::necessity;
            lifecycle.active = true;
            runtime.firms[static_cast<std::size_t>(id.value())] =
                lifecycle;
            if (firm.sector == core::FirmSector::consumption
                && runtime.rules.firm_equity) {
                equity_firms.push_back(id);
            }
        }
    );
    build_genesis_watchlists(
        value.root,
        equity_firms,
        runtime.rules,
        runtime
    );
    std::vector<std::vector<HouseholdId>> watchers(
        equity_firms.size()
    );
    for (std::size_t household_index = 1;
         household_index < runtime.watchlist_rows.size();
         ++household_index) {
        ...
        for (const auto equity : household_watchlist(
                 ...
                 row.household)) {
            if (equity.value() > 0
                && equity.value() <= watchers.size()) {
                watchers[
                    static_cast<std::size_t>(equity.value() - 1)
                ].push_back(row.household);
            }
        }
    }
    for (std::size_t index = 0; index < equity_firms.size(); ++index) {
        ...
        auto contract = initial_firm_equity(
            value.root,
            runtime.rules,
            firm_id,
            *firm,
                ...
                holdings.push_back(
                    {
                        ...
                        Money(
                            lifecycle->statement.book_equity
                            * units / runtime.rules.shares_per_firm
                        ),
                    }
                );
            }
        }
        const auto created = runtime.securities.create_equity(
            contract,
            holdings
        );
        if (!created.ok()) {
    ...
    if (runtime.rules.bank_equity) {
        value.root.banks.for_each_alive(
            [&](BankId bank_id, const core::BankComponent& bank) {
                ...
                const double price = std::max(
                    kEconomicEpsilon,
                    book / runtime.rules.bank_shares
                );
                contract.price = Price(price);
                contract.last_price = Price(price);
                contract.peak_price = Price(price);
...
void M6TickScratch::reserve(
    ...
    orders_.reserve(
        ...
        * static_cast<std::size_t>(
            runtime.rules.watchlist_size
            + state.banks.alive_count()
        )
    );
    buyers_.reserve(state.households.alive_count());
    sellers_.reserve(state.households.alive_count());
    margin_loans_.reserve(
        runtime.margin_loans.size() + state.households.alive_count()
    );
...
Result<M6AdvanceResult> advance_m6_ticks(
    core::RootState& state,
