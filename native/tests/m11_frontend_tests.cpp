...
void test_snapshot_is_role_scoped_and_deterministic() {
    ...
    for (const auto &policy :
         central_bank.get_if()->policies) {
        const auto *lever = find_m11_policy_lever(policy.lever);
        assert(lever != nullptr);
        assert(lever->owner_role == "central_bank");
    }
    assert(!has_release(
        *treasury.get_if(), "bank_reserves_total"));
    assert(has_release(
        *central_bank.get_if(), "bank_reserves_total"));
...
void test_delta_requires_exact_base_and_reports_changes() {
    ...
    const M11AccessScope scope{
        "player", EconomyId(0U), "treasury"};
    auto base =
        projection.snapshot(*session.get_if(), scope, "epoch-b", 1U);
    assert(base.ok());
    assert(session.get_if()
               ->advance_until_decision({1U, true})
               .ok());
    auto result =
        projection.snapshot(*session.get_if(), scope, "epoch-b", 2U);
    assert(result.ok());
    auto delta =
        projection.delta(*base.get_if(), *result.get_if());
    assert(delta.ok());
    assert(delta.get_if()->base_snapshot_id ==
           base.get_if()->snapshot_id);
    assert(delta.get_if()->result_snapshot_id ==
           result.get_if()->snapshot_id);
    ...
    wrong_sequence.snapshot_id =
        m11_frontend_snapshot_id(wrong_sequence);
    assert(projection.delta(
                         *base.get_if(), wrong_sequence)
               .status()
               .code() == ErrorCode::stale_handle);

    auto wrong_epoch = *result.get_if();
    wrong_epoch.cache_epoch = "epoch-c";
    wrong_epoch.snapshot_id =
        m11_frontend_snapshot_id(wrong_epoch);
    assert(projection.delta(*base.get_if(), wrong_epoch)
               .status()
               .code() == ErrorCode::stale_handle);
...
void test_context_projection_is_private_and_fully_hashed() {
    ...
    auto treasury = projection.snapshot(
        *session.get_if(),
        {"player", EconomyId(0U), "treasury"}, "epoch-d", 1U);
    auto regulator = projection.snapshot(
        *session.get_if(),
        {"player", EconomyId(0U), "regulator"}, "epoch-d", 1U);
    assert(treasury.ok());
    assert(regulator.ok());
    ...
    assert(m11_frontend_snapshot_id(changed) !=
           treasury.get_if()->snapshot_id);
    if (!changed.contexts.front()
             .permitted_actions.empty()) {
        changed = *treasury.get_if();
        changed.contexts.front()
            .permitted_actions.front()
...
void test_invalid_scope_is_rejected() {
    ...
    assert(!projection
                .snapshot(
                    *session.get_if(),
                    {"", EconomyId(0U), "treasury"},
                    "epoch-e", 1U)
                .ok());
    assert(!projection
                .snapshot(
                    *session.get_if(),
                    {"player", EconomyId(9U), "treasury"},
                    "epoch-e", 1U)
                .ok());
    assert(!projection
                .snapshot(
                    *session.get_if(),
                    {"player", EconomyId(0U), "invalid-seat"},
                    "epoch-e", 1U)
                .ok());
}
