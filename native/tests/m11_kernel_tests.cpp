...
void test_default_contract() {
    auto scheduler =
        macro_sim::control::M11DecisionScheduler::create();
    assert(scheduler.ok());
    const auto &value = *scheduler.get_if();
    assert(value.calendars().size() ==
           macro_sim::control::kM11DecisionGroupCount);
    assert(value.triggers().size() ==
           macro_sim::control::kM11DefaultTriggerCount);
    ...
    auto capacity =
        value.administrative_capacity("tax_and_transfers");
    assert(capacity.ok());
...
void test_trigger_hysteresis() {
    ...
    const std::vector<M11MetricSample> high{{
        "reserve_floor_breach_share", 0.15,
    }};
    auto first =
        scheduler.evaluate_triggers(Tick(10U), EconomyId(0U), high);
    assert(first.ok());
    assert(first.get_if()->empty());
    auto second =
        scheduler.evaluate_triggers(Tick(11U), EconomyId(0U), high);
    assert(second.ok());
    assert(second.get_if()->size() == 1U);
    assert(second.get_if()->front().trigger_id ==
           "bank_liquidity_stress");
    assert(second.get_if()->front().expires_at == Tick(12U));

    auto held =
        scheduler.evaluate_triggers(Tick(12U), EconomyId(0U), high);
    assert(held.ok());
    assert(held.get_if()->empty());
    const std::vector<M11MetricSample> low{{
        "reserve_floor_breach_share", 0.01,
    }};
    auto exited =
        scheduler.evaluate_triggers(Tick(13U), EconomyId(0U), low);
    assert(exited.ok());
    assert(exited.get_if()->empty());
    auto cooling_one =
        scheduler.evaluate_triggers(Tick(14U), EconomyId(0U), high);
    auto cooling_two =
        scheduler.evaluate_triggers(Tick(15U), EconomyId(0U), high);
    assert(cooling_one.ok() && cooling_two.ok());
    assert(cooling_two.get_if()->empty());
    auto after_cooldown =
        scheduler.evaluate_triggers(Tick(41U), EconomyId(0U), high);
...
void test_costs() {
    ...
    const auto *lever =
        macro_sim::control::find_m11_policy_lever("tax_income_rate");
    ...
    auto cost =
        macro_sim::control::m11_adjustment_cost(spec, changes, false);
    assert(cost.ok());
    const auto distance = 0.05 / *lever->control_scale;
    const auto expected =
        spec.major.fixed + spec.major.linear * distance +
        spec.major.quadratic * distance * distance;
    assert(std::abs(*cost.get_if() - expected) < 1.0e-12);
    auto emergency =
        macro_sim::control::m11_adjustment_cost(spec, changes, true);
    assert(emergency.ok());
    assert(std::abs(*emergency.get_if() -
                    expected * spec.emergency_premium) <
           1.0e-12);
    auto administrative =
        macro_sim::control::m11_administrative_cost(spec, changes);
    assert(administrative.ok());
    assert(std::abs(*administrative.get_if() -
                    (0.25 + lever->administrative_weight)) <
           1.0e-12);
    ...
    auto zero =
        macro_sim::control::m11_administrative_cost(spec, no_change);
    assert(zero.ok());
...
void test_event_stream() {
    ...
    auto first = stream.append(
        Tick(0U), "boundary_started", "op-1", "system", "{}",
        M11EventVisibility::public_record);
    assert(first.ok());
    auto second = stream.append(
        Tick(0U), "proposal_received", "op-2", "player",
        "{\"lever\":\"income_tax\"}",
...
void test_release_stream() {
    ...
    auto first = stream.append(
        EconomyId(0U), "real_output", Tick(1U), Tick(3U), 0U,
        10.0, 4U);
    assert(first.ok());
    auto revision = stream.append(
        EconomyId(0U), "real_output", Tick(1U), Tick(5U), 1U,
        10.5, 7U);
    assert(revision.ok());
    auto other = stream.append(
        EconomyId(0U), "price_index", Tick(2U), Tick(2U), 0U,
        std::nullopt, 8U);
    assert(other.ok());
    ...
    std::vector<M11ReleaseRecord> records(
        stream.releases().begin(), stream.releases().end());
...
void emit_golden() {
    ...
    std::cout << "{\"calendar_count\":" << scheduler.calendars().size()
              ...
              << ",\"groups\":[";
    for (std::size_t index = 0; index < scheduler.calendars().size();
         ++index) {
        if (index != 0U) {
            std::cout << ',';
        }
        ...
        std::cout << "{\"admin\":" << std::setprecision(17)
                  ...
                  << ",\"id\":\"" << calendar.decision_group
                  << "\",\"period\":" << calendar.period_ticks << '}';
    }
    std::cout << "],\"adjustment\":" << std::setprecision(17)
              << *adjustment.get_if() << ",\"administrative\":"
              << *administrative.get_if() << ",\"notice_count\":"
              << second.get_if()->size() << ",\"notice_expiry\":"
              << second.get_if()->front().expires_at.value()
              << ",\"notice_id\":\""
              << second.get_if()->front().trigger_id << "\"}\n";
}
