...
[[nodiscard]] bool has_series(
    const std::vector<M11ReleasedObservation> &values,
...
void test_contract_and_access() {
    ...
    for (std::size_t index = 0U; index < fields.size(); ++index) {
        assert(find_m11_observation_field(fields[index].series_id) ==
               &fields[index]);
        if (index > 0U) {
            assert(fields[index - 1U].series_id <
                   fields[index].series_id);
        }
    }
    const auto *reserves =
        find_m11_observation_field("bank_reserves_total");
    ...
    const auto *oracle =
        find_m11_observation_field("oracle_daily_output");
...
void test_cadence_lag_aggregation_and_role_filter() {
    ...
    auto initial = service.publish_due(
        engine.metrics(), engine.world(), engine.tick(), 0U, stream);
    assert(initial.ok());
    assert(*initial.get_if() > 0U);
    const auto count = stream.releases().size();
    auto duplicate = service.publish_due(
        engine.metrics(), engine.world(), engine.tick(), 0U, stream);
    assert(duplicate.ok());
    ...
    assert(advanced.ok());
    auto first_day = service.publish_due(
        engine.metrics(), engine.world(), engine.tick(), 1U, stream);
    assert(first_day.ok());
    auto public_values = service.observation(
        stream, EconomyId(0U), Tick(1U), "treasury");
    auto central_bank_values = service.observation(
        stream, EconomyId(0U), Tick(1U), "central_bank");
    auto oracle_values = service.observation(
        stream, EconomyId(0U), Tick(1U), "oracle");
    assert(public_values.ok() && central_bank_values.ok() &&
           oracle_values.ok());
    assert(!has_series(*public_values.get_if(),
                       "bank_reserves_total"));
    assert(has_series(*central_bank_values.get_if(),
                      "bank_reserves_total"));
    assert(!has_series(*public_values.get_if(),
                       "oracle_daily_output"));
    assert(has_series(*oracle_values.get_if(),
                      "oracle_daily_output"));

    for (std::uint64_t day = 2U; day <= 9U; ++day) {
        auto next = engine.advance_ticks(1U);
        assert(next.ok());
        auto published = service.publish_due(
            engine.metrics(), engine.world(), engine.tick(), day,
            stream);
        assert(published.ok());
    }
    auto released = service.observation(
        stream, EconomyId(0U), Tick(9U), "treasury");
    assert(released.ok());
    ...
    const auto price = std::find_if(
        released.get_if()->begin(), released.get_if()->end(),
    ...
    const auto samples =
        service.trigger_samples(stream, EconomyId(0U), Tick(9U));
    assert(samples.size() == 8U);
    assert(std::any_of(
        samples.begin(), samples.end(),
        [](const M11MetricSample &sample) {
