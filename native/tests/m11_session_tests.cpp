...
[[nodiscard]] M11PolicyProposal no_action(const M11DecisionContext &context,
    ...
    for (const auto &version : context.policy_versions) {
        proposal.based_on_policy_versions.push_back({version.lever, version.version});
    }
    return proposal;
}
...
void test_null_occupants_advance_complete_boundary() {
    ...
    assert(advanced.get_if()->opened_context_ids.size() == kM11DecisionGroupCount * 2U);
    assert(advanced.get_if()->decisions.size() == kM11DecisionGroupCount * 2U);
    assert(session.get_if()->phase() == M11BoundaryPhase::boundary_start);
...
void test_human_pause_consumes_no_tick() {
    ...
    auto session = M11ControlledSession::create(engine(), std::move(spec));
    assert(session.ok());
    const auto opening_digest = session.get_if()->engine().world().digest();
    ...
    assert(session.get_if()->engine().world().digest() == opening_digest);

    std::vector<std::string> unanswered;
    for (const auto &context_id : session.get_if()->state().opened_context_ids) {
        const auto *context = session.get_if()->coordinator().find_context(context_id);
        assert(context != nullptr);
        if (context->seat == "treasury" && !context->answered_by_proposal.has_value()) {
    ...
    for (std::size_t index = 0U; index < unanswered.size(); ++index) {
        const auto *context =
            session.get_if()->coordinator().find_context(unanswered[index]);
        assert(context != nullptr);
        auto decision = session.get_if()->submit_human_proposal(
            no_action(*context, "human-hold-" + std::to_string(index)), "player");
        assert(decision.ok());
    }
    assert(session.get_if()->phase() == M11BoundaryPhase::ready_to_commit);
    auto committed = session.get_if()->advance_until_decision({1U, true});
    assert(committed.ok());
    assert(session.get_if()->tick() == Tick(1U));
}
...
void test_scheduled_policy_reaches_effective_world() {
    ...
    M11ControllerRunSpec spec;
    spec.assignments.push_back({EconomyId(0U), "treasury", std::move(scheduled)});
    auto session = M11ControlledSession::create(engine(), std::move(spec));
    assert(session.ok());
    auto first = session.get_if()->advance_until_decision({1U, true});
    assert(first.ok());
    const auto accepted = std::find_if(
        first.get_if()->decisions.begin(), first.get_if()->decisions.end(),
        [](const M11PolicyDecision &decision) {
            return decision.status == M11DecisionStatus::accepted_pending &&
                   decision.effective_at == Tick(7U);
        });
    assert(accepted != first.get_if()->decisions.end());

    auto through_effective = session.get_if()->advance_until_decision({7U, false});
    assert(through_effective.ok());
    assert(session.get_if()->tick() == Tick(8U));
    auto policy = session.get_if()->engine().world().domestic_policy(EconomyId(0U));
    assert(policy.ok());
    assert(policy.get_if()->fiscal_monetary.government_deficit_target == 0.02);
    const auto *decision =
        session.get_if()->coordinator().find_decision(accepted->decision_id);
    assert(decision != nullptr);
    assert(decision->status == M11DecisionStatus::effective);
    auto replayed = replay_m11_decisions(session.get_if()->events().events());
    assert(replayed.ok());
    assert(*replayed.get_if() ==
           std::vector<M11PolicyDecision>(
               session.get_if()->coordinator().decisions().begin(),
               session.get_if()->coordinator().decisions().end()));
    std::vector<M11ControllerEvent> corrupt(session.get_if()->events().events().begin(),
                                            session.get_if()->events().events().end());
    const auto derived = std::find_if(
        corrupt.begin(), corrupt.end(), [](const M11ControllerEvent &event) {
...
void test_native_rl_occupant_runs_without_python() {
    ...
    rl.artifact_path = std::filesystem::path(MACRO_SIM_SOURCE_DIR) /
                       "macro_sim/rl/artifacts/fiscal_stabilization_v1.msrl";
    M11ControllerRunSpec spec;
    spec.assignments.push_back({EconomyId(0U), "treasury", std::move(rl)});
    auto session = M11ControlledSession::create(engine(), std::move(spec));
    assert(session.ok());
    const auto *runtime = session.get_if()->seat(EconomyId(0U), "treasury");
    ...
    assert(session.get_if()->tick() == Tick(1U));
    assert(std::any_of(
        advanced.get_if()->decisions.begin(), advanced.get_if()->decisions.end(),
        [](const M11PolicyDecision &decision) {
            return decision.status == M11DecisionStatus::accepted_noop ||
                   decision.status == M11DecisionStatus::accepted_pending;
        }));
    auto replayed = replay_m11_decisions(session.get_if()->events().events());
    assert(replayed.ok());
    assert(*replayed.get_if() ==
...
void test_checkpoint_restores_pending_execution_exactly() {
    ...
    auto original_advance = session.get_if()->advance_until_decision({12U, false});
    auto restored_advance = restored.get_if()->advance_until_decision({12U, false});
    ...
    assert(restored.get_if()->events().head_hash() ==
           session.get_if()->events().head_hash());
    const auto restored_releases = restored.get_if()->releases().releases();
    const auto original_releases = session.get_if()->releases().releases();
    assert(restored_releases.size() == original_releases.size());
    assert(std::equal(restored_releases.begin(), restored_releases.end(),
...
void test_checkpoint_preserves_human_pause_and_embedded_rl() {
    ...
    human_spec.assignments.push_back({EconomyId(0U), "treasury", std::move(human)});
    auto human_session = M11ControlledSession::create(engine(), std::move(human_spec));
    assert(human_session.ok());
    auto paused = human_session.get_if()->advance_until_decision({1U, true});
    assert(paused.ok() && paused.get_if()->awaiting_human);
    auto human_checkpoint = save_m11_checkpoint(*human_session.get_if());
    assert(human_checkpoint.ok());
    auto restored_human = load_m11_checkpoint(*human_checkpoint.get_if());
    assert(restored_human.ok());
    assert(restored_human.get_if()->phase() == M11BoundaryPhase::awaiting_human);
    ...
    rl.artifact_path = std::filesystem::path(MACRO_SIM_SOURCE_DIR) /
                       "macro_sim/rl/artifacts/fiscal_stabilization_v1.msrl";
    rl_spec.assignments.push_back({EconomyId(0U), "treasury", std::move(rl)});
    auto rl_session = M11ControlledSession::create(engine(), std::move(rl_spec));
    assert(rl_session.ok());
    auto rl_checkpoint = save_m11_checkpoint(*rl_session.get_if());
    assert(rl_checkpoint.ok());
    auto restored_rl = load_m11_checkpoint(*rl_checkpoint.get_if());
    assert(restored_rl.ok());
    const auto *runtime = restored_rl.get_if()->seat(EconomyId(0U), "treasury");
    assert(runtime != nullptr && runtime->artifact.has_value());
    assert(!runtime->artifact->source_bytes().empty());
    assert(restored_rl.get_if()->advance_until_decision({1U, true}).ok());
}

void test_seat_archive_restore_and_checkpoint_idempotency() {
    auto session = M11ControlledSession::create(engine());
    assert(session.ok());
    const auto *opening = session.get_if()->seat(EconomyId(0U), "treasury");
    assert(opening != nullptr);
    assert(opening->assignment.occupant.kind == M11OccupantKind::null_occupant);

    M11OccupantSpec human;
    human.kind = M11OccupantKind::human_queue;
    human.occupant_id = "cabinet-player";
    const M11SeatAssignmentRequest assignment{
        "assign-cabinet", "operator", EconomyId(0U), "treasury", human,
    };
    ...
    assert(session.get_if()->state().archived_seat_occupants.size() == 1U);

    auto repeated = session.get_if()->assign_seat(assignment);
    assert(repeated.ok());
    ...
    const auto before_invalid = save_m11_checkpoint(*session.get_if());
    ...
    assert(!invalid.ok());
    const auto after_invalid = save_m11_checkpoint(*session.get_if());
    assert(after_invalid.ok());
    assert(*before_invalid.get_if() == *after_invalid.get_if());

    auto checkpoint = save_m11_checkpoint(*session.get_if());
    assert(checkpoint.ok());
    auto restored_session = load_m11_checkpoint(*checkpoint.get_if());
    assert(restored_session.ok());
    assert(restored_session.get_if()->state().archived_seat_occupants.size() == 1U);
    assert(restored_session.get_if()->state().seat_operations.size() == 1U);

    const M11SeatRestoreRequest restoration{
        "restore-opening", "operator", EconomyId(0U), "treasury", null_archive,
    };
    auto restored = restored_session.get_if()->restore_seat(restoration);
    assert(restored.ok());
    assert(!restored.get_if()->repeated);
    assert(restored.get_if()->archived_occupant_id.has_value());
    assert(restored_session.get_if()
               ->seat(EconomyId(0U), "treasury")
               ->assignment.occupant.kind == M11OccupantKind::null_occupant);
    assert(restored_session.get_if()->state().archived_seat_occupants.size() == 1U);

    auto restore_retry = restored_session.get_if()->restore_seat(restoration);
    ...
    const auto final_checkpoint = save_m11_checkpoint(*restored_session.get_if());
    assert(final_checkpoint.ok());
    auto final_round_trip = load_m11_checkpoint(*final_checkpoint.get_if());
    assert(final_round_trip.ok());
    assert(final_round_trip.get_if()->state().seat_operations.size() == 2U);
}
...
void test_controlled_shocks_are_authorized_atomic_and_idempotent() {
    ...
    const auto event_count = session.get_if()->events().next_sequence();
    auto repeated = session.get_if()->schedule_shock(request);
    assert(repeated.ok());
    assert(repeated.get_if()->repeated);
    assert(repeated.get_if()->event_sequence == accepted.get_if()->event_sequence);
    ...
    assert(session.get_if()->schedule_shock(request).status().code() ==
    ...
    assert(session.get_if()->tick() == Tick(1U));
    assert(
        session.get_if()->engine().world().last_metrics().external[0U].active_shocks >
        0U);
}
...
void test_shock_scheduled_during_human_pause_starts_next_boundary() {
    ...
    spec.assignments.push_back({EconomyId(0U), "treasury", std::move(human)});
    auto session = M11ControlledSession::create(engine(), std::move(spec));
    assert(session.ok());
    auto paused = session.get_if()->advance_until_decision({1U, true});
    assert(paused.ok() && paused.get_if()->awaiting_human);
