...
[[nodiscard]] BondContract bond_contract(
    std::uint64_t issuer,
    std::uint64_t account,
    std::uint64_t issue,
    std::uint64_t maturity,
    double face
) {
    return {
        {},
...
[[nodiscard]] EquityContract equity_contract(
    std::uint64_t firm,
    std::uint64_t account,
    double shares
) {
...
void test_contracts_and_indexes() {
    SecurityBook book;
    const auto bond = book.issue_bond(
        bond_contract(1, 1, 0, 8, 100.0),
    ...
    const auto equity = book.create_equity(
        equity_contract(1, 2, 100.0),
        holdings
    );
    assert(equity.ok());
    assert(book.validate(1.0e-9).ok());
    assert(
        book.securities_for_issuer(
            OwnerId::firm(macro_sim::FirmId(1))
        ).size()
        == 1
    );
    ...
    auto status = book.transfer_units(
        ...
        OwnerId::household(HouseholdId(1)),
        OwnerId::household(HouseholdId(3)),
        15.0,
        Money(31.5)
    );
    assert(status.ok());
...
void test_bank_index() {
    SecurityBook book;
    const auto bond = book.issue_bond(
        bond_contract(1, 1, 0, 2, 25.0),
        OwnerId::bank(BankId(3)),
        Money(25.0)
    );
    assert(bond.ok());
...
void test_fuzzed_mutations() {
    ...
    for (std::uint64_t index = 1; index <= 8; ++index) {
        ...
        const auto created = book.create_equity(
            equity_contract(index, 100 + index, 100.0),
            holdings
        );
        assert(created.ok());
    ...
    for (std::size_t step = 0; step < 2'000; ++step) {
        ...
        if (held > 1.0e-8) {
            ...
            const double issued = 0.01
                * static_cast<double>((next() % 10) + 1);
            assert(
                book.issue_equity_units(
                    equity,
                    destination,
                    issued,
                    Money(issued * 2.0)
