...
[[nodiscard]] Harness build(M6SimulationSpec spec = base_spec()) {
    auto initialization =
        macro_sim::simulation::build_m6_genesis(spec);
    if (!initialization.ok()) {
        std::cerr << "M6 genesis failed: "
...
[[nodiscard]] macro_sim::Result<
    macro_sim::simulation::M6AdvanceResult
> advance(
    Harness& harness,
    std::uint64_t count,
...
void test_validation_and_prices() {
    ...
    assert(std::abs(
        macro_sim::simulation::bond_price(
            100.0,
            1,
            0.0,
            0.0
    ...
    assert(std::abs(
        macro_sim::simulation::residual_income_fundamental(
            100.0,
            2.0,
            10.0,
            0.10
...
void test_genesis_and_multiday_advance() {
    ...
    assert(macro_sim::simulation::validate_m6_state(
        harness.root,
        harness.real_runtime,
        harness.monetary_runtime,
        harness.runtime,
    ...
    if (!result.ok()) {
        std::cerr << "M6 advance failed: "
                  << result.status().message()
                  << " at tick " << harness.tick.value() << "\n";
        for (const auto& pnl : harness.monetary_scratch.bank_pnl_) {
            const auto capital = std::find_if(
                harness.monetary_scratch.bank_capital_.begin(),
                harness.monetary_scratch.bank_capital_.end(),
                [&](const auto& value) {
                    return value.bank == pnl.bank;
                }
            );
            if (capital
                != harness.monetary_scratch.bank_capital_.end()) {
    ...
    assert(std::abs(result.get_if()->metrics.clearing_residual)
        < 1.0e-7);
    assert(macro_sim::simulation::validate_m6_state(
        harness.root,
        harness.real_runtime,
...
void test_fault_is_atomic() {
    ...
    const auto lifecycle_counter =
        harness.runtime.lifecycle_rng_counter;
    ...
    assert(
        harness.runtime.lifecycle_rng_counter == lifecycle_counter
    );
}
...
void test_forced_firm_exit_and_bank_entry() {
    ...
    if (!result.ok()) {
        std::cerr << "M6 forced exit failed: "
                  << result.status().message() << "\n";
    }
    assert(result.ok());
    ...
    if (!result.ok()) {
        std::cerr << "M6 forced bank entry failed: "
                  << result.status().message() << "\n";
    }
    assert(result.ok());
