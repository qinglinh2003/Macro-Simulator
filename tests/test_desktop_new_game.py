...
def test_native_product_baseline_audit_covers_every_mapped_field() -> None:
    ...
    assert payload["field_count"] == 252
    ...
    assert rows["consumption_strata"]["status"] == "exact"
    assert rows["mpc_dispersion"]["status"] == "exact"
    ...
    assert rows["bank_assignment"]["status"] == "exact"
    assert rows["bank_enabled"]["status"] == "exact"
    assert rows["household_interest_arrears"]["status"] == "exact"
    assert rows["run_market_weight"]["status"] == "exact"
