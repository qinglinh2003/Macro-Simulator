...
def test_switching_closes_the_gap_and_conserves():
    """Switching on (a bit less conservative so switches fire in the test window):
    it FIRES, moves necessity capital share DOWN toward its demand share (closing the
    measured lag), conserves, and stays quiet (no oscillation)."""
    ...
    # capital reallocates toward the growing (luxury) sector: necessity capital share falls
    assert mature(on, "necessity_capital_share") < mature(off, "necessity_capital_share"), \
        "switching did not move capital out of the over-capitalized necessity sector"
    # conserving
