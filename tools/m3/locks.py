#!/usr/bin/env python3
"""Generate or verify deterministic M3 contract, vendor, and source locks."""
...
M2_CONTRACT_SHA256 = "d5a49325d6558bb8802209c1e89f512f6cc38fd8081b3347011c0d88e8fc3241"
HASHES_PATH = ROOT / "schemas/m3/hashes.lock.json"
SOURCE_PATH = ROOT / "schemas/m3/source.lock.json"
VENDOR_PATH = ROOT / "schemas/m3/vendor.lock.json"
...
def file_records(paths: Iterable[Path]) -> tuple[list[dict[str, Any]], str]:
    ...
    for path in sorted(set(paths), key=lambda item: item.relative_to(ROOT).as_posix()):
        ...
        records.append({"byte_count": len(content), "path": relative, "sha256": digest})
...
def build_vendor_lock() -> dict[str, Any]:
    ...
    paths = [path for path in (ROOT / "native/vendor").rglob("*") if path.is_file()]
    files, aggregate = file_records(paths)
    return {
        "aggregate_sha256": aggregate,
        "dependencies": selected,
        "files": files,
        "schema_version": "m3-vendor-lock-v1",
    }


def contract_paths() -> tuple[Path, ...]:
    excluded = {HASHES_PATH, SOURCE_PATH, VENDOR_PATH}
    return tuple(
        path
        for path in (ROOT / "schemas/m3").rglob("*")
...
def build_contract_lock() -> dict[str, Any]:
    ...
    return {
        ...
        "schema_version": "m3-contract-lock-v1",
    }
...
def source_paths() -> tuple[Path, ...]:
    exact = {
        ...
        ROOT / "docs/cpp_engine_m3_execution_plan_v33.md",
        ROOT / "pyproject.toml",
        ROOT / "uv.lock",
    }
    paths = set(exact)
...
def build_source_lock() -> dict[str, Any]:
    ...
    return {
        ...
        "schema_version": "m3-source-lock-v1",
    }
