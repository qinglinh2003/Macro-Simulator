#!/usr/bin/env python3
"""Verify exact M8 continuation after checkpoint transfer to a new process."""
...
def write_mode(native_dir: Path, checkpoint: Path) -> int:
    ...
    session = native.M8Session(2808)
    session.initialize_m8(make_spec(native, 2808))
    session.advance_m8_ticks(14)
    checkpoint.write_bytes(session.checkpoint())
    session.advance_m8_ticks(19)
...
def orchestrate(native_dir: Path) -> int:
    with tempfile.TemporaryDirectory(prefix="macro-sim-m8-") as directory:
        checkpoint = Path(directory) / "continuation.m8cp"
    ...
    print(
        json.dumps(
            {
                "schema_version": "m8-fresh-process-result-v1",
                "status": "passed",
                "tick": direct["tick"],
            },
            sort_keys=True,
